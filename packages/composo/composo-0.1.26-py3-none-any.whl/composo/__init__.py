...
class BackendEventGress:
	...
	def __init__(self,app_registration):
		...
		if os.environ.get(_B,_C)==_D:packageLogger.info('Connecting to Composo local');self.backend_url='http://localhost:8000'
		elif os.environ.get(_B,_C)=='dev':packageLogger.info('Connecting to Composo dev');self.backend_url='https://composo-prod-backend-composo-dev-backend.azurewebsites.net'
		elif os.environ.get(_B,_C)=='test':packageLogger.info('Connecting to Composo test');self.backend_url='http://composo-prod-backend-composo-test-backend.azurewebsites.net'
...
def experiment_controller(func,demo_args,demo_kwargs,demo_globals,api_key='cp-XXX_FAKE_KEY_FOR_TESTING_XXXX',event_ingress=_A,event_egress=_A,poll_wait_time=3):
	...
	if os.environ.get(A)is not _A:
		try:poll_wait_time=int(os.environ.get(A));assert poll_wait_time>1 and poll_wait_time<10;poll_wait_time=int(os.environ.get(A))
		except:pass
					...
					for case in event.payload.cases:
						...
						try:
							...
							if app_is_streaming:
								...
								report_case_intermediate=lambda current_value:report_case(CaseResult(case_id=case.case_id,value=current_value,value_type='str',error=_A,output_stream_incomplete=True));final_result=stream_handler(ret,report_case_intermediate)
							else:final_result=ret
							case_result=CaseResult(case_id=case.case_id,value=final_result,value_type=str(type(final_result).__name__),error=_A,output_stream_incomplete=False);print('Setting output_stream_incomplete to False');print(case_result);report_case(case_result)
						except ComposoUserException as e:conditional_raise(e);case_result=CaseResult(case_id=case.case_id,value=_A,value_type=_A,error='ERROR: '+str(e));report_case(case_result)
						except Exception as e:conditional_raise(e);packageLogger.debug(f"Unidentified exception caught with case {case}: {str(e)}");case_result=CaseResult(case_id=case.case_id,value=_A,value_type=_A,error='ERROR: The composo package has failed with an unidentified error. Please contact composo support.');report_case(case_result)
		...
		def actual_decorator(func):
			def wrapped_func(*args,**kwargs):
				...
				if not hasattr(Composo,'activated'):
					...
					permissable_return_types=['int','float','str',_F,_E];result_type=type(result).__name__
					...
					packageLogger.info(B);packageLogger.info('######### Your Composo API Key #########');packageLogger.info('### '+api_key+' ###');packageLogger.info(B);experiment_controller(func,args,kwargs,func.__globals__,api_key=api_key);return result
