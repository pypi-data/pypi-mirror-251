...
def run_enrich_multiome(args):
    ...
    if atac_path.endswith('.tsv'):
        ...
        atac_landscape = sc.read_10x_h5(atac_path, gex_only=False)
    elif feature_matrix_path.endswith('.h5ad'):
    ...
    if barcodes == '0':
        ...
        if atac_file_type='fragment':
            scripro.get_supercell_fragment(cellgroup,'.',atac_path,chunksize = 10000000)
            scripro.process_tsv('./supercell_fragment/', species)
        else: 
        ...
        scglue.data.get_gene_annotation(feature_matrix, gtf="../data/gencode.v43.chr_patch_hapl_scaff.annotation.gtf.gz",gtf_by="gene_name")
...
def add_enrich_parser_multiome( subparsers ):
    ...
    group_input.add_argument( "-s", "--species", dest = "species", choices= ['hg38', 'mm10'], required = True,
                              help = 'Species. "hs"(human) or "mm"(mouse). REQUIRED.' )
    roup_input.add_argument( "-a", "--atac_file_type", dest = "atac_file_type", choices= ['fragment', 'matrix'], required = True,
                              help = 'Species. "hs"(human) or "mm"(mouse). REQUIRED.' )
