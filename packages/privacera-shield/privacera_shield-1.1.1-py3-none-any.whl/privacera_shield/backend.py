...
class ShieldRestHttpClient:
    ...
    def init_shield_server(self) -> None:
        ...
        if response.status == 200:
            ...
            _logger.error(ErrorMessage.SHIELD_SERVER_INITIALIZATION_FAILED.format(response_status=response.status,
                                                                                  response_data=response.data))
            raise PAIGException(ErrorMessage.SHIELD_SERVER_INITIALIZATION_FAILED, response_status=response.status,
                                response_data=response.data)
