...
from common import logger, download_file_local, delete_local_file
import requests
import os
import boto3
...
def heroku_upload_file_from_url(file_url, folder, heroku_config_dict=None, bnewname=False):
    ...
    if bnewname:
        ...
        # Get the file extension from the original file URL
        file_extension = os.path.splitext(file_url)[1]
        print(f"File extension: {file_extension}")

        # Create a new filename using the timestamp and original file extension
        new_filename = f"{timestamp}{file_extension}{folder}"
        print(f"New filename: {new_filename}")

        # Rename the local file with the new filename
        new_file_path = os.path.join(os.path.dirname(file_path), new_filename)
        os.rename(file_path, new_file_path)
        print(f"New filepath: {file_path}")
