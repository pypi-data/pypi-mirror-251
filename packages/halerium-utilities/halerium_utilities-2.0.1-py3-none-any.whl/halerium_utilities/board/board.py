...
class Board:
    ...
    def _remove_all_connections_to_card_id(self, card_id):
        ...
        # # loop backwards to be able to pop indices
        # for index in range(len(self.connections)-1, -1, -1):
        #     cur_id = self.connections[index].id
        #     if cur_id in connection_ids:
        #         self.connections.pop(index)  # remove connection from board
        #         connection_ids.remove(cur_id)  # remove id form to remove set
        #     print(len(connection_ids))
        #     print(connection_ids)
        #     if len(connection_ids) == 0:  # if the to remove set is empty we can stop
        #         return None
        #
        # # if the to remove set is not empty at the end something went wrong
        # if len(connection_ids) != 0:
        #     raise IdNotFoundError(f"Connections {connection_ids} not found.")
