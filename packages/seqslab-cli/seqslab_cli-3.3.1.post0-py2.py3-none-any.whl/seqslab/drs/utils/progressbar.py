...
class ProgressBarObject:
    ...
    def __init__(self, total_tasks: int, progress_name: str = "Progress", bar_length: int = 30, log: bool = True):
    ...
    @staticmethod
    def __bar_string_format(spinner: str, progress_name: str, bar: str, space: str, percentage: int,
                            complete_tasks: int,
                            total_tasks: int) -> str:
    ...
    def update(self, complete_tasks) -> str:
        if self.total_tasks != 0:
            ...
            progress = self.__bar_string_format(
                self.spinner[self.clocker], self.progress_name, "█" * self.bar_cnt,
