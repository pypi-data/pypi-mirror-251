...
def establish(context: Context, pro_clazz: ProClazz):
    exec('_{}(context, pro_clazz)'.format(pro_clazz.name.lower()))


def _gw698(context: Context, pro_clazz: ProClazz, retry_times=3):
    try:
        # 1. 读取esam参数
        result = pro.encode(
            func.to_dict(protocol=pro_clazz.name, comm_addr=context.meter.addr,
        ...
        data = pro.decode(func.to_dict(protocol=pro_clazz.name, frame=frame, session_key=context.test_sn))
        # 2. 协商密钥
        em.handle(pro_clazz.name, 'negotiate', data.get('result'))
        # 3. 连接
        result = pro.encode(
            func.to_dict(protocol=pro_clazz.name, comm_addr=context.meter.addr,
        ...
        data = pro.decode(func.to_dict(protocol=pro_clazz.name, frame=frame, session_key=context.test_sn))
        # 4. 加密
        em.handle(pro_clazz.name, 'secret', data.get('result'))
        ...
        if retry_times <= 0:
            ...
            base.sleep(5)
            _gw698(context, pro_clazz, retry_times)
