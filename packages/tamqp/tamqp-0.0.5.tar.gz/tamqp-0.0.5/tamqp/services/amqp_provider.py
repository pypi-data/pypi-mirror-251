...
class AMQPProvider(EncodedProvider, AMQPProviderConfig, AMQPUtils):
    def __init__(self, parameters: AMQPInit, key: str) -> None:
        super().__init__(key)
    ...
    def establish_connection_and_channels(self):
        while True:
            try:
                ...
                print(f"Error while establishing connection: {e}, retrying...")
    ...
    def publish(self, routing_key: str, message: str, common_id: str = str(uuid.uuid4()),
                encoded: bool = True, exchange: Optional[Exchange] = None, sensitive_content: bool = False, delay: int = 0) -> None:
        try:
