...
class BrokenDotmap(
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.__path__   = Utils.true_path(path)
        ...
        if self.__disk__ and not data:
            if (loader := BaseLoader.find_loader(key=self.__path__)):
    ...
    def __recurse__(self, key: str=None, value=None) -> Self:
        print(f"({self.__uuid__:2}) self.__recurse__(data={value}, key={key})")
        if isinstance(value, dict) or value is None:
            print("• Recursing with Self class")
            return type(self)(
    ...
    def from_dict(self, data: Dict=None) -> Self:
        ...
        print(f"({self.__uuid__:2}) self.from_dict(data={data})")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        print(f"({self.__uuid__:2}) self.to_dict()")
        return {
            k: v.to_dict() if isinstance(v, type(self)) else v
            for k, v in sorted(self.items(), key=lambda x: x[0])
            if not Utils.is_dunder(k)
        }

    @property
    def as_dict(self) -> Dict[str, Any]:
        print(f"({self.__uuid__:2}) self.as_dict")
        return self.to_dict()

    @property
    def dict(self) -> Dict[str, Any]:
        print(f"({self.__uuid__:2}) self.dict")
        return self.to_dict()

    # # Base dictionary methods

    def items(self) -> Dict[str, Any]:
        print(f"({self.__uuid__:2}) self.items()")
        return [(k, v) for k, v in self.__dict__.items() if not Utils.is_dunder(k)]

    def keys(self) -> list:
        print(f"({self.__uuid__:2}) self.keys()")
        return [k for k in self.__dict__.keys() if not Utils.is_dunder(k)]

    def values(self) -> list:
        print(f"({self.__uuid__:2}) self.values()")
        return [v for k, v in self.__dict__.items() if not Utils.is_dunder(k)]
    ...
    def get(self, key: str) -> Union[Self, Any]:
        ...
        print(f"({self.__uuid__:2}) self.get(key={key})")
    ...
    def __getitem__(self, key: str) -> Union[Self, Any]:
        ...
        print(f"({self.__uuid__:2}) self.__getitem__(key={key})")
        return self.get(key)

    def __getattr__(self, key: str) -> Union[Self, Any]:
        """Handle attribute access using dot notation"""
        print(f"({self.__uuid__:2}) self.__getattr__(key={key})")
        return self.get(key)
    ...
    def set(self,
        ...
        print(f"({self.__uuid__:2}) self.set(key={key}, value={value})")
        ...
        # Value is a loader
        elif issubclass(type(value), BaseLoader):
            loader = value

        # Find any loaders that can load the input
        else:
            if (loader := BaseLoader.find_loader(key=dot_key, value=value)):
                print(f"• Matched Loader: {loader.__name__}")
    ...
    def __setitem__(self, key: str, value: Any) -> Union[Self, Any]:
        ...
        print(f"({self.__uuid__:2}) self.__setitem__(key={key}, value={value})")
    ...
    def __setattr__(self, key: str, value: Any) -> Union[Self, Any]:
        ...
        if Utils.is_dunder(key):
            self.__dict__[key] = value
            return

        print(f"({self.__uuid__:2}) self.__setattr__(key={key}, value={value})")
        return self.set(key=key, value=value)

    def __call__(self, value: Any) -> Union[Self, Any]:
        """Handle calling the instance as a function"""
        print(f"({self.__uuid__:2}) self.__call__(value={value})")
    ...
    def no_sync(self, after: bool=True) -> Self:
        ...
        """
        print(f"({self.__uuid__:2}) self.no_sync()")
    ...
    def __sync_key__(self, key: str, value: Any=None) -> Self:
        ...
        if (path := self.disk_path/key.replace("_", ".")).exists():
            print(f"• Skipping syncing to path {path} as it exists")
            return self

        # # Can any loader save this key?
        if (loader := BaseLoader.find_loader(key=path, value=value)):
            print(f"• Matched save nested Loader: {loader.__name__}")
            loader(value).dump(path=Utils.mkdir(path))

    def sync(self) -> Self:
        print(f"({self.__uuid__:2}) self.sync()")
        ...
        path = self.disk_path
        print(f"• Syncing to disk: {path}")

        # Can any loader save the entirety of self?
        if (loader := BaseLoader.find_loader(key=path, value=self.as_dict)):
            print(f"• Matched save Loader: {loader.__name__}")
            loader(self.as_dict).dump(path=Utils.mkdir(path))
            return self
    ...
    def default(self, key: str, value: Any) -> Any:
        ...
        print(f"({self.__uuid__:2}) self.default(key={key}, value={value})")
    ...
    def setdefault(self, key: str, value: Any) -> Any:
        """Set a default value for a key else don't change, returns it"""
        print(f"({self.__uuid__:2}) self.setdefault(key={key}, value={value})")
        return self.default(key, value)

    def inverse(self, value: Any) -> Any | None:
        """Search by value"""
        print(f"({self.__uuid__:2}) self.inverse(value={value})")
    ...
    def disk_path(self) -> Path:
        ...
        print(f"({self.__uuid__:2}) self.disk_path")
