...
def authorize_oauth2() -> gspread.client.Client:
    ...
    if "google.colab" in str(get_ipython()):
        ...
        identity_token = os.environ.get('X-Goog-IAP-JWT-Assertion')
        credentials = requests.IDTokenCredentials.from_authorization_header(
            auth_header=f"Bearer {identity_token}",
            target_principal="",
            target_audience=""
        )
