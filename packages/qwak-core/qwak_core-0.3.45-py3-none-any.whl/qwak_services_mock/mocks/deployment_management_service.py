...
class DeploymentManagementServiceMock(DeploymentManagementServiceServicer):
    ...
    def given_deployment(self, model_id: str,
                environment_id: str, status: int, model_uuid: str, deployment_id: str) -> str:

        deployment_details = (
            self.active_deployments.get(
                (model_id, "", environment_id)
            )
        )
        ...
        self.model_uuid_to_active_deployment_ids = defaultdict(
            list
        )
        self.model_uuid_to_active_deployment_ids[
            model_uuid
