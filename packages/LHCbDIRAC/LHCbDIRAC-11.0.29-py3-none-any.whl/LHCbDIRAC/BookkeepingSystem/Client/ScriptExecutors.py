...
def _scaleValue(val, units, factor=1000):
...
def scaleiSize(size):
    """Return size in appropriate unit."""
    return _scaleValue(size, ("Bytes", "kiB", "MiB", "GiB", "TiB", "PiB"), factor=1024)
    ...
    for prod in prodList:
        ...
        for name, value in zip(paramNames, records):
            if name == "NbofFiles":
                ...
                size_bytes = value
                sizePerEvt = f"({size_bytes / nevts / 1000.0:.1f} kB per evt)" if nevts and nDatasets == 1 else ""
                size, sizeUnit = scaleSize(size_bytes)
                gLogger.notice("Total size".ljust(tab) + ": ", f"{size:.3f} {sizeUnit} {sizePerEvt}")
                isize, isizeUnit = scaleiSize(size_bytes)
                isizePerEvt = f"({size_bytes / nevts / 1024.0:.1f} kiB per evt)" if nevts and nDatasets == 1 else ""
                gLogger.notice("Total size (1024)".ljust(tab) + ": ", f"{isize:.3f} {isizeUnit} {isizePerEvt}")
