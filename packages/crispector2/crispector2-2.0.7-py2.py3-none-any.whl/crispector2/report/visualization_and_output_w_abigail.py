...
def plot_editing_activity(result_df: AlgResultDf, confidence_interval: float, editing_threshold: float, html_d: Dict,
    ...
    for idx in range(plot_num):
        ...
        #axes[idx].annotate(ha='center', va='bottom')
        ...
        if idx == 0:
            ...
            axes[idx].legend(loc='upper right', bbox_to_anchor=(0.8, 0.8, 0.4, 0.4))

        accum_bars += max_bars

    if allele_display_option == "all":
        if edit_df.shape[0] > 0:
            fig.savefig(os.path.join(output, 'editing_activity_all.png'), box_inches='tight', dpi=dpi)
            fig.savefig(os.path.join(output, 'editing_activity_all.svg'), pad_inches=1, box_inches='tight')
        ...
        if edit_df.shape[0] > 0:
            fig.savefig(os.path.join(output, 'editing_activity_allele_only.png'), box_inches='tight', dpi=dpi)
            fig.savefig(os.path.join(output, 'editing_activity_allele_only.svg'), pad_inches=1, box_inches='tight')
