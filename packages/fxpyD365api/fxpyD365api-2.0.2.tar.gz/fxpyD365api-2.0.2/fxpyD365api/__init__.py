...
class BaseApiWrapper:
    def __init__(self, crmorg=None, token=None, tenant=None, client_id=None,
        ...
        if token:
            self._token = token
            self.token_expires_at = datetime.datetime.fromisoformat(token['expiresOn'])
        else:
    ...
    def _get_token(self, tenant, client_id, client_secret):
        ...
        token = app.acquire_token_for_client(scopes=[f'{self.get_scope(platform)}/.default'])
        self.token_expires_at = datetime.datetime.now() + datetime.timedelta(seconds=token['expires_in'] - 60)
        return token
