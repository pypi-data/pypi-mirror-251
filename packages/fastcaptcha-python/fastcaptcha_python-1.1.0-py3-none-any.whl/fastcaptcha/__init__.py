...
from fastcaptcha.exeptions import FastCaptchaException
from fastcaptcha.helpers import convert_img_to_base64


class FastCaptchaClient:
    def __init__(self, apikey):
        self.apikey = apikey
        self.BASE_URL = "https://api.fastcaptcha.ru"
    ...
    def get_balance(self):
        ...
        if r.status_code == 200:
            ...
            if data['Code'] == 0:
                ...
                raise FastCaptchaException(f"Error response {r.text}")
        else:
            raise FastCaptchaException("Error " + r.text)
            ...
            if r.status_code == 200:
                ...
                if data['Code'] == 0:
                    if data['Status'] == "SUCCESS":
                        ...
                        raise FastCaptchaException(f"Error: {data['Message']}")
                    time.sleep(time_sleep)
                else:
                    raise FastCaptchaException(f"Error {data['Message']}")
            else:
                raise FastCaptchaException(f"Error {r.text}")
        raise FastCaptchaException("TIMEOUT")
