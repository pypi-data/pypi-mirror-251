...
class TransactionQuery:
	def __init__(self):
		if os.getenv('MPESA_ENV') == 1:
			...
			self.path=os.getenv('MPESA_LIVE_CERT_PATH')
			...
			self.path=os.getenv('MPESA_TEST_CERT_PATH')
			self.identifier_type = os.getenv('MPESA_TEST_TRANSACTION_QUERY_IDENTIFIER_TYPE')

		self.security_credential=generate_security_credential(self.password,self.path)
