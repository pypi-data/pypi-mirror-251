...
from .geral import src_RetornoData,src_RetornoHora,criptoToSHA256
...
def sql_criarConta(tabela:str, campoNome:str, campoEmail:str, campoSenha:str, valorNome:str, valorEmail:str, valorSenha:str):
    try: 
        ...
        if (emailUtilizado == True):
            ...
            if (emailValido == True):
                valorSenha = criptoToSHA256(valorSenha)
...
def sql_alterarConta(tabela:str,campo:str,valor,condicao:str):
    try:
        conexao.commit()

        comando = f'UPDATE {tabela} SET {campo}={valor} WHERE {condicao}'
        cursor.execute(comando)
        conexao.commit()
...
def sql_excluirConta(tabela:str,condicao:str):
    try:
        conexao.commit()
