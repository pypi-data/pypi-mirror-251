...
def declararCostante(nome:str,valor):
    if (len(constantesCriadas)) <=0:
        novaConstante = {'nome':nome,'valor':valor,'qtd':1}
        ...
        for i in range(len(constantesCriadas)):
            ...
            novaConstante = {'nome':nome,'valor':valor}
            constantesCriadas.append(novaConstante)
            return valor
...
def iniciarTipagem(caminho):
    ...
    for i in range(numeroDeLinhas):
        ...
        for item in range(quantidadeConst):
            ...
            if (vezesUsada >=2):
                erro = f'ERRO: A Constante {nomeConstante} está sendo atribuida mais de uma vez'
                print(Fore.RED + erro)
                logAtivado = statusLog()
                if logAtivado == True:
                    configLog(criarLogs=True,exibirLogsEmConsole=False)
