...
from .geral import retornar_data_atual,retornar_hora_atual,criptografar_SHA256
...
def sql_criar_conta(tabela:str, coluna_nome:str, coluna_email:str, coluna_senha:str, nome:str, email:str, senha:str):
    try: 
        if (sql_verificar_uso_email(tabela,email)):
            return [False, 'Email Utilizado']
        else:
            if (verificar_validade_email(email)):
                senha = criptografar_SHA256(senha)
...
def sql_logar_conta(tabela:str, coluna_email:str, coluna_senha:str, email:str, senha:str):
    try:
        conexao.commit()
