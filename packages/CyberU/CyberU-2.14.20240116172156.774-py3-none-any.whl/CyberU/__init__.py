...
def combineimages(inputpath=None, outputpath=None, outputname=None, mode='vertical', reverse=None, filelist=None, cuttop=0, cutbottom=0, cutleft=0, cutright=20, overlap_width=None, overlap_height=None, benchmark=0.75, overlap_ratio=0.2):
    ...
    def do(img1, img2, outputpath=None, ratio1=overlap_ratio, ratio2=overlap_ratio, overlap_width=0, overlap_height=0):
        """
        裁剪后自动识别拼接图片
        ...
        copyfile(img1, temp_path1), copyfile(img2, temp_path2)
...
def out(s=None, d=None, json_str=None, silent=False, target='out',
        mode='rewrite'
        ):
    """
...
def file_operate(mode=None, path=None, IOList=None, encoding=None, with_lock=True, b=None, **leak):
    ...
    try:
        ...
        release_lock(name=path)
        Exit('文件读错误', path, IOList, trace=False)
...
class Json(txt):
    ...
    def merge_key(self, b=None, **leak):
        allkey = []
        for line1 in list(self.l):
            d = jsontodict(line1)
            if not key(d) in self.d:
                allkey.append(key(d))
                continue
            values = []
            for line2 in list(self.l):
                ii = jsontodict(i)
                if not key(ii) == k:
                    continue
                self.l.remove(line2)
...
class RefreshJson(Json):
    ...
    @manage()
    def add(self, d, silent=None, b=None, save=None, **leak):
        if silent == None:
            silent = self.silent
        d = jsontodict(s=d)
...
def Log(s, front=242, font=1, background=238, withtime=True, traceback=False, deleted=False, plain_text=None):
...
get_workspace = disk_path = diskpath = root_path = working_directory = working_space = os.getcwd
getDiskPath = get_disk

def get_work_root():
    return os.getcwd()
root=get_working_root=working_root=get_work_root
...
def Elements(root=None, xpath=None, depth=3, silent=True, method=By.XPATH, strict=None, pre_wait=2,
             complete=True, interval=2, b=None, **leak
             ) -> list:
    """
        ...
        if len(ret):
            ...
            if depth < 0:
                ...
                if depth < 0:
                    self.refresh()
                    warn('未找到元素。已刷新页面。')
                return Elements(xpath=original_xpath, depth=depth - 1,
...
def _browser(url=None, mine=None, silent=None, t=100, mute=True, _type='edge', download_path=None, method=None, manual_manage_webdriver=True, clone=None, b=None,
             **leak) -> selenium.webdriver | bool:
    """
    ...
    if manual_manage_webdriver:
        driver = {
            'edge': lambda: webdriver.Edge(options=options), 'chrome': lambda: webdriver.Chrome(options=options),
        }.get(_type, )()
    else:
        driver = {
...
class Browser():
    ...
    def __init__(self, url=None, download_path=default_page_download_path, silent=False, driver=None, mine=False, family=None, mute=False, clone=None, _js=None,
    ...
    #  爬取论坛的每一页
    def forum(self, url=None, titletail=None, hostname=None, func0=None, func1=None, func2=None, func3=None, minsize=(150, 150), t=3,
              scale=200, mine=None, redownload=None, saveuid=True, look=True, silent=False):
        """
        ...
        if saveuid:
            ...
            page.save(path=shotspath(f'{hostname}/{title}{pastcount}/第1页/'), minsize=minsize, direct=True, look=look, scale=scale, redownload=False)
        ...
        for url in urllist:
            ...
            func3([page])
            if saveuid:
                page.save(shotspath(f'{hostname}/{uid}_{title}{pastcount}/第{count}页/'), minsize=minsize, direct=True, look=look, scale=scale, redownload=False)
            else:
                page.save(path=shotspath(f'{hostname}/{title}{pastcount}/第{count}页/'), minsize=minsize, redownload=False,
    ...
    def click(self, a=None, strict=True, depth=1, b=None, **leak) -> bool:
        ...
        if type(a) in [list]:
            tell = False
            for a in list(a):
                tell = tell or self.click(strict=False, a=a, **(exclude(b, ['strict', 'a'])))
                if tell:
                    break
            return tell
...
if (element) {
    ...
    @manage(bottom=['cutbottom'], left=['cutleft'], right=['cutright'], top=['cuttop'], auto_down=['down'], clipsize=['scale', 'cut_scale'],
    ...
    def fullscreen(self, path=None, clipsize=None, roll_scale=1300, auto_down=False, pause=1, clip=True, clipinterval=0.6, top=180, bottom=200, left=400, right=400, maxheight=10000, _look=False, overwrite=None, click_xpath=[], overlap_ratio=0.3, b=None, **leak):
        ...
        if istype(click_xpath, list):
            click_xpath += [
                '//*[(contains(text(),"阅读全文")or contains(text(),"展开"))and not (contains(text(),"收起"))]']
        path = standarlizedPath(path)
        createpath(path)
...
if useDisk(d='d') in get_settings('develop_computer'):
    # 启用调试模式
    Debug()
