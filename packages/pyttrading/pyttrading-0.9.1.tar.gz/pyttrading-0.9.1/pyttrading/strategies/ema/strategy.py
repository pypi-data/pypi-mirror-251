...
class Strategy: 
    ...
    def plot(self, df=None, show_graph :bool = True, save_figure: bool = False, params=None, title: str = 'Title figure'):
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['ema'], mode='lines', name='EMA', line=dict(color='red', width=1)), row=2, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=df['ema_s'], mode='lines', name='EMA_S', line=dict(color='yellow', width=1)), row=2, col=1)

        fig.add_trace(go.Scatter(x=buy_indices, y=df['ema'].loc[buy_indices], mode='markers', name='Buy', marker=dict(color='green', symbol='circle', size=10)), row=2, col=1)
        fig.add_trace(go.Scatter(x=sell_indices, y=df['ema_s'].loc[sell_indices], mode='markers', name='Sell', marker=dict(color='red', symbol='circle', size=10)), row=2, col=1)
    ...
    def eval(self, df=None, params=params_default):
        ...
        df['ema_s'] = talib.SMA(df['ema'], timeperiod=int(ema_smoothed))
