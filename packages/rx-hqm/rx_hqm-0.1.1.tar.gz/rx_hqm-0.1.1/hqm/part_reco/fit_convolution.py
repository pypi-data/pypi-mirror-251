...
class FitConvolution:
    ...
    def fit(self):
        ...
        plot_kde_path = (
            project_root + f"output/part_reco/fit_convolution/latest/{self._kind}/{self._year}_{self._trigger}/kde.pdf"
        )
        ...
        fitter.fit_data(ntries=1)

        plot_path = (
            project_root
            + f"output/part_reco/fit_convolution/latest/{self._kind}/{self._year}_{self._trigger}/fit_plot.pdf"
        )
        fitter.plot(plot_path=plot_path, leg={"Data": "ee MC"})

        pickle_path = (
            project_root
            + f"data/part_reco/fit_convolution/latest/{self._kind}/{self._year}_{self._trigger}/fit_result.pickle"
        )
