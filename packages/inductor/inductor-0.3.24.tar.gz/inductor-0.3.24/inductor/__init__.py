...
def logger(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        if _logger_decorator_enabled:
            ...
            # - We use backslashes to split the clause, rather than enclosing
            # in parentheses, because enclosing in parentheses apparently causes
            # a SyntaxError when running in Python 3.8.
            # - We actually don't need to capture stdout and stderr if we are
            # not in the primary execution.  However, since stdout and
            # stderr are not suppressed, the user will not be impacted, and
            # we allow stdout and stdout to be captured nonetheless to simplify
            # the code.
