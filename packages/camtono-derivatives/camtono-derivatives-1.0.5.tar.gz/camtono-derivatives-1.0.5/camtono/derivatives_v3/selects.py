...
def generate_filter_selects(definition_outputs, grain, feature_map, input_query_sets, subquery_groupings):
    ...
    for idx, input_query_set in enumerate(input_query_sets):
        ...
        for output in shared_output:
            if 'value' in output.keys():
            ...
            elif output['column_name'] == 'pipeline_uuids' and output.get('transform_sub_queries'):
                ...
                for i in range(len(input_query_set)):
                    value, data_type = apply_transforms(value='t{idx}_pipeline_uuid'.format(idx=i)
                                                        , data_type=data_type
                                                        , transforms=[{'name':'array_agg_distinct'}] #output['transforms']
                                                    )
                    inputs.append(
                        dict(
                            name='t{idx}_pipeline_uuids'.format(idx=i),
                            value=value,
                            type=data_type
                        )
                    )
...
def get_final_pipeline_selects(schema):
    columns = []
