def generate_multi_query_skeleton(input_query_sets, output_queries, grain, selects, group_by, table_prefix,
    ...
    from camtono.derivatives_v3.selects import get_final_pipeline_selects
    from camtono.derivatives_v3.union_query_formatting import remove_pipeline_uuid_columns_from_selects
    ...
    final_select = remove_pipeline_uuid_columns_from_selects(selects['final']) + get_final_pipeline_selects(list(queries[-1].values())[0]['schema'])
    if len(final_select) == 1:
        final_select.append({'name': 't0_pipeline_uuids', 'value': 'pipeline_uuids', 'type': 'ARRAY'})
    final_query = {
...
def generate_filters(input_query_sets, grain, selects: dict, group_by, table_prefix, dialect_name):
    ...
    from camtono.derivatives.selects import extract_select_output
    from camtono.derivatives_v3.union_query_formatting import remove_pipeline_uuid_columns_from_selects, get_list_pipeline_columns, create_select_for_unioned_query, create_union_filter, wrap_union_ast_in_subquery
    queries = [dict(), dict()]
    ...
    if len(queries[1]) > 1:
        ...
        base_select = remove_pipeline_uuid_columns_from_selects(extract_select_output(selects['filter']))
        all_pipelines_columns = get_list_pipeline_columns(queries[1])
        for query_info in queries[1].values():
            union_ast['union_all'].append(
                {
                    "select": create_select_for_unioned_query(base_select, all_pipelines_columns, query_info['schema']),
                    'from': dict(value=query_info['table_name'], name='t0')
                }
            )
        final_ast = wrap_union_ast_in_subquery(base_select, all_pipelines_columns, group_by.get('final'), union_ast)

        base_table_name, table_body = generate_table_query(
            select=create_union_filter(selects['filter'], all_pipelines_columns), ast=final_ast,
            table_prefix=table_prefix, dialect_name=dialect_name
        )
...
def generate_filter(input_idx, input_query_set, grain, table_prefix, selects, group_by, dialect_name):
    ...
    for query_idx, query in enumerate(input_query_set):
        ...
        if sub_ast['from']:
            ...
            sub_ast['from'].append({'cross join': {'name': f't{query_idx}_pipeline_uuid', 'value': {'unnest': f't{query_idx}.pipeline_uuids'}}})
