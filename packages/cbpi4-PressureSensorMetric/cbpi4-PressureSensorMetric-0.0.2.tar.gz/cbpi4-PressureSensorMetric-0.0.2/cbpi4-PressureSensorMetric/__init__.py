...
class PressureSensor(CBPiSensor):
    ...
    async def run(self):
        while self.running is True:
            ...
            liquidLevel = self.convert_m_to_cm(
                (self.convert_kpa_to_pa(pressureValue))
                / (self.GRAVITY * self.RHO_WATER)
            )
            # if the liquid level is lower then 0.5, we interpret the tank as empty, else we have to add the sensor height to the liquid level
            if liquidLevel > 0.5:
            ...
            if self.props.get("sensorType", "Liquid Level") == "Voltage [V]":
                self.value = chan.voltage
            elif self.props.get("sensorType", "Liquid Level") == "Pressure [kPa]":
                self.value = pressureValue
            elif self.props.get("sensorType", "Liquid Level") == "Liquid Level [cm]":
                self.value = liquidLevel
            elif self.props.get("sensorType", "Liquid Level") == "Volume [l]":
                self.value = volume
            else:
                self.value = chan.voltage
