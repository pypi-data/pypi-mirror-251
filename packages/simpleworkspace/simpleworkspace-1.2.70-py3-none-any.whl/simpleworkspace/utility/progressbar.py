...
class ProgressBar:
    ...
    def __init__(self, iterable=None, total=None):
        ...
        self._printAsyncState: ProgressBar._PrintAsyncState = None
    ...
    class _PrintAsyncState:
        def __init__(self, thread:threading.Thread) -> None:
            self._thread = thread
            self._thread_stopEvent = threading.Event()
        ...
        def Cancel(self):
            ...
            if(not self.IsRunning):
                return
            self._thread_stopEvent.set()
            self._thread.join()

        def Wait(self):
            '''awaits until thread closed'''
    ...
    def PrintAsync(self, refreshDelay=1):
        ...
        if self._printAsyncState and self._printAsyncState.IsRunning:
            return self._printAsyncState

        def _PrintAsync(refreshDelay: float):
            stopEvent = self._printAsyncState._thread_stopEvent
            ...
            while not stopEvent.is_set():
                if self.IsFinished():
                    break
                stopEvent.wait(refreshDelay)
                self.Print()
            stopEvent.clear()
            self.Console_CleanUp()
            self._printAsyncState = None
      
        self.setting_printOnIncrement = False

        thread = threading.Thread(target=_PrintAsync, args=[refreshDelay])
        thread.daemon = True
        self._printAsyncState = self._PrintAsyncState(thread)
        thread.start()
        return self._printAsyncState
