...
class GraphFeaturePreprocessor:
    ...
    Note that ``time_window`` can be set by calling ``set_params``. However, this parameter
    can be overwritten if a graph pattern requires an even larger time window, i.e., if
    ``<pattern-name>_tw`` is larger than ``time_window`` for a pattern that is enabled.
    Lastly, the edge list does not have to be sorted. However, the pattern detection is most
    effective when the edges in the list are sorted in the increasing order of their timestamps.
    ...
    params : dict
        ...
        time_window : int, default=10*3600
            ...
            The unit of time is the same as the one used for timestamps of edges.
