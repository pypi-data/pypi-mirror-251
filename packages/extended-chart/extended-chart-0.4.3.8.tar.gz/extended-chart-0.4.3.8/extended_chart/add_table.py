...
def move_pnl_table(table, footer_index, page_number, total_page_count, list_of_pnls):
    ...
    for x in _table.itertuples():
        row = table.new_row(x.Index, x.entry_time, x.exit_time, x.direction, x.entry_price, x.exit_price, x.quantity, x.entry_type,
                            x.pnl_with_commission, x.pnl_tick, x.mfe, x.mae, x.time_to_live)
...
def add_pnl_table(chart, data, height, width=1, position='bottom', color_style=color_style,
    ...
    columns = ['entry_time', 'exit_time', 'direction', 'entry_price', 'exit_price', 'quantity', 'entry_type', 'pnl_with_commission',
               'pnl_tick', 'mfe', 'mae', 'time_to_live']

    alignments = (
        'center', 'center', 'center', 'center', 'right', 'right', 'center','center' ,'right', 'right', 'right', 'right', 'right')
    heading_color = (color_style.get('background_style').get('header_color') for _ in range(len(alignments)))
