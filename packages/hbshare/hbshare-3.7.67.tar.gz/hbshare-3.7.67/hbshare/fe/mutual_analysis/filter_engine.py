...
def factor_test(asofdate1,asofdate2,theme_name,pool_size,save_to_db=False):
    ...
    for i in range(len(asofdate2)):
        ...
        if(theme_name=='HB医药'):
            ...
            initial_pool['final_score'] = \
                (
                 initial_pool['卖出 PCF(出持仓前)_rank']+initial_pool['买入 净利率(出重仓前)_rank']-initial_pool['equity_selection']+initial_pool['买入 PCF(出重仓前)_rank']
                 )
        elif (theme_name == '制造'):
            initial_pool['final_score'] = \
                (
                 initial_pool['ir_theme']-initial_pool['卖出 毛利率(出持仓前)_rank']
                 )
        elif (theme_name == '消费'):
            initial_pool['final_score'] = \
                (
                 initial_pool['ir_theme']+initial_pool['买入 净利润增速(出持仓前)_rank']+initial_pool['买入 PEG(出重仓前)_rank']
                 )
        elif (theme_name == '医药'):
            initial_pool['final_score'] = \
                (
                 initial_pool['二级行业集中度']+initial_pool['卖出 PEG(出重仓前)_rank']
                 )
