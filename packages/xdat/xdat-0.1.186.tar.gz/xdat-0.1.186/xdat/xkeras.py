...
class ImageDataGenerator(Sequence):
    ...
    def __init__(self, dataframe, batch_size, target_size_hw=None, target_size_wh=None, shuffle=True, image_path_col='image_path', target_col='target', weight_col=None, process_image=None, file_cache=None, rgb=True, scale_colors=True, mode=None, n_jobs=-1, parallel_one_epoch_only=True):
        """
        :param dataframe:
        :param batch_size:
        ...
        print(f'- INFO: generator mode={mode}, shuffle={self.shuffle}, n_jobs={self.n_jobs}, file_cache={"on" if self.file_cache else "off"}')
    ...
    def __getitem__(self, idx):
        ...
        images = xparallel.x_on_iter(batch, lambda row: self.load_and_preprocess_image(row[self.image_path_col]), with_tqdm=False, n_jobs=self.n_jobs)
        ...
        if self.weight_col:
            weights = np.array([row[self.weight_col] for row in batch])
            res.append(weights)
