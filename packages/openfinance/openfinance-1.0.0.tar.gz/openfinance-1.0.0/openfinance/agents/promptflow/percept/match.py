...
class MatchFlow(BaseFlow):
    ...
    async def acall(
        ...
        **kwargs: Any        
        ...
        #result = []
        try:
            for data in content["consequence"]:
                ...
                index = self.index_manager.get(data['level'])
                if index:
                    entity = data["entity"]
                    docs = self.index_manager.search(
                        data['level'],
                    ...
                    data["entity"] = entity + "|"+ docs[0]

                resp = await self.llm.acall(
                    self.prompt.prepare({"content" : indicator_name, "indicators": indicators[:-1]}))
                data["indicator"] = indicator_name + "|" + resp.content

                print(data)
            return {self.output: content["consequence"]}
        except:
            return {self.output: ""}