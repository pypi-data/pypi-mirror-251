...
@pytest.mark.parametrize(
    ("operation_state", "operation_error"),
    [
        (Operation.Status.DONE, None),
        (Operation.Status.RUNNING, None),
        (Operation.Status.DONE, "Failed to delete resources"),
    ],
)
def test_delete_tls_certificates(
    operation_state: Operation.Status, operation_error: str
):
    mock_factory = Mock()
    ...
    # Mock operation_client.wait
    mock_operation_client.wait = Mock(
        return_value=Mock(status=operation_state, error=operation_error)
    )
    ...
    # If operation calls succeed and there are no operation errors
    # delete_tls_certificates should return without exceptions
    if operation_state == Operation.Status.DONE and operation_error is None:
        delete_tls_certificates(mock_factory, project_id, cloud_id)

    else:
        # If operation calls either do not succeed or operation returns error response
        # Raise ClickException
        with pytest.raises(ClickException) as e:
            delete_tls_certificates(mock_factory, project_id, cloud_id)

        if operation_state != Operation.Status.DONE:
            e.match("Timeout when trying to remove resource.")
        elif operation_error:
            e.match(f"Failed to remove url map. {operation_error}")
        else:
            raise AssertionError("This state should not occur")
