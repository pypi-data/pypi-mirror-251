...
def delete_tls_certificates(
    ...
    try:
        ...
        wait_on_operation(operations, operation_client, project_id)

    except (PermissionDenied, Forbidden, BadRequest, NotFound) as e:
        raise ClickException(f"Failed to delete tls certificate. {e}")


def wait_on_operation(operations, operation_client, project_id):
    for operation in operations:
        response = operation_client.wait(project=project_id, operation=operation.name)
        if response.status != Operation.Status.DONE:
            # timeout
            raise ClickException("Timeout when trying to remove resource.")
        if response.error:
            raise ClickException(f"Failed to remove url map. {response.error}")
