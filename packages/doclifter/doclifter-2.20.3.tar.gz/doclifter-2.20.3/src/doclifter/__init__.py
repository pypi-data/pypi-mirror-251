...
# pylint: disable=anomalous-backslash-in-string,fixme,line-too-long,too-many-lines,invalid-name,missing-function-docstring,missing-class-docstring,no-else-return,no-else-continue,no-else-break,no-else-raise,multiple-statements,consider-using-enumerate,attribute-defined-outside-init,too-many-branches,too-many-statements,too-many-locals,too-many-nested-blocks,too-many-return-statements,too-many-instance-attributes,no-self-use,too-many-public-methods,too-few-public-methods,consider-using-f-string,consider-using-with,consider-using-dict-items

# pylint: disable=multiple-imports
import sys, os, glob, re, string, tempfile, time, pprint, subprocess, io, getopt
...
class SemanticHintsRegistry:
    ...
    def read(self, rinput):
        ...
        if hasattr(rinput, "read"):
            fp = open(rinput, encoding='ascii', errors='surrogateescape')
            data = fp.readlines()
            fp.close()
        else:
            data = rinput.split('\n')
...
class DocLifter:
        ...
        if '\t' in line and self.tabstops:
            ...
            for c in line:
                if c != '\t':
                    ...
                    i = 0
                    for i, stop in enumerate(self.tabstops):
                        if stop > len(expanded):
                            break
                    if self.tabstops[i] > len(expanded):
                        expanded += " " * (self.tabstops[i] - len(expanded))
                    else:
                        expanded += '\t'
            line = expanded
    ...
    def interpretBlock(self, lines, divert=None):
        ...
        try:
            ...
            if not self.quiet:
                self.error("exception on %s" % (line,))
            raise
...
class TroffInterpreter:
    ...
    def interpret(self, line, tokens, _):
        ...
        if command in {"br", "nl"}:
            ...
            if path:
                try:
                    text = self.preprocess(open(path, encoding='ascii', errors='surrogateescape').read())
        ...
        # String and macro interpretation
            ...
            if command==TroffInterpreter.ctrl+"de" or not name in self.macros:
                self.macros[name] = []
            isused = [x for x in self.source.lines if isinstance(x, str) and x[0:len(name)+1]==TroffInterpreter.ctrl+name]
            suppressed = False
            for interpreter in self.source.interpreters:
    ...
    def preprocess(self, text):
        ...
        for line in text.split("\n"):
            ...
            if line and line[0]=="'" and not isComment(line) and len(line)>3:
                if not line[1].isalpha() or not line[2].isalpha():
                    line = r"\&" + line
...
def gatherItem(source, tag=None):
    ...
    if [x for x in outlines if x and x[:4] != "<!--"]:
...
def isFileOrCommandName(tok):
    ...
    if tok in ('link', 'with'):
        ...
        return tok[0].islower() or tok[0] == "/" or (tok[0] in string.digits and tok[-1].isalpha())
...
class ManInterpreter:
    ...
    def interpret(self, _dummyLine, tokens, _dummyCaller):
        cmd = tokens[0][1:]
        args = tokens[1:]
        # Highlighting
...
class DocBookInterpreter:
    ...
    def __init__(self, source, verbose=0):
        self.source = source
        self.verbose = verbose
    def interpret(self, _dummy, tokens, _dummyCaller):
        cmd = tokens[0][1:]
...
class FoojzsInterpreter:
    ...
    def __init__(self, source, verbose=0):
        self.source = source
        self.verbose = verbose
    def interpret(self, _dummy, _dummyTokens, _dummyCaller):
        return False
    def preprocess(self, text):
...
class ASTInterpreter:
    ...
    def interpret(self, _dummy, tokens, _dummyCaller):
        ...
        if cmd in ("H0", "H1", "H2", "H3", "H4"):
            ...
            self.headerset = set([])
            self.source.interpreters[1].interpret(_dummy, tokens, _dummyCaller)
        else:
            return False
        return True
    def preprocess(self, text):
...
class MdocInterpreter:
    ...
    def interpret(self, _dummy, tokens, _dummyCaller):
        ...
        # but usage on the groff_mdoc(7) list shows they should be.
            ...
            if self.hasargs("Ft", args):
                if self.source.bodySection():
                    self.source.pushline("".join(self.encloseargs(args,
                                     "<type remap='Ft'>", "</type>")))
                else:
    ...
    def stylehook(self, args, tag, attr, _dummyPrefix):
...
class MsInterpreter:
    ...
    def __init__(self, source, verbose=0):
        ...
        self.AB = None
        self.flushed = False
    def interpret(self, _dummy, tokens, caller):
        command = tokens[0][1:]
        args = tokens[1:]
...
class MmInterpreter:
    ...
    def interpret(self, _dummy, tokens, caller):
        ...
        if cmd == "AE":
            ...
            self.source.endBlock("literallayout", remap="DE")
            if self.FG:
                self.source.emit("<caption><phrase>%s</phrase></caption>" % self.FG)
    ...
    if not arguments:
        ...
        for ifile in arguments:
            ...
            if outdoc is None:
                ...
                elif isinstance(transFilename, str):
...
def main(args, _dummyMainout=stdout, mainerr=stderr):
    ...
    try:
        ...
        if hintfile:
            fp = open(hintfile, "w", encoding='ascii', errors='surrogateescape')
