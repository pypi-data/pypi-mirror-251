...
class IBClient:    
    ...
    def get_completed_orders(self):
        with completed_orders() as app:
            app.connect(ib_settings.HostIP, self.port, self.clientID)                
            t = Timer(ib_settings.SLEEP_TIME*10, app.stop)
            t. start()            
            app.run()
            t.cancel()            
...
if __name__ == '__main__':
    ...
    summary_dict = get_ib_account_summary(TWS=True, Live=False)

    exit(0)

    value_dic, protolio_df = get_ib_accounts_values_protfolio(TWS=True, live=False)
