...
def data_write_save(simname, excel_file,start,plot_start,d4_df,d4_sum,result_df,feeder):
    ...
    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
        ...
        info_row = {'S_inst':'Refer Appendix E in 157897-NRD-REP-EDS-000003 and 000004, MML Braybrooke Assessment',
                    ...
                    'tn_step': 'tn = 18.4*(100*d*F) with F=1',
...
def criteria_calculate(pri_v,max_op_range,min_op_range,load_1_min,load_10_min,load_30_min,time_increment):
    ...
    df.at[12, 'Assessment Criteria'] = criteria_values[time_increment - 1]
...
def result_rvc_update(simname, result_df, pri_v, fault, event_f,event_if,event_vif):
    ...
    if isinstance(event_f, (int, float)) and not np.isnan(event_f):
        ...
        result_df.at[9, 'Simulation Result'] = '-'
    ...
    if isinstance(event_if, (int, float)) and not np.isnan(event_if):
        ...
        result_df.at[10, 'Simulation Result'] = '-'
    ...
    if isinstance(event_vif, (int, float)) and not np.isnan(event_vif):
        ...
        result_df.at[11, 'Simulation Result'] = temp
    else:
        result_df.at[11, 'Simulation Result'] = '-'
...
# 8                       PCC Step Voltage Change (SVC)     %                      0.03                NaN
# 9     PCC Rapid Voltage Change (RVC) - Frequent Event     %                      0.06                NaN
# 10  PCC Rapid Voltage Change (RVC) - Infrequent Event     %                       0.1                NaN
# 11  PCC Rapid Voltage Change (RVC) - Very Infreque...     %                      0.12                NaN
# 12  Flicker Severity - Short Term - RVC with Pst <...     %                     0.006                NaN
# 13  Flicker Severity - Short Term - Memory Time Te...     s                       600                NaN
# 14                 Flicker Severity - Long Term - Plt   N/A  Pass if Stage 2 Pst pass                NaN
    ...
    target.loc[8,'Simulation Result'] = df_sum.at[0,'abs_SVC'] # SVC
    target.loc[12,'Simulation Result'] = df_sum.at[0,'abs_SVC'] # Flicker Short Term stage 2
    target.loc[13,'Simulation Result'] = df_sum.at[0,'t_memory'] # Flicker memory time
...
def get_result_range(feeder,d4_df):
    ...
    # 0
    result_range = [f"B11:E26"]
    range_list.append(result_range)
    # 1
    data_range = [f"C12:E26"]
    ...
    #3 4 5
    add_range_1 = [f"D14:D19",f"E12:E19",f"E25:E25"]
    range_list.append(add_range_1)

    add_range_2 = [f"D20:E24"]
