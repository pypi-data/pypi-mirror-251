...
def run_tool_pre_container_conf(base_dir, build_dir):

    pre_run_file = 'tools/pre-img-run.sh'
    pre_run_path = os.path.join(build_dir, pre_run_file)

    if os.path.isfile(pre_run_path):

        log.info(f"Executing {pre_run_file}")
        ...
        log.block(result.stdout.decode())

        if result.returncode != 0:
            log.warn(result.stderr.decode())
            raise log.FatalMsg()
