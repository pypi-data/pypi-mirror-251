...
class MetricsListResponseTypeWrapper(BaseModel):
    ...
    """
    results: MetricsListResponseType = Field(...)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MetricsListResponseTypeWrapper:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MetricsListResponseTypeWrapper:
        """Create an instance of MetricsListResponseTypeWrapper from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MetricsListResponseTypeWrapper.parse_obj(obj)

        _obj = MetricsListResponseTypeWrapper.parse_obj({
            "results": MetricsListResponseType.from_dict(obj.get("results")) if obj.get("results") is not None else None
        })
        return _obj
