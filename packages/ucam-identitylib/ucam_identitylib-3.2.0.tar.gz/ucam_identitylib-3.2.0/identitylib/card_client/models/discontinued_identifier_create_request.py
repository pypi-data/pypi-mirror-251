...
class DiscontinuedIdentifierCreateRequest(BaseModel):
    ...
    """
    notes: Optional[StrictStr] = Field(None, description="Notes about the discontinued identifier")
    permitted_identifier: Optional[StrictStr] = Field(None, alias="permittedIdentifier", description="The identifier permitted to use the discontinued identifier in {value}@{scheme} format (if not specified, the discontinued identifier cannot be used by other identifiers)")
    ...
    value: constr(strict=True, min_length=1) = Field(..., description="The discontinued identifier's value")
    __properties = ["notes", "permittedIdentifier", "scheme", "value"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DiscontinuedIdentifierCreateRequest:
    ...
    def to_dict(self):
        ...
        if self.permitted_identifier is None and "permitted_identifier" in self.__fields_set__:
            _dict['permittedIdentifier'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> DiscontinuedIdentifierCreateRequest:
        """Create an instance of DiscontinuedIdentifierCreateRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DiscontinuedIdentifierCreateRequest.parse_obj(obj)

        _obj = DiscontinuedIdentifierCreateRequest.parse_obj({
            "notes": obj.get("notes"),
            "permitted_identifier": obj.get("permittedIdentifier"),
