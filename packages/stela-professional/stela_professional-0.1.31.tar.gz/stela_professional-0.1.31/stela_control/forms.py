    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            ...
            field.widget.attrs.update(
                {'class': 'form-control'})
            field.required = True
            field['subject'].validators = [text_regex]
            field['messages'].validators = [text_regex]
...
class InvoiceForm(forms.ModelForm):
    ...
    def clean_pdf_file(self):
        ...
        image_format = imghdr.what(pdf)
        if image_format not in ['pdf', 'png']:
            raise forms.ValidationError(_("The pdf format is not valid. Only JPEG and PNG files are allowed."))
        return pdf
