...
from .forms import (
    ...
    TaxReturnForm, SupportForm, ReadOnlySupportForm, ReadOnlySupportFormCostumer
)
...
form_mapping = {
    ...
    'ReadOnlySupportForm': ReadOnlySupportForm,
    'ReadOnlySupportFormCostumer': ReadOnlySupportFormCostumer,
    #Inventory
    'Inventory': Inventory,
    'Elements': Elements,
    'Variant': Variant,
...
def renderData(request):
    if request.method=='POST':
        ...
        if action == "tax_return":
            files=TaxReturn.objects.filter(owner_id=user_id)
...
def dynamicForms(request):
     if request.method=='POST':
        ...
        model_child = request.POST.get('model_child')
        schedule = request.POST.get('schedule')
        print(form_id, form_name, formset_name, model_parent, model_child, action, schedule, data) 
