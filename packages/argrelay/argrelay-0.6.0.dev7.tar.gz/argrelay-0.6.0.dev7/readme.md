...
A tool to provide generic object selector (structured data search filter) for command line interface (CLI):
...
*   Fuzzy-search (yet easily predictable) is achieved by
    ...
    *   allowing unordered args (using priorities to resolve arg type in cases `enum` sets intersect)
