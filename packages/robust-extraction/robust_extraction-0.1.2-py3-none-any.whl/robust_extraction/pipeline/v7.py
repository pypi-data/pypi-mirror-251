...
def pipeline7(
    ...
    verbose = False, auto_h = 1080, descale_h = 1080
) -> Error | Result:
    """Like `v6` but descaling and rescaling"""
    big_corr = pve.descaled_autocorrect(img, model, pve.Params(rescale_h=auto_h))
    corr_img = vc.descale_h(big_corr, descale_h)
    ...
    if len(inlier_rows) < min_rows:
        return NotEnoughRows(detected=len(inlier_rows), required=min_rows)
    ...
    if len(inlier_cols) < min_cols:
        return NotEnoughCols(detected=len(inlier_cols), required=min_cols)
    ...
    return Result(corr_img=big_corr, boxes=boxes, contours=contours)
