...
from .. import templates as ts, contours as cs, lines as ls
...
def correct(
    ...
    tl, tr, br, bl = corners
    pl, pr, pt, pb = pads
    detected_w = int(max(abs(tl[0]-tr[0]), abs(bl[0]-br[0])))
    detected_h = int(max(abs(tl[1]-bl[1]), abs(tr[1]-br[1])))
...
def descaled_autocorrect(
    img: cv.Mat, model: ts.SheetModel, params: Params = Params()
) -> cv.Mat | None:
    ...
    rescaled = vc.descale_h(img, params['rescale_h'])
