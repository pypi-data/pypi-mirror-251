...
class MessageLocal(Item, GenericCRUD):
    ...
    def __init__(self, *, campaign_id: int, to_recipients: List[Recipient], api_type_id: int,
        ...
        self.__ordered_message_channels = [message_channel for message_channel in MessageChannels]
        self.__compound_message = CompoundMessage(message_id=message_id).get_compound_message_after_text_template(
            campaign_id=campaign_id, body=original_body, subject=original_subject, to_recipients=to_recipients)
    ...
    def get_msg_type(self, recipient: Recipient) -> MessageChannels:
    ...
    def get_message_channel(self, recipient: Recipient) -> MessageChannels:
    ...
    @staticmethod
    def get_message_provider_id(message_channel_id: MessageChannels, recipient: Recipient) -> int:
        """return message provider"""
        logger.start()
        if message_channel_id == MessageChannels.SMS and recipient.get_canonical_telephone().startswith("972"):
            provider_id = AWS_SMS_MESSAGE_PROVIDER_ID
        elif message_channel_id == MessageChannels.EMAIL:
            provider_id = AWS_EMAIL
        elif message_channel_id == MessageChannels.WHATSAPP:
            # TODO: or vonage
    ...
    def get_body_after_text_template(self, recipient: Recipient = None,
                                     message_channel: MessageChannels = None) -> str:
