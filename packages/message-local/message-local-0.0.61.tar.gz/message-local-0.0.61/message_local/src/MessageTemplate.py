...
class MessageTemplate(GenericCRUDML):
    def __init__(self):
    ...
    def is_critiria_id_match_profile_id(self, criteria_json: dict, profile_id: int) -> dict:
        where = self.get_where_by_criteria(criteria_json)
        if profile_id:  # else raise?
...
SELECT DISTINCT user_id, person_id, user_main_email, user.profile_id AS profile_id, 
 ...
 FROM user.user_general_view AS user
  ...
  WHERE {where}"""
    ...
    def get_where_by_criteria(self, criteria_json: dict):
    ...
    def get_critiria_json(self, message_template_id: int) -> dict:
        ...
        return self.convert_to_dict(self.cursor.fetchone(), columns)
...
FROM message_template.message_template_text_block_view AS message_template_text_block
    ...
    JOIN message_template.message_template_message_template_text_block_table AS message_template_message_template_text_block
    ...
    JOIN message_template.message_template_text_block_type_ml_table AS message_template_text_block_type_ml
        ON message_template_text_block_type_ml.message_template_text_block_type_id = message_template_message_template_text_block.message_template_text_block_type_id
...
"""  # noqa
    ...
    def _get_possible_answers(self, question_id: int) -> list[dict]:
        ...
        query = "SELECT value FROM question.question_possible_answers_ml_table WHERE question_id = %s"
