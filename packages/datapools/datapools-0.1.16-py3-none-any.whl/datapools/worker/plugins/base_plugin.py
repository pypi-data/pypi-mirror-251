...
from PIL.ExifTags import TAGS, Base as ExifTagsList
import io
...
class BasePlugin(metaclass=ABCMeta):
    ...
    def parse_image_tag(cls,content) -> Union[BaseTag,None]:
        #load image from bytes content, parse ImageDescription exif field for a license tag
        try:
            image = Image.open(io.BytesIO(content))
            exifdata = image.getexif()
            desc = exifdata.get(ExifTagsList.ImageDescription)
            logger.info( f'{desc=} {type(desc)=}')
            
            if type(desc) is str:
                return cls.parse_tag_in_str( desc )
        except Exception as e:
