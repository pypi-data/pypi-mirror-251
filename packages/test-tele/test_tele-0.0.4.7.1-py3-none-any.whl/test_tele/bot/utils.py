...
def premium_protect(org_func):
    """Decorate to restrict non subscriber from accessing feature"""

    async def wrapper_func(event):
        """Wrap the original function."""
        users = query.read_datas('users', ['chat_id', 'is_subscriber', 'is_block'])
        for user in users:
            if event.sender_id == user[0] and user[1] and not user[2]:
                return await org_func(event)
        else:
            raise events.StopPropagation
