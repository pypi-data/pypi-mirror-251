...
async def move(s3_client: S3Client, source_bucket_id: str, source_key: str, target_bucket_id: str, target_key: str,
    ...
    :raises ValueError: if preflight fails.
    ...
    async def gen():
        ...
        async for obj in awsservicelib.list_objects(s3_client, source_bucket_id, source_key, max_keys=1000, loop=loop):
            if obj['StorageClass'] in (S3StorageClass.DEEP_ARCHIVE.name, S3StorageClass.GLACIER.name):
                raise ValueError(f'{awsservicelib._activity_object_display_name(source_bucket_id, source_key)} contains archived objects')
