...
def show_face(frame, resized_faces, start_person, direction):
    ...
    for index, faces in enumerate(resized_faces[1][0]):
        if faces is not None:
            ...
            cv2.rectangle(frame, (round(face_x1), round(face_y1)), (round(face_x2), round(face_y2)), color, 2)

            cv2.putText(frame, text, (round(face_x1), round(face_y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
