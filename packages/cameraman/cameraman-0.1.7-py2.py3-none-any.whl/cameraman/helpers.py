...
class ConfigHelper:
    ...
    def expand(self, addresses):
        return [addresses]

    def settings(self, host):
        INVENTORY = self.cfg.inventory
    ...
    def url(self, settings, cmd):
        url = settings.get("url", "https://{host}{cmd}")

        return url

    def command(self, settings, cmd):
        return self.api(settings).get(cmd)
