    ...
    if auto_reload:
        ...
        class AmsdalInitConsumer(LifecycleConsumer):
            def on_event(self, *args: Any, **kwargs: Any) -> None:  # noqa: ARG002
                ...
                migration_manager._refresh_state_db(
                    migration_manager._class_manager.import_class(
                        BaseClasses.OBJECT.value,
                        SchemaTypes.TYPE,
                    )
                )

                migration_manager._refresh_state_db(
                    migration_manager._class_manager.import_model_class(
                        BaseClasses.CLASS_OBJECT.value,
                        SchemaTypes.CORE,
                    )
                )
