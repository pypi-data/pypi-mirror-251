...
from .utils import process_insert_data_json  # noqa E402
from .utils import (process_update_data_json, validate_none_select_table_name,
                    validate_select_table_name)

# from sql_to_code.sqltocode import SQL2Code
...
# TODO: add ` around schema, table and column names if not already there, because they might be reserved words
# TODO: verify that schema, table and column names does not contain ; to prevent SQL injection
class GenericCRUD:
    ...
    def __init__(self, default_schema_name: str,
        ...
        '''
        sql_to_code = SQL2Code(default_schema_name='database')
        table_data_dict = sql_to_code.read_table(table='table_definition_table')
        self.table_definition_table_json_string = json.dumps(table_data_dict)
        '''
        logger.end()

    # TODO: add schema optional parameter to all functions, and call set_schema on them.
    ...
    def get_test_entity_id(self, entity_name: str, entity_creator: callable,
        ...
        (assuming entity_creator expects `number` and `is_test_data` as parameters,
            and returns the expected arguments for insert_function)
        ...
        if fetched_result:
            ...
            # TODO Shall we move NumberGenerator into entity_creator() functions
            entity_number = NumberGenerator().get_random_number(schema_name=schema_name, view_name=view_name)
            entity_result = entity_creator(number=entity_number, is_test_data=True)
            test_entity_id = insert_function(entity_result)
