...
print('Software de visualizacao de arquivos de STS Nanosurf v1.1.0. Arquivos tipo csv (x,y,z) ')
    ...
    def __init__(self,path_file,n,smooth = 5,delta = 10):
        ...
        def select_sts(path_file, n = [],smooth = 5,delta = 10):
            ...
            try:
                ...
                folder_name = path.join(folder_save,path_file[:-4])
            try: 
                mkdir(folder_save )
            ...
            for i in range(len(file[0])):
                if (i in n) == False:
                        try:
                            name = path.join(paste,file[1][i]+'_'+path_file[:-4][ct:])
                        except UnboundLocalError:
                            name = path.join(paste,file[1][i]+'_'+path_file[:-4])
