...
class GroupedOption(click.Option):
    ...
    async def handle_parse_result(self, ctx: click.Context, opts: Mapping[str, Any],
                            args: List[str]) -> Tuple[Any, List[str]]:
        with augment_usage_errors(ctx, param=self):
            if not ctx.resilient_parsing:
                await self.group.handle_parse_result(self, ctx, opts)
...
class OptionGroup:
    ...
    async def handle_parse_result(self, option: GroupedOption, ctx: click.Context, opts: Mapping[str, Any]) -> None:
...
class RequiredMutuallyExclusiveOptionGroup(MutuallyExclusiveOptionGroup):
    ...
    async def handle_parse_result(self, option: GroupedOption, ctx: click.Context, opts: Mapping[str, Any]) -> None:
        await super().handle_parse_result(option, ctx, opts)
