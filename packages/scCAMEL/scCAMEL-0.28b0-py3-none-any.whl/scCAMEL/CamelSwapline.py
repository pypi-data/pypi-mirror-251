...
def writedata(adatax,filename,filepath=""):
    ...
    """
    if type(adatax.X) != sparse.csr.csr_matrix:
        adatax.X= sparse.csr_matrix(adatax.X)
    if adatax.obsm['ConstrualValue_DeepLift'].size != 0:
        if type(adatax.obsm['ConstrualValue_DeepLift']) != sparse.csr.csr_matrix:
            adatax.obsm['ConstrualValue_DeepLift']= sparse.csr_matrix(adatax.obsm['ConstrualValue_DeepLift'])
    if adatax.obsm['ConstrualValue_DeepLift'].size != 0:
        if type(adatax.obsm['NormalizedMatrix']) != sparse.csr.csr_matrix:
