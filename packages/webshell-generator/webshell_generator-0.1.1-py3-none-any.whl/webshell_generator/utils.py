import os
from .webshell import BehinderWebshell, GodzillaWebshell, DefineClassWebshell


BEHINDER_DEFAULT_PASS = "rebeyond"

GODZILLA_DEFAULT_PASS = "pass"
GODZILLA_DEFAULT_KEY = "key"


def get_behinder4_jsp_shell(_pass=BEHINDER_DEFAULT_PASS):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "behinder4", "default_aes", "shell.jsp")
    return BehinderWebshell(webshell_path, "jsp", "behinder4", "default_aes", _pass)


def get_behinder4_php_shell(_pass=BEHINDER_DEFAULT_PASS):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "behinder4", "default_aes", "shell.php")
    return BehinderWebshell(webshell_path, "php", "behinder4", "default_aes", _pass)


def get_behinder4_aspx_shell(_pass=BEHINDER_DEFAULT_PASS):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "behinder4", "default_aes", "shell.aspx")
    return BehinderWebshell(webshell_path, "aspx", "behinder4", "default_aes", _pass)


def get_godzilla_jsp_shell(_pass=GODZILLA_DEFAULT_PASS, _key=GODZILLA_DEFAULT_KEY):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "godzilla", "java_aes_base64.jsp")
    return GodzillaWebshell(webshell_path, "jsp", "godzilla", "java_aes_base64", _pass, _key)


def get_godzilla_jspx_shell(_pass=GODZILLA_DEFAULT_PASS, _key=GODZILLA_DEFAULT_KEY):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "godzilla", "java_aes_base64.jspx")
    return GodzillaWebshell(webshell_path, "jspx", "godzilla", "java_aes_base64", _pass, _key)


def get_godzilla_php_shell(_pass=GODZILLA_DEFAULT_PASS, _key=GODZILLA_DEFAULT_KEY):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "godzilla", "php_xor_base64.php")
    return GodzillaWebshell(webshell_path, "php", "godzilla", "php_xor_base64", _pass, _key)


def get_godzilla_aspx_shell(_pass=GODZILLA_DEFAULT_PASS, _key=GODZILLA_DEFAULT_KEY):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "godzilla", "cshap_aes_base64.aspx")
    return GodzillaWebshell(webshell_path, "aspx", "godzilla", "cshap_aes_base64", _pass, _key)


def get_godzilla_ashx_shell(_pass=GODZILLA_DEFAULT_PASS, _key=GODZILLA_DEFAULT_KEY):
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "godzilla", "cshap_aes_base64.ashx")
    return GodzillaWebshell(webshell_path, "ashx", "godzilla", "cshap_aes_base64", _pass, _key)


def get_define_class_shell():
    # webshell_generator 长度 1515
    webshell_path = os.path.join(os.path.dirname(__file__), "webshells", "other", "define_class_memshell.jsp")
    define_class_webshell = DefineClassWebshell(webshell_path, "jsp", "godzilla", "java_aes_base64", "PaSs", "key")
    # Godzilla PaSs / key
    define_class_webshell.shell_class = """"""
    define_class_webshell.headers = "clz: Godzilla"
    return define_class_webshell

