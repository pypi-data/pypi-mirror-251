...
class SnowparkClient(BaseClient):
    ...
    def write_df_to_table(self, df, table: str, append_if_exists: bool = False) -> None:
        table_name = table.upper()
        df.columns = df.columns.str.upper()

        self.snowpark_session.write_pandas(
            df, table_name=table_name, auto_create_table=True, overwrite=False if append_if_exists else True)
