...
logger = LoggerLocal.create_logger(object=variable_local_logger_init_object)
...
class VariablesLocal(GenericCRUD):
    ...
    def set_variable_value_by_variable_id(self, variable_id: int, variable_value: str) -> None:
        logger.start(object={'variable_id': variable_id,
                             'variable_value': variable_value})
        # TODO I believe we should keep more fields
        self.set_schema(schema_name='logger')
        data_json = {'variable_id': variable_id, 'variable_value_new': variable_value}
    ...
    def load_variable_names_dict_from_variable_table(self, person_id: int = None) -> dict:
        ...
        rows = self.select_multi_dict_by_where(view_table_name="variable_view",
                                               where="person_id = %s" if person_id is not None else None,
                                               params=(person_id,) if person_id is not None else None,
