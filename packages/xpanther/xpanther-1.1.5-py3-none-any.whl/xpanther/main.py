...
class XPanther:
        ...
        if type(target) == str and re.match(element_regex, target):
            ...
            if occurrences:
                ...
                for index_of_element in indexes_of_element:
                    ...
                    self.__target = self.clean_up_elements(self.__keys[row_index])
    ...
    @staticmethod
    def clean_up_elements(outerHTML_input):
        for element in outerHTML_input[0][1:]:
            if '="' not in element:
                outerHTML_input[0].remove(element)
        return outerHTML_input
