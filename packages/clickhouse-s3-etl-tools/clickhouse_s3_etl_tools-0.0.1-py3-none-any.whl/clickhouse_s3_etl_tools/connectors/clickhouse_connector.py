...
class ClickHouseConnector:
    ...
    def fetch_rows_by_cumulative_sum(
        ...
        query = f"""
            SELECT
                toString(partition) as partition,
        ...
        while start_index < len(cumulative_sums):
            ...
            rows.append(
                (result[start_index][0], result[end_index][0], selected_group_sum)
            )
    ...
    def drop_table(self, database: str, table_name: str, on_cluster: str = "") -> bool:
        ...
        """
        drop_table_query = f"""DROP TABLE IF EXISTS {database}.{table_name} {on_cluster}"""
