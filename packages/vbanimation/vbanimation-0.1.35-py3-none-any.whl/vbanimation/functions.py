...
def process_tex_to_text(tex_file, key):
    tex_audio = tex_file.replace(r'\intertext{', r'').replace(r'$', r'').replace(r'-', r' minus ') 
    tex_audio = tex_audio.replace(r'\text{', r'').replace(r'\left', r'').replace(r'\right', r'')
    tex_audio = tex_audio.replace(r'\cdot', r' times ').replace(r'\times', r' times ').replace(r'\div', r' divided by ')
    tex_audio = tex_audio.replace(r'}{', r' upon ')
    tex_audio = tex_audio.replace(r'(', r'').replace(r')', r'').replace(r'[', r'').replace(r']', r'')
    tex_audio = tex_audio.replace(r'\sqrt', r' square root of ')
    tex_audio = tex_audio.replace(r'^2', r' squared ').replace(r'^3', r' cubed ').replace(r'^4', r' to the power 4 ')
    tex_audio = tex_audio.replace(r'\theta', r' theta ').replace(r'\alpha', r' alpha ').replace(r'\beta', r' beta ')
    tex_audio = tex_audio.replace(r'\frac', r'').replace(r'{', r'').replace(r'}', r'').replace(r'\sqrt', r'').replace(r'\pi', r' pi ')
    tex_audio = tex_audio.replace(r'\sin', r' sine ').replace(r'\cos', r' cosine ').replace(r'\tan', r' tangent ')
    tex_audio = tex_audio.replace(r'\cot', r' cotangent ').replace(r'\sec', r' secant ').replace(r'\csc', r' cosecant ')
    tex_audio = tex_audio.replace(r'\log', r' log ').replace(r'\ln', r' natural log ').replace(r'\lim', r' limit ')
    tex_audio = tex_audio.replace(r'\sum', r' sum ').replace(r'\prod', r' product ').replace(r'\int', r' integral ')
    tex_audio = tex_audio.replace(r'\infty', r' infinity ').replace(r'\to', r' to ').replace(r'\in', r' in ')
    tex_audio = tex_audio.replace(r'\neq', r' not equal to ').replace(r'\leq', r' less than or equal to ').replace(r'\geq', r' greater than or equal to ')
    tex_audio = tex_audio.replace(r'\left(', r'').replace(r'\right)', r'').replace(r'\left[', r'').replace(r'\right]', r'')
    tex_audio = tex_audio.replace(r'\left\{', r'').replace(r'\right\}', r'').replace(r'\left|', r'').replace(r'\right|', r'')
    tex_audio = tex_audio.replace(r'\left\langle', r'').replace(r'\right\rangle', r'').replace(r'\left\lfloor', r'').replace(r'\right\rfloor', r'')
    tex_audio = tex_audio.replace(r'\left\lceil', r'').replace(r'\right\rceil', r'').replace(r'\left\|', r'').replace(r'\right\|', r'')
    tex_audio = tex_audio.replace(r'\left\lfloor', r'').replace(r'\right\rfloor', r'').replace(r'\left\lceil', r'').replace(r'\right\rceil', r'')
    tex_audio = tex_audio.replace(r'\left\langle', r'').replace(r'\right\rangle', r'').replace(r'\left\lfloor', r'').replace(r'\right\rfloor', r'')
   
    audio_command = f'say -o {key}.aac {tex_audio}'
    ...
    def construct(self):
        ...
        for key, value in equations.items():
            ...
            if len(value) == 1:
                ...
                process_tex_to_text(value[0], key)
