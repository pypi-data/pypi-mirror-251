...
from ..utils.util import unzip_data, is_directory_empty, get_subdirectories
...
def prepare_dataset_for_mount():
    ...
    if dataset_need_unzip == constants.DATASET_NEED_UNZIP_TRUE:
        ...
        if mount_success:
            subdirectories_list = get_subdirectories(local_dataset_path)
...
def prepare_pretrain_model_for_mount():
    ...
    if pretrain_model_need_unzip == constants.PRETRAIN_MODEL_NEED_UNZIP_TRUE:
        ...
        if mount_success:
            subdirectories_list = get_subdirectories(local_pretrain_model_path)
            for subdirectory in subdirectories_list:
