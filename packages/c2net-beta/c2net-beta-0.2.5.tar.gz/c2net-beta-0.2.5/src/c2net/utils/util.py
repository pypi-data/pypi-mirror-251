...
def unzip_data(zipfile_path, unzipfile_path):
    try:
        if zipfile_path.endswith(".tar.gz"):
            ...
            tar_file_path = os.path.join(unzipfile_path, os.path.splitext(os.path.basename(zipfile_path))[0])
            with tarfile.open(tar_file_path, 'r') as tar:
                tar.extractall(unzipfile_path)
            try:
                os.remove(tar_file_path)
            except Exception as e:
                print(f'Deletion failed for {tar_file_path}: {str(e)},but this does not affect the operation of the program, you can ignore')
            unzip_success = True
