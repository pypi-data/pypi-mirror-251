...
class WebullMarkets(DatabaseManager):
    ...
    async def get_top_gainers(self, rank_type:str='1d', pageSize: str='100', as_dataframe:bool=True):
        ...
        if self.connection_string is not None:
            ...
            await self.connect()
            await self.batch_insert_dataframe(df, table_name=f'top_gainers_{rank_type}', unique_columns='symbol, rank_type, gainer_type')
        return df
    ...
    async def get_top_losers(self, rank_type:str='1d', pageSize: str='100', as_dataframe:bool=True):
        ...
        if self.connection_string is not None:
            ...
            await self.connect()
            await self.batch_insert_dataframe(df, table_name=f'top_losers_{rank_type}', unique_columns='symbol, rank_type, gainer_type')
        return df
    ...
    async def get_forex(self):
        ...
        if self.connection_string is not None:
            if 't_sectype' in df.columns:
                df.drop(['t_sectype'], axis=1, inplace=True)
            await self.connect()
            await self.batch_insert_dataframe(df, table_name='forex', unique_columns='ticker')
        return df
