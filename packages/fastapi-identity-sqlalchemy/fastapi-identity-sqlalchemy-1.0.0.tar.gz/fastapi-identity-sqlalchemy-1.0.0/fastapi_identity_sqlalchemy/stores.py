...
class SQLAlchemyRoleStore(IRoleStore[TRole], Generic[TRole]):
    def __init__(self, session: AsyncSession, role_db: Type[IdentityRole] = IdentityRole):
        self.role_db = role_db
        self.session = session

    def create_model_from_dict(self, **kwargs):
        return self.role_db(**kwargs)
    ...
    async def find_by_id(self, role_id: str) -> Optional[TRole]:
        ...
        statement = select(self.role_db).where(self.role_db.id == role_id)
    ...
    async def find_by_name(self, normalized_name: str) -> Optional[TRole]:
        ...
        statement = select(self.role_db).where(self.role_db.normalized_name == normalized_name)
        return await self._find_role(statement)
...
class SQLAlchemyUserStore(
    ...
    def __init__(
            ...
            user_db: Optional[Type[TUser]] = None,
            role_db: Optional[Type[TRole]] = None,
            user_role_db: Optional[Type[TUserRole]] = None,
            user_claim_db: Optional[Type[TUserClaim]] = None,
            user_login_db: Optional[Type[TUserLogin]] = None,
            user_token_db: Optional[Type[TUserToken]] = None
    ):
        self.user_db = user_db or IdentityUser
        self.role_db = role_db or IdentityRole
        self.user_role_db = user_role_db or IdentityUserRole
        self.user_claim_db = user_claim_db or IdentityUserClaim
        self.user_login_db = user_login_db or IdentityUserLogin
        self.user_token_db = user_token_db or IdentityUserToken
        self.session: AsyncSession = session

    def create_model_from_dict(self, **kwargs) -> TUser:
        return self.user_db(**kwargs)
    ...
    async def find_by_id(self, user_id: str) -> Optional[TUser]:
        ...
        statement = select(self.user_db).where(self.user_db.id == user_id)
    ...
    async def find_by_name(self, normalized_username: str) -> Optional[TUser]:
        ...
        statement = select(self.user_db).where(self.user_db.normalized_username == normalized_username)
        return await self._find_user(statement)
    ...
    async def find_by_email(self, normalized_email: str) -> Optional[TUser]:
        ...
        statement = select(self.user_db).where(self.user_db.normalized_email == normalized_email)
        return await self._find_user(statement)
    ...
    async def add_to_role(self, user: TUser, normalized_role_name: str) -> None:
        ...
        if role := await self._find_role(normalized_role_name):
            statement = insert(self.user_role_db).values(user_id=user.id, role_id=role.id)
    ...
    async def get_roles(self, user: TUser) -> list[str]:
        ...
        statement = select(self.role_db.name).where(
            and_(
                self.user_db.id == user.id,
                self.user_db.id == self.user_role_db.user_id,
                self.role_db.id == self.user_role_db.role_id
            )
        )
    ...
    async def is_in_role(self, user: TUser, normalized_role_name: str) -> bool:
        ...
        if role := await self._find_role(normalized_role_name):
            statement = select(self.user_role_db).where(
                and_(
                    self.user_role_db.user_id == user.id,
                    self.user_role_db.role_id == role.id
                )
            )
    ...
    async def remove_from_role(self, user: TUser, normalized_role_name: str) -> None:
        ...
        if role := await self._find_role(normalized_role_name):
            statement = delete(self.user_role_db).where(
                and_(
                    self.user_role_db.user_id == user.id,
                    self.user_role_db.role_id == role.id
                )
    ...
    async def find_by_login(self, login_provider: str, provider_key: str) -> Optional[TUser]:
        ...
        statement = select(self.user_login_db).where(
            and_(
                self.user_login_db.login_provider == login_provider,
                self.user_login_db.provider_key == provider_key
            )
        )

        if user_login := (await self.session.scalars(statement)).one_or_none():
            statement = select(self.user_db).where(self.user_db.id == user_login.user_id)
            return await self._find_user(statement)
    ...
    async def get_logins(self, user: TUser) -> list[UserLoginInfo]:
        ...
        statement = select(self.user_login_db).where(self.user_login_db.user_id == user.id)
    ...
    async def remove_login(self, user: TUser, login_provider: str, provider_key: str) -> None:
        ...
        statement = delete(self.user_login_db).where(
            and_(
                self.user_login_db.user_id == user.id,
                self.user_login_db.login_provider == login_provider,
                self.user_login_db.provider_key == provider_key
            )
        )
    ...
    async def remove_token(self, user: TUser, login_provider: str, name: str) -> None:
        ...
        statement = delete(self.user_token_db).where(
            and_(
                self.user_token_db.user_id == user.id,
                self.user_token_db.login_provider == login_provider,
                self.user_token_db.name == name
            )
        )
        await self.session.execute(statement)
    ...
    async def get_claims(self, user: TUser) -> list[Claim]:
        ...
        statement = select(self.user_claim_db).where(self.user_claim_db.user_id == user.id)
    ...
    async def get_users_for_claim(self, claim: Claim) -> list[TUser]:
        ...
        statement = select(self.user_claim_db).where(
            and_(
                self.user_claim_db.claim_type == claim.type,
                self.user_claim_db.claim_value == claim.value
            )
        )
        user_claims = (await self.session.scalars(statement)).all()
    ...
    async def remove_claims(self, user: TUser, *claims: Claim) -> None:
        ...
        for claim in claims:
            statement = select(self.user_claim_db).where(
                and_(
                    self.user_claim_db.user_id == user.id,
                    self.user_claim_db.claim_type == claim.type,
                    self.user_claim_db.claim_value == claim.value
                )
            )
    ...
    async def replace_claim(self, user: TUser, claim: Claim, new_claim: Claim) -> None:
        ...
        statement = update(self.user_claim_db).where(
            and_(
                self.user_claim_db.user_id == user.id,
                self.user_claim_db.claim_type == claim.type,
                self.user_claim_db.claim_value == claim.value
    ...
    async def _find_token(self, user: TUser, login_provider: str, name: str) -> TUserToken:
        statement = select(self.user_token_db).where(
            and_(
                self.user_token_db.user_id == user.id,
                self.user_token_db.login_provider == login_provider,
                self.user_token_db.name == name
    ...
    async def _find_role(self, name: str) -> Optional[TRole]:
        result = await self.session.scalars(
            select(self.role_db)
            .where(self.role_db.normalized_name == name)
        )
        return result.one_or_none()
