...
class Residue:
    ...
    def get_closest_point(self):
        if self.ss == 'coil':
            ...
            closest_point = Graphical.closest_point_on_line((self.x,self.y), ss_line)
...
class Domain:
    ...
    def visualizeV1(self,svg_plane,thickness):
        ...
        for part in cross_class_functions.divide_list_into_parts(self.residue_list,1):
            start_res = part[0]
            end_res = part[-1]
            svg_plane.add(Graphical.create_rectangle_between_2_points(start_res.x,start_res.y,end_res.x,end_res.y,random_color,thickness))
    ...
    def create_dom_ss_vis (self,start_point,end_point, ss_percentages,domain_color, thickness):
        ...
        dx,dy = Graphical.get_dx_dy(start_point[0],start_point[1],end_point[0],end_point[1])
        length = Graphical.get_vector_length(dx,dy)
        norm_vec = Graphical.get_normalized_vector(dx,dy)
        #create sheet:
        sheet_len = ss_percentages[1] * length
        sheet_start_point = Graphical.move_point_vec(middle_point,norm_vec,-(sheet_len/2))
        sheet_end_point = Graphical.move_point_vec(middle_point,norm_vec,sheet_len/2)
        sheet = Graphical.create_arrow_line_between_2_points(sheet_start_point[0],sheet_start_point[1],sheet_end_point[0],sheet_end_point[1],cross_class_functions.lighten_color(domain_color),thickness)
        #create coil:
        coil_len = ss_percentages[2] * length
        coil_start_point = Graphical.move_point_vec(middle_point,norm_vec,-(coil_len/2))
        coil_end_point = Graphical.move_point_vec(middle_point,norm_vec,coil_len/2)
        ...
        helix_start_point = Graphical.move_point_vec(middle_point,norm_vec,-(helix_len/2))
        helix_end_point = Graphical.move_point_vec(middle_point,norm_vec,helix_len/2)
        helix = Graphical.create_simple_helix_line(helix_start_point[0],helix_start_point[1],helix_end_point[0],helix_end_point[1],cross_class_functions.darken_color(domain_color), thickness, 30, 100)
...
class Protein:
    ...
    def check_number_overlappong_ss(self):
        lines = []
        for ss in self.ss_objs:
            if ss.type != 'coil':
                line = ((ss.start_res.x,ss.start_res.y),(ss.end_res.x,ss.end_res.y))
                lines.append(line)
        line_objects = [LineString(line) for line in lines]
        intersecting_lines = [(i, j) for i, j in combinations(range(len(lines)), 2)if Graphical.do_lines_intersect(line_objects[i], line_objects[j])]
        #print("intersecting: "+ str(len(intersecting_lines)))
        return len(intersecting_lines)

    def check_xy_area_of_content_size(self):
        min_x = min_y = min_z =  float('inf')
        max_x = max_y = max_z =  -float('inf')
        for res in self.residues:
            if res.ss !='coil': #ignore coils for viewpoint finding
                min_x, min_y,min_z = min(min_x,res.x), min(min_y,res.y),min(min_z,res.z)
                max_x, max_y,max_z = max(max_x,res.x), max(max_y,res.y),max(max_z,res.z)
        
        return abs(max_x-min_x) * abs(max_y-min_y)

    def check_vis_depth(self):
        min_z = float('inf')     
        max_z = -float('inf')
        for res in self.residues:
            if res.ss !='coil': #ignore coils for viewpoint finding
    ...
    def find_best_view_angle(self,step_width=30):
        ...
        best_num_OV = self.check_number_overlappong_ss()
        best_xy_content_area = self.check_xy_area_of_content_size()
        best_z_depth = self.check_vis_depth()

        print("Start optimal viewpoint calculation: ")
        counter = 0
        for theta_set in tqdm(theta_sets, desc="Testing rotations", unit="set"):
            ...
            points_rotated = rot_protein.rotate_by(rotation_matrix, True)
        
            current_xy_content_area = rot_protein.check_xy_area_of_content_size()
            current_num_OV = rot_protein.check_number_overlappong_ss()
            current_z_depth = rot_protein.check_vis_depth()
    ...
    def recalc_positions_based_based_on_new_avg(self,new_avg):
        ...
        dx,dy = Graphical.get_dx_dy(original_avg_point[0],original_avg_point[1],new_avg[0],new_avg[1])
        norm_vec = Graphical.get_normalized_vector(dx,dy)
        distance = Graphical.get_vector_length(dx,dy)
        for res in self.residues:
            res.x,res.y = Graphical.move_point_vec((res.x,res.y),norm_vec,distance)
    ...
    def split_aligned_part(self,aligned_segment):
        ...
        counter=0
        for residue in self.residues:
            if residue.chain_pos >= aligned_segment[0] and residue.chain_pos <= aligned_segment[1]:
                counter+=1
        print(f'aligned res cointer: {counter}')
    ...
    def connect_to_protein_dashline(self,second_protein):
        if len(self.residues) ==0 or len(second_protein.residues) ==0:
            return None
        dasharray = "15,10"
        dash_line = svgwrite.shapes.Line(start=(self.residues[-1].x, self.residues[-1].y), end=(second_protein.residues[0].x, second_protein.residues[0].y), stroke='black',fill='none', stroke_width=5, stroke_dasharray=dasharray)
        return dash_line
    ...
    def get_protein_ordered_vis_objects(self, avg_coil, mark_endings):
        ...
        for res in self.residues:
            if res.included_in != None:
                ...
                line_x,line_y = Graphical.closest_point_on_line((res.x,res.y),res.ss_obj.get_line()) if res.ss!='coil' else (res.x,res.y)
        ...
        if mark_endings:
            ...
            start_res = self.residues[0]
            end_res = self.residues[-1]
