...
\hline
{Дисперсия} & \sigma^2 = \frac{\sum(x_i - \mu)^2}{N-1} & s^2 = \frac{\sum(x_i-\overline{x})^2}{n-1}\\
\hline
\text{Среднее квадратичное отклонение} & \sigma = \sqrt{\frac{\sum(x_i - \mu)^2}{N-1}} & s = \sqrt{\frac{\sum(x_i-\overline{x})^2}{n-1}}\\
\hline
\end{array}$
...
<hr>'''),
            7: (r'''Сформулируйте определение случайной выборки из распределения. Как в этом случае определяются: выборочное среднее, начальные и центральные моменты выборки, функция распределения выборки? Что в данном контексте означает генеральное среднее?''',
                r'''**Случайной выборкой** объема n из распределения $\mathcal{L}$ называется последовательность n независимых случайных величин $X_1, X_2, ...X_n$, распределенных по одному и тому же закону $\mathcal{L}$. При этом n - объем случайной выборки, а случайные величины $X_1, X_2, ...X_n$ - элементы случайной выборки.
...
= F_Z(z_{\frac{\alpha}{2}}) - F_Z(-z_{\frac{\alpha}{2}}) = 1 - \frac{\alpha}{2} - \frac{\alpha}{2} = 1 - \alpha = \gamma
$
<br>
...
<br>
$P(\theta < \hat{\theta}_{2, n}) = P(\theta > \hat{\theta}_{1, n})$, а следовательно данный доверительный интервал является симмтеричным по вероятности
...
<br>
3) $ \frac{(n-1)s^2}{\sigma^n} \sim \chi(n-1)$
...
+ 2\sum\limits_{j=1}^k \sum\limits_{i=1}^{n_j} (\overline{X_j} - \overline{X}) \cdot (X_{ij} - \overline{X_j})
= \sum\limits_{j=1}^k n_j \cdot (\overline{X_j} - \overline{X})^2
+ \stackrel{\color{lightgreen}{опр.}}{SSE}
+ \sum\limits_{j=1}^k (\overline{X_j} - \overline{X}) \sum\limits_{i=1}^{n_j} (X_{ij} - \overline{X_j})
= \stackrel{\color{lightgreen}{опр.}}{SSTR} + SSE
+ \sum\limits_{j=1}^k (\overline{X_j} - \overline{X}) ((\sum\limits_{i=1}^{n_j} X_{ij}) - n_j \cdot \overline{X_j})
= SSTR + SSE + \sum\limits_{j=1}^k (\overline{X_j} - \overline{X}) (n_j \cdot \overline{X_j} - n_j \cdot \overline{X_j})
= SSTR + SSE + 0 = SSTR + SSE$
...
<br>
1. $\Phi_0$ - неубывающая функция. Допустим, что $\mu_1 = \mu_0$, тогда:
<br>
$W = \frac{1}{2} - \Phi_0(z_\alpha - \frac{\sqrt{n}}{\sigma}\cdot 0) = \frac{1}{2} - \Phi_0(z_\alpha) = F(z_\alpha) = \alpha$
<br>
...
<hr>'''),
            20:
                (r'''По двум независимым выборкам $X_1, X_2, . . . , X_n$ объема n из $N(\mu_X; \sigma^2_X)$ и $Y_1, Y_2, . . . , Y_m$ объема m из $N(\mu_Y; \sigma^2_Y)$, с независимыми но равными дисперсиями ;$\sigma^2_X = \sigma^2_Y = \sigma$; проверяется на уровне значимости α
основная гипотеза $H_0$ : $\mu_X = \mu_Y$ против альтернативной гипотезы $H_1$ : $\mu_X \neq \mu_Y$ . 1) Приведите необходимую статистику критерия и критическое множество для проверки $H_0$ против $H_1$. 2) Приведите (с доказательством) основные свойства критерия. 3) Приведите (с выводом) выражение для P-значения критерия.''',
                 r'''Пусть $X_1, X_2,...,X_n$ - выборка объёма n из $N(\mu_X; \sigma^2_X)$ и $Y_1, Y_2,...,Y_n$ - выборка объёма m из $N(\mu_Y; \sigma^2_Y)$, причём $Var(X) = Var(Y) = \sigma^2$, но неизвестны.
- Основная гипотеза $H_0: \mu_X=\mu_Y$
- Альтернативная гипотеза $H_0: \mu_X \neq \mu_Y$
...
<hr>'''),
            24:
                (r'''Пусть $X_1, X_2, . . . , X_n$ – выборка из дискретного распределения $P(X = −1) = \theta, P(X = 1) = 4\theta, P(X = 2) = 2\theta, P(X = \theta) = 1 − 7\theta, \theta \in (0; \frac {1}{7})$. Найдите оценку параметра $\theta$ по методу максимального правдоподобия. Является ли полученная оценка: а) несмещенной; б) состоятельной. Ответ обосновать.''',
                 r'''Составим функцию правдоподобия:
...
''',
                '''Пусть $X$ - монета, $S$ - сумма монет, $N$ - выпавшее число нужных монет
...
```'''),
            10:
                ('''В группе Ω учатся студенты: $ω_1,...,ω_30$. Пусть X и Y – 100-балльные экзаменационные оценки по математическому анализу и теории вероятностей. Оценки студента $ω_i$ обозначаются: $x_i=X(ω_i)$ и $y_i=Y(ω_i)$, i=1,...,30. Все оценки известны: x1=71,y1=71
, x2=52,y2=58
, x3=72,y3=81
, x4=87,y4=92
, x5=81,y5=81
...
. Требуется найти следующие условные эмпирические характеристики: 1) ковариацию X и Y при условии, что одновременно X⩾50 и Y⩾50; 2) коэффициент корреляции X и Y при том же условии.''',
                 '''$Cov(X, Y) = E(XY) - E(X) \cdot E(Y)$ или $np.cov(X, Y, ddof=0)[0, 1]$
...
; 2) (эмпирическое) стандартное отклонение X
 на потоке Ω
.''',
                 '''* $E = \frac{\sum{n_i \cdot x_i}}{\sum{n_i}}$
...
```'''),
            15:
                ('''Две игральные кости, красная и синяя, подбрасываются до тех пор, пока не выпадет 19 различных (с учетом цвета) комбинаций очков. Пусть $R_i$
...
```'''),
            16:
                ('''Имеется 11 пронумерованных монет. Монеты подбрасываются до тех пор, пока не выпадет 257 различных (с учетом номера монеты) комбинаций орел-решка. Пусть $X_i$
...
```'''),
            17:
                ('''Эмпирическое распределение признаков X
 и Y
...
; 4) квантиль $X_{0, 2}$
.''',
                 '''По условию дан массив частот - необходимо его преобразовать в массив-выборку.
...
Исходные данные: x
 ...
 = (-1,34; -0,25; 0,101; -0,626; -0,088; 0,539; -0,451; 0,233; -1,186; -0,423; -1,329; 0,231; 0,209; 0,638; -0,274; -0,491; -0,319; 0,294; 0,895; 1,164; -0,57; -1,078; -1,526; 1,491; 0,182; -0,31; -1,001; -0,969; -0,918; -0,904; -0,595).''',
                 '''Формула для выборочного коэффициента корреляции $\hat\rho$:
...
```'''),
            21:
                ('''Пусть $\overrightarrow x =(x_1,…,x_{30})$
...
```'''),
            26:
                ('''Для трех групп финансовых показателей $A: (X_1;...;X_{27})
, B: (Y_1;...;Y_{33})
, C: (Z_1;...;Z_{39})$
...
```'''),
            27:
                ('''По содержащейся в файле ds.6.4.1.csv реализации случайной выборки из двумерного нормального распределения $N((E(X); E(Y)); \begin{Vmatrix}
Var(X) & Cov(X, Y)\\
Cov(X, Y) & Var(Y)
...
$A = \overline{(x + 3)(y - 2)} \Rightarrow \sum{(x+3)_i(y-2)_i} = nC$
