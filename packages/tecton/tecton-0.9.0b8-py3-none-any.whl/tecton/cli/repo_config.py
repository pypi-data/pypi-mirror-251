...
def load_repo_config(repo_config_path: Path) -> None:
    ...
    try:
        config = repo_config_module.RepoConfig(**repo_config_dict if repo_config_dict else {})
        ...
        for error_details in e.errors():
            ...
            printer.safe_print(
                ...
                + f"    Path: '{pretty_details.path}'. Value: {pretty_details.value} ({type(pretty_details.value)})."
                + Fore.RESET
            )
...
class _PrettyPydanticErrorDetails:
    ...
    def message(self) -> str:
        pretty_error_msgs = {
            "extra_forbidden": f"Unexpected field name '{self.field_name}' found at path",
            "union_tag_not_found": f"Missing 'kind' field at '{self.path}'. Kind is needed to discrimate Union types, e.g. 'kind: ParquetConfig'",
        }
