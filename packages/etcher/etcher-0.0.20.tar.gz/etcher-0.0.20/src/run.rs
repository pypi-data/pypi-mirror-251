...
pub fn run() -> Result<(), TracedErr> {
    ...
    let logger = setup_logger(vec![LogTarget {
        ...
        // If a read command and verbose hasn't been entered, only log errors to prevent polluting the chainable print output.
        level_filter: if !args.log_level_args.verbose {
            match &args.command {
                args::Command::ReadConfig(_) => log::LevelFilter::Error,
                args::Command::ReadVar(_) => log::LevelFilter::Error,
                _ => args.log_level_args.level_filter(),
            }
        } else {
            args.log_level_args.level_filter()
        },
