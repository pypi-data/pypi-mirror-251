...
from ..internal import lcConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and lcConfig.blacklist:
        if lcConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
def check_command(decorator):
    ...
    def decorator_wrapper(func):
        ...
        if name not in lcConfig.blacklist.disabled_commands:
...
async def check_overwrite(name, *args) -> bool:
    ...
    if not lcConfig.blacklist:
        return False

    if name in lcConfig.blacklist.overwrites:
        await lcConfig.blacklist.overwrites[name](*args)
        return True
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if lcConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="littxlecord admin commands",
            guild_ids=lcConfig.admin_guilds,
