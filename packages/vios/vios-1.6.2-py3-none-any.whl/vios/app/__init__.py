...
from quark import connect as _connect
...
def connect(name: str = 'QuarkServer', host: str = '127.0.0.1', port: int = 2088, user: str = 'baqis'):
    con = _connect(name, srv.get('host', host), srv.get('port', port))
    logger.info(con.login(bootstrap['quarkserver'].get('user', user)))
    return con


sp = defaultdict(lambda: connect())
_vs = _connect('QuarkViewer', port=2086)
