...
async def WorksCleaner(api : AsyncIcodeAPI, page : list[int] | tuple[int] | set[int] = ALL_PAGES, getNum : int = 20) -> bool:
    if page != ALL_PAGES:
        works = []
        for i in page:
            works += await api.getMyWorks(page = i, getNum = getNum)
    else:
        works = await api.getMyWorks(page = 1, getNum = INFINITY)
    if not works:
        return False
    coros = [api.deleteWork(i.get('id')) for i in works]
