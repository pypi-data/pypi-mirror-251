...
Documents: [https://xbzstudio.github.io/icodeapi/docs](https://xbzstudio.github.io/icodeapi/docs).

turingAPI documents: [https://xbz-studio.gitbook.io/turingapi](https://xbz-studio.gitbook.io/turingapi)

------

by [xbzstudio](https://xbzstudio.github.io)
'''
...
class IcodeAPI():
    ...
    def countMessages(self) -> dict:
        ...
        if not self.__loginStatus:
            raise LoginError('User is not logged in')
        response = self.client.get('https://icodeshequ.youdao.com/api/user/not_read/message/count', headers = headers)
        result = response.json().get('data')
        return result
    ...
    def report(self,reportType : Work | Reply | Comment | User,reason : str, reportCategory : int, reportId : str, copyWorksUrl : str | None = None) -> dict:
        '''
        Report work, user, reply or comment.
        ...
        if reportType == Work: # reportType
            body = {'worksIdStr':reportId,
            'category':reportCategory,
            'description':reason,
            'copyWorksUrl' : copyWorksUrl
            }
            url = 'https://icodeshequ.youdao.com/api/works/report'
            ...
            url = 'https://icodeshequ.youdao.com/api/works/user/report'
            ...
            url = 'https://icodeshequ.youdao.com/api/works/comment/report'
        elif reportType == Reply:
            body = {
                'replyId' : reportId,
                'category' : reportCategory,
                'description' : reason
            }
            url = 'https://icodeshequ.youdao.com/api/works/reply/report'
...
class AsyncIcodeAPI(IcodeAPI):
    ...
    async def countMessages(self) -> dict:
        '''
        Get the number of messages.
        ...
        if not self.__loginStatus:
            raise LoginError('User is not logged in')
        response = await self.client.get('https://icodeshequ.youdao.com/api/user/not_read/message/count', headers = headers)
        result = response.json().get('data')
        return result
    ...
    async def report(self,reportType : Work | Reply | Comment | User,reason : str, reportCategory : int, reportId : str, copyWorksUrl : str | None = None) -> dict:
        '''
        Report work, user, reply or comment.
...
正式版：
    ...
    v1.4.0
        支持了小图灵的三个新api:举报用户,举报评论,举报回复,这些功能已被集成在全新的IcodeAPI.report和AsyncIcodeAPI.report方法中,
