    ...
    def run(self):
        ...
        if LiveFeeds.price_feed_connected():
            for index in self.indices_to_trade:
                ...
                strikes_to_track = min(max(strikes_to_track, 10), 25)
                logger.info(
...
class Client:
    ...
    @classmethod
    def from_name(cls, client: str, websockets: bool = False) -> "Client":
        try:
        ...
        instance = cls(user, pin, apikey, authkey, websockets=websockets)
    ...
    def continuously_handle_open_orders(self):
        ...
        while not self.session_terminated:
            ...
            try:
                ...
                if not order_book:
                    logger.info(
                        f"Orderbook is empty. Skipping continuous order handling at {current_time()}"
                    )
                    continue
...
def prepare_complete_exit_params(
    positions: list[dict],
