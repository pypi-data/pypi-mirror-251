...
class SummaryManager(BaseModel):
    ...
    async def get_summarizations_with_timeout(self) -> list:
        print(f'Looking to summarize {len(self.summarization_tasks)} files...')
        if len(self.summarization_tasks) > self.max_summarization_tasks:
            print('Cancelling summarization_tasks...')
            await self.cancel_summarization_tasks(self.summarization_tasks)
            print('Cancelling summarization_tasks_copy...')
            await self.cancel_summarization_tasks(self.summarization_tasks_copy)
            print('Cancelled summarization_tasks and summarization_tasks_copy!')
            error_message = (
                '📚 Workspace too large. '
        ...
        try:
            ...
            print('Cancelling summarization_tasks...')
            await self.cancel_summarization_tasks(self.summarization_tasks)
            print('Cancelled summarization_tasks!')
            ...
            print('Cancelling summarization_tasks_copy...')
            await self.cancel_summarization_tasks(self.summarization_tasks_copy)
            print('Cancelled summarization_tasks_copy!')
    ...
    async def cancel_summarization_tasks(self, task_list: list) -> None:
        tasks = [asyncio.create_task(t) if not isinstance(t, asyncio.Task) else t for t in task_list]
