...
class OptPortfolio:
    """
    利用风险因子数据构建最小方差组合、风险等权组合、特异性风险等权组合
    """
    ...
    def get_min_variance_weight(self, myquad, df_w, use_close_form=True):
        '''
        获取最小方差组合权重
        :param myquad:用于估计协方差矩阵的quad
        :param df_w:指数权重矩阵
        :param use_close_form:是否使用计算最小方差组合的近似形式
        return 最小方差组合权重矩阵
        '''
        import pandas as pd
        import numpy as np
        ivar_ts = myquad.get_ivar().unstack()

        weight_lst = []
        # his_ret_lst = []
        ret_lst = []
        for i, date in enumerate(df_w.index[1:]):
            expected_returns = df_w.loc[date].replace(0, np.nan)
            expected_returns = expected_returns.dropna()
            expected_returns = pd.DataFrame({'returns': expected_returns}, index=expected_returns.index)
            # 使用四元组sigma，构建最小方差组合
            sigma = myquad.get_sigma(date)
            ivar = pd.DataFrame(np.diag(ivar_ts.loc[date].values.tolist()), index=ivar_ts.loc[date].index,
                                columns=ivar_ts.loc[date].index)
            beta = myquad.get_beta(date)
            pred_cov = beta.dot(sigma).dot(beta.T) + ivar
            common_codes = list(set(pred_cov.index) & set(expected_returns.index))
            pred_cov = pred_cov.loc[common_codes][common_codes]
            pred_cov = pred_cov.loc[pred_cov.T.dropna(how='all').index][pred_cov.T.dropna(how='all').index]
            pred_cov = pred_cov.loc[pred_cov.dropna().index].T.loc[pred_cov.dropna().index].T
            # 转换为对称矩阵
            symmetric_matrix = np.tril(pred_cov) + np.diag(pred_cov.values.diagonal()) + np.tril(pred_cov).T
            pred_cov = pd.DataFrame(symmetric_matrix, index=pred_cov.index, columns=pred_cov.columns)
            import pandas as pd
            expected_returns_pred = expected_returns.loc[pred_cov.columns]
            if use_close_form:
                pred_cov_inv = pred_cov * 0 + np.linalg.inv(pred_cov)
                raw_weights = pred_cov_inv.sum() / pred_cov_inv.sum().sum()
                min_var = pd.DataFrame(raw_weights)
                min_var.columns = [date]
            else:
                from pypfopt import EfficientFrontier
                # Optimize for maximal Sharpe ratio
                ef = EfficientFrontier(expected_returns_pred['returns'], pred_cov)
                raw_weights = ef.min_volatility()
                min_var = pd.DataFrame([i for i in raw_weights.values()], index=[i for i in raw_weights.keys()])
                min_var.columns = [date]
            # 过去一期MVP作为权重计算收益率
            weight_lst.append(min_var.T)
        # min_var_ret = (pd.concat(weight_lst).shift() * df_w).dropna(how='all').T.sum()
        weight = pd.concat(weight_lst)
        weight.index.name = 'date'

        return weight

    def get_ivar_equal_weight(self, myquad, ptfl_w):
        '''
        获取等特异性风险组合权重
        :param myquad:用于估计特异性风险的quad
        :param ptfl_w:指数权重矩阵
        return 等特异性风险组合权重
        '''

        ivar = np.sqrt(myquad.get_ivar().unstack())
        weight = (1 / ivar)[~ptfl_w.isna()]
        weight = (weight.T / (weight).sum(axis=1)).T
        return weight

    def get_tot_var_equal_weight(self, myquad, ptfl_w):
        '''
        获取等风险组合权重
        :param myquad:用于估计总风险的quad
        :param ptfl_w:指数权重矩阵
        return 等风险组合权重
        '''
        sys_var = myquad.get_systematic_var(myquad.beta_ts)
        if type(sys_var) == pd.Series:
            sys_var = sys_var.unstack().astype('float')
        var = np.sqrt(sys_var + myquad.get_ivar().unstack())
        weight = (1 / var)[~ptfl_w.isna()]
        weight = (weight.T / (weight).sum(axis=1)).T

        return weight

    def get_opt_portfolio(self, opt='mvp', method='origin'):
        """
        利用风险因子数据求解最优化，输出组合收益与用模型风控后的组合
        :param opt: 最优化投资组合方式mvp(最小方差组合)、eip(等特异性风险组合)、etp(等风险组合)
        :return: 最优化组合结果，包括最优化组合净值、最优化组合风控后净值、最优化组合用模型测度的波动率、最优化组合风控后波动率、最优化组合风控后杠杆
        """
        import pandas as pd
        import numpy as np
        index_weight = self.index_weight
        df = self.stock_ret_df
        myquad = self.factor_quad
        df.index = pd.to_datetime(df.index)
        df = df.asfreq('D', method='pad')
        import we_dms.wiser_data_api.return_api as return_api
        # 月频月度无风险利率
        risk_free_rate = return_api.load_cash_ti(myquad.date_list[0], myquad.date_list[-1]).loc[
            myquad.date_list].pct_change()

        nv_lst = []
        index_name_lst = []
        for index_name, weight in index_weight.items():
            print(f'producing {index_name}  portfolio report')
            # df_w = (df.loc[myquad.date_list].pct_change(20) * index_weight).loc[myquad.date_list]
            df_w = df.loc[myquad.date_list][
                ~index_weight[index_name].loc[
                    sorted(list(set(pd.to_datetime(myquad.date_list)) & set(index_weight[index_name].index)))].replace(
                    0, np.nan).isna()].loc[
                myquad.date_list]
            df_w = df_w.pct_change()
            df_w = (df_w.T - risk_free_rate).T
            index_mean_vol = df_w.sum(axis=1).pct_change().std() * np.sqrt(12)
            index_mean_vol = ((df.loc[myquad.date_list].pct_change() * index_weight[index_name].loc[
                sorted(list(set(pd.to_datetime(myquad.date_list)) & set(index_weight[index_name].index)))].replace(
                0, np.nan)).sum(axis=1)).std() * np.sqrt(12)
            # 计算opt权重,mvp最小方差组合、eip 特异性风险加权组合、etp总风险加权组合
            if opt == 'mvp':
                weight_frame = self.get_min_variance_weight(myquad, df_w)
            elif opt == 'eip':
                weight_frame = self.get_ivar_equal_weight(myquad, df_w)
            elif opt == 'etp':
                weight_frame = self.get_tot_var_equal_weight(myquad, df_w)
            # 最小方差组合权重与资产超额收益率数据相乘，计算优化后组合收益
            opt_portfolio_ret = (weight_frame.shift() * df_w).fillna(0).dropna(how='all').T.sum()
            # 测试最小方差组合波动率
            exante_vol = np.sqrt(myquad.compute_ptfl_var(weight_frame)[0])
            opt_portfolio_nv = (opt_portfolio_ret + 1).cumprod()
            opt_portfolio_rolling_vol = opt_portfolio_nv.pct_change().rolling(12).std().shift(-12) * np.sqrt(12)
            managed_mvp = (((opt_portfolio_ret) * index_mean_vol) / exante_vol + 1).fillna(1).cumprod()
            # 利用组合总收益率计算组合净值曲线
            # min_var_nv = min_var_ret.cumsum() + 1
            # 根据评测的风险将组合权重调整到与指数的风险相同
            # managed_mvp = (((min_var_ret) * index_mean_vol) / exante_vol).cumsum() + 1
            # exante_vol与指数风险相同
            managed_exante_vol = np.sqrt(myquad.compute_ptfl_var(((weight_frame.T * index_mean_vol) / exante_vol).T)[0])
            # 计算组合vol
            managed_rolling_vol = managed_mvp.pct_change().rolling(12).std().shift(-12) * np.sqrt(12)
            # 计算组合杠杆
            leverage = index_mean_vol / exante_vol.replace(0, np.nan)
            if method == 'origin':
                nv_lst.append(opt_portfolio_nv)
                nv_lst.append(opt_portfolio_rolling_vol)
                index_name_lst += [f'{index_name}_opt_nv', f'{index_name}_opt_vol']
            elif method == 'managed':
                nv_lst.append(managed_mvp)
                nv_lst.append(managed_exante_vol)
                nv_lst.append(managed_rolling_vol)
                nv_lst.append(leverage)
                index_name_lst += [f'{index_name}_managed_exante_vol', f'{index_name}_managed_rolling_vol',
                                   f'{index_name}_leverage']

        nv_frame = pd.concat(nv_lst, axis=1)
        nv_frame.columns = index_name_lst
        return nv_frame
...
class RiskDecomposition:
    ...
    def __init__(self,
                 factor_quad: FactorQuadEQ,
                 stock_close_df: Union[pd.DataFrame] = None,
                 date_var_name: str = "date"):
        """
        :param factor_quad: 一个因子四要素对象
        :param stock_close_df: 按照隔离设计原则，不能通过 stock_tickers 来导入数据，而是必须是完整的数据
        :param date_var_name: 日期的变量名，具体是stock_ret_df中的变量名
        """
        self.factor_quad = factor_quad
        ...
        quad_tickers = list(self.factor_quad.psi_ts.columns)
        if stock_close_df is None:  # 如果没有特别指定，就全部有的都用起来；全部对象、全部时间
            self.stock_tickers = quad_tickers
        else:  # 如果有特别指定，就选择对象的交集、时间的交集
            if date_var_name in stock_close_df.columns:
                stock_close_df.set_index(date_var_name, inplace=True)  # 转为index
            stock_close_df.index = pd.to_datetime(stock_close_df.index)  # 转为统一的时间格式
            # stock_close_df.dropna(axis=1, how="any", inplace=True)  # 去掉存在空的列，输入的股票必须是完整的
            selected_tickers = list(stock_close_df.columns)
            self.stock_tickers = list(set(quad_tickers).intersection(set(selected_tickers)))  # 能够分析的，只是交集
            self.date_range = list(set(stock_close_df.index).intersection(set(self.factor_quad.psi_ts.index)))  # 时间的交集
            # todo 似乎还需要判断一下，这个日期中间不应该有缺漏
    ...
    def get_portfolio_return(self, weights: pd.DataFrame):
        ...
        :return:
        """
        return (weights * self.stock_close_df).sum(axis=1).loc[weights.index].pct_change()
...
class CompareReports:
    ...
    def __init__(self,
        ...
        self.stock_portfolio = stock_portfolio
        self.opt_portfolio = opt_portfolio
    ...
    def get_reports(self, report_contents: Union[str, List[str]] = "full", output_file: str = 'stock_risk_report.xlsx'):
        ...
        full_contents = ["full", "sys_idio", "factor_vol", "volatility", "rmv_vov", "stock_vol",
                         "portfolio", "opt_portfolio"]  # 能够处理的所有功能
        ...
        if "opt_portfolio" in report_contents or is_full:
            min_variance_portfolio_data, ivar_equal_portfolio_data, tot_var_portfolio_data = self.get_opt_portfolio_report()
            min_variance_portfolio_data.index = min_variance_portfolio_data.index.astype('str')
            ivar_equal_portfolio_data.index = ivar_equal_portfolio_data.index.astype('str')
            tot_var_portfolio_data.index = tot_var_portfolio_data.index.astype('str')
            page_data = report_data.PageData(
                tables=[min_variance_portfolio_data.reset_index(), ivar_equal_portfolio_data.reset_index(),
                        tot_var_portfolio_data.reset_index()],table_texts=['min_variance_portfolio','ivar_equal_portfolio_data','tot_var_portfolio_data'])
            page_dic["最优化投资组合"] = page_data
    ...
    def get_var_rmv_vov(self) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        for i in range(len(self.decomp_names)):
            ...
            vov = np.sqrt(
                ((target_ret.apply(lambda x: x.dropna().rolling(window=12).std()) * np.sqrt(
                    12) - target_vol) ** 2)).stack()
            rmv_lst.append(rmv)
            var_lst.append(var)
            vov_lst.append(vov)
    ...
    def get_opt_portfolio_report(self):
        """
        生成最小方差组合等报告
        输出：最小方差组合净值列表、总风险等权组合净值列表
        """
        min_variance_portfolio_list = []
        ivar_equal_portfolio_list = []
        tot_var_equal_portfolio_list = []

        for i in range(len(self.opt_portfolio)):
            mvp = self.opt_portfolio[i].get_opt_portfolio(opt='mvp', method='origin')
            mvp.columns = self.decomp_names[i] + ':' + mvp.columns
            eip = self.opt_portfolio[i].get_opt_portfolio(opt='eip', method='origin')
            eip.columns = self.decomp_names[i] + ':' + eip.columns
            etp = self.opt_portfolio[i].get_opt_portfolio(opt='etp', method='origin')
            etp.columns = self.decomp_names[i] + ':' + etp.columns

            min_variance_portfolio_list.append(mvp.asfreq('BM', method='pad'))
            ivar_equal_portfolio_list.append(eip.asfreq('BM', method='pad'))
            tot_var_equal_portfolio_list.append(etp.asfreq('BM', method='pad'))

        return pd.concat(min_variance_portfolio_list, axis=1), pd.concat(ivar_equal_portfolio_list, axis=1), pd.concat(
            tot_var_equal_portfolio_list, axis=1)
...
def test_RiskControl():
    ...
    #                                           pd.to_datetime(start), pd.to_datetime(end), freq='D')  # ['000300.SH']
    # del ptfl_w['000852.SH']
    # del ptfl_w['000016.SH']
    # # del ptfl_w['000300.SH']
    # del ptfl_w['000905.SH']
    ...
    def test_get_simple_report(model_list: List):
        ...
        opt_portfolio_list = []  # 存放最小方差组合等信息
        for model in model_list:  # 生成 decompose 对象
            myquad = FactorQuadEQ.create_factor_quad(factor_system=eval(f'StocksOutputReport.{model}_factor_case_name'),
                                                     ...
                                                     universe=settings.quad_universe,
                                                     local_path=local_path)
            myquad.capped_psi_adjustment()
            mydecomp = RiskDecomposition(factor_quad=myquad, stock_close_df=px_close)  # 在这里就应该指定和选择了
            risk_decomp_list.append(mydecomp)
            # opt_portfolio = OptPortfolio(factor_quad=myquad, stock_ret_df=px_close, index_weight=ptfl_w)
            # opt_portfolio_list.append(opt_portfolio)
        ...
        my_compare_report = CompareReports(decomp_names=model_list, risk_decomp=risk_decomp_list, px_close=px_close,
                                           ...
                                           # opt_portfolio=opt_portfolio_list)
        # 只测mvp
        # my_compare_report = CompareReports(decomp_names=model_list, px_close=px_close,
        #                                    stocks=stocks, stock_names=stock_names, stock_portfolio=stock_portfolio,
        #                                    opt_portfolio=opt_portfolio_list)

        # 由这个report来负责生成各种报告，按照定制化要求，想要什么样的报告，就生成什么样的报告
        # my_compare_report.get_reports(report_contents="opt_portfolio", output_file="stock_risk_report8.xlsx")
