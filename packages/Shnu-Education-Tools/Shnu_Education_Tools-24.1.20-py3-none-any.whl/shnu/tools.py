...
class Assistant:
    ...
    def homework_check(self):
        ...
        for i in homework_dict:
            data[i] = data['姓名'].apply(lambda x: x.strip() in homework_dict[i])
        data['提交次数'] = reduce(lambda x, y: x + data[y], homework_dict.keys(), 0)
        data['提交率(%)'] = data['提交次数'] / len(homework_dict) * 100

        if self.sorted:
            if self.get_sorted_homework_dir() is None:
               self.sorted_homework_dir = self.get_homework_dir() + '_排序'
            data['学号-姓名'] = data.index.map(lambda x: Path(
                self.sorted_homework_dir) / f"{x + 1:02d}-{(df['学号'].str.strip() + '-' + df['姓名'].str.strip())[x]}")

            data['学号-姓名'].apply(lambda x: Path(x).mkdir(parents=True, exist_ok=True))

            dd = data.set_index('学号')['学号-姓名'].to_dict()
        ...
        # 作业记录到Excel 文件中
            data.drop(columns=['学号-姓名'], inplace=True)
        data.to_excel(output_file, index=None)
        Assistant.result=data
        return data,data[data['提交次数']==0],data[data['提交次数']>0]
...
def work(ini_file='shnu.ini'):
        ...
        # 获取配置项的值
        name_list = config.get('shnu','name_list').replace('\n', '')
        homework_dir = config.get('shnu','homework_dir').replace('\n', '')
        sorted=eval(config.get('shnu','sorted').replace('\n', ''))
        sorted_homework_dir = config.get('shnu','sorted_homework_dir').replace('\n', '')
        ast = Assistant(name_list=name_list,homework_dir=homework_dir,sorted=True, sorted_homework_dir='e:/ok')
