...
class JsonPaths:
    ...
    def _analyze_types(value):
        ...
        type_value = val_replace.get(type_value,"UNKNOWN")
    ...
    def _check_delim(self,obj_key):
        ...
        if self._collapse_parent_fields:
            self._collapse_parent_fields = [x.replace(self.__previous_delim,self.delim) for x in self._collapse_parent_fields]
            self.ch
    ...
    def retrieve_objects(self, 
        ...
        def find_descendants(item_path:str):
            ...
            while True:
                ...
                # for x in indirect_descendants:
                #     chk = [s.get('full_path') for s in self.json_schema if s.get('parent_path') == x]
                #     if chk:
                #         direct_descendants.extend(chk)
                #         indirect_descendants = chk 
                #         print(indirect_descendants)
                #     else:
                #         break_loop = True 
                # if break_loop:
                #     break 
        ...
        def check_parent_fields(current_obj,current_obj_schema):
            ...
            if not isinstance(current_obj,dict):
                return None 
            if not self._collapse_parent_fields:
                return 
            fld_paths = [f'{current_obj_schema.get("parent_path")}{self.delim}{x}' for x in current_obj.keys()]
            check_matches = [p.split(self.delim)[-1] for p in fld_paths if p in self._collapse_parent_fields]
            if check_matches:
        ...
        def _check_collapse_parent_flds():
            star_strings = [x.replace(f'{self.delim}*','') for x in self._collapse_parent_fields if x.endswith('*')]
            self._collapse_parent_fields = [x for x in self._collapse_parent_fields if not x.endswith('*') and self.jpschema.schema_obj.get(x)]
            if star_strings:
                ...
                if all_flds:
                    self._collapse_parent_fields.extend(all_flds)           
        ...
        self._collapse_parent_fields = collapse_parent_fields
    ...
    def _create_consumption_schema(self,object_path,replacements:dict=None):
        ...
        if self._collapse_parent_fields:
            for co in self._collapse_parent_fields:
