...
class Model(Module, pl.LightningModule, FromParameters):
    """
    ...
    def save(self,
        ...
        serialized['vocabulary'] = {
            ...
            'parameters': {
                'directory': vocab_serialization_dir,
                'padding_token': self.vocab._padding_token,
                'oov_token': self.vocab._oov_token
            }
        }
    ...
    def _load(
        ...
        if vocab_params['type'] == 'from_files_vocabulary':
            vocab_params['parameters']['directory'] = vocab_dir
        ...
        model = resolve(model_params, pass_down_parameters={'model_name': config.get("model_name", "")})
    ...
    def load(
        ...
        cuda_device: `int = -1`
            ...
            for GPU usage you can specify the idx of your GPU here
    ...
    @classmethod
    def from_archive(cls, archive_file: str, vocabulary: Vocabulary = None) -> "Model":
        """
        ...
        """
        from modules.archival import load_archive  # here to avoid circular imports

        model = load_archive(archive_file).model
        if vocabulary:
            model.vocab.extend_from_vocab(vocabulary)
            model.extend_embedder_vocab()
        return model
    ...
    def __set_validation_metrics(self, validation_metrics: List[str]):
        validation_metrics = validation_metrics or []
        self.validation_metrics = validation_metrics

    def set_training_configuration(self,
                                   optimizer_type: Type = Adam,
                                   scheduler_type: Type = Scheduler,
                                   validation_metrics: List[str] = None,
                                   optimizer_kwargs: Optional[Dict[str, Any]] = None,
                                   scheduler_kwargs: Optional[Dict[str, Any]] = None):
        self.__set_optimizer(optimizer_type, optimizer_kwargs)
        self.__set_scheduler(scheduler_type, scheduler_kwargs)
        self.__set_validation_metrics(validation_metrics)
    ...
    def lr_scheduler_step(self, scheduler: torch.optim.lr_scheduler, metric: Optional[Any]) -> None:
        if not self.validation_metrics:
            warnings.warn("Validation metrics are not set! The learning scheduling will take no effect.")
        scheduler.step(metric=metric)

    @overrides
    def configure_optimizers(self):
        optimizer_set = self.optimizer is not None
        scheduler_set = self.scheduler is not None
        if not optimizer_set or not scheduler_set:
            if not (optimizer_set or scheduler_set):
                message = "optimizer and scheduler"
            else:
                message = "scheduler" if optimizer_set else "optimizer"
            raise ConfigurationError("No " + message + " set!")
