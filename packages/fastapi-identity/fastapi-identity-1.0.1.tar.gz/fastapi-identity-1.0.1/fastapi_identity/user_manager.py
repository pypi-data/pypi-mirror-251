...
from pydentity.password_hasher import IPasswordHasher, PasswordHasher, PasswordVerificationResult
from pydentity.password_validator import IPasswordValidator
from pydentity.claims import ClaimsPrincipal, Claim
from pydentity.schemas import BaseRegisterUser
from pydentity.stores import (
...
from pydentity.token_provider import IUserTwoFactorTokenProvider
from pydentity.types import TUser, DependencyCallable
from pydentity.user_login_info import UserLoginInfo
from pydentity.user_validator import IUserValidator
from pydentity.exc import ArgumentNoneError, InvalidOperationException, NotSupportedException
...
class UserManager(Generic[TUser]):
    ...
    def __init__(
            ...
            *,
            ...
            password_validator: Optional[IPasswordValidator[TUser]] = None,
            user_validator: Optional[IUserValidator[TUser]] = None,
        ...
        :param password_validator: A collection of IPasswordValidator[TUser] to validate passwords against.
        :param user_validator: A collection of IUserValidator[TUser] to validate users against.
    ...
    def _initPV(  # noqa
            self,
            password_validators: Optional[Iterable[IPasswordValidator[TUser]]]
    ) -> tuple[IPasswordValidator[TUser]]:
        return tuple(password_validators) if password_validators else tuple()

    def _initUV(  # noqa
            self,
            user_validators: Optional[Iterable[IUserValidator[TUser]]]
    ) -> tuple[IUserValidator[TUser]]:
        return tuple(user_validators) if user_validators else tuple()
    ...
    def create_model_from_schema(self, schema: BaseRegisterUser):
        return self.store.create_model_from_dict(**schema.model_dump(exclude={'password'}))
    ...
    def register_token_provider(self, provider_name: str, provider: IUserTwoFactorTokenProvider[TUser]) -> None:
        ...
        if not provider_name:
            raise ArgumentNoneError("provider_name")
        if provider is None:
            raise ArgumentNoneError("provider")
    ...
    async def get_username(self, user: ClaimsPrincipal | TUser) -> Optional[str]:
        ...
        if isinstance(user, ClaimsPrincipal):
            return user.find_first_value(self.options.ClaimsIdentity.USER_NAME_CLAIM_TYPE)
        else:
    ...
    async def get_user_id(self, user: ClaimsPrincipal | TUser) -> Optional[str]:
        ...
        if isinstance(user, ClaimsPrincipal):
            return user.find_first_value(self.options.ClaimsIdentity.USER_ID_CLAIM_TYPE)
        else:
    ...
    async def _validate_user(self, user: TUser) -> IdentityResult:
        ...
        if self.user_validator:
            result = await self.user_validator.validate(self, user)
            if not result.succeeded:
    ...
    async def _validate_password(self, user: TUser, password: str) -> IdentityResult:  # noqa
        ...
        if self.password_validator:
            result = await self.password_validator.validate(self, password)
            if not result.succeeded:
                errors.extend(result.errors)
...
UserManagerDependency = DependencyCallable[UserManager[TUser]]
