...
from pydentity.schemas import BaseCreateRole
from pydentity.stores import IRoleStore
...
class RoleManager(Generic[TRole]):
    ...
    def __init__(
            ...
            *,
            role_validator: Optional[IRoleValidator[TRole]] = None,
        ...
        :param role_validator: A collection of validators for roles.
    ...
    def create_model_from_schema(self, schema: BaseCreateRole) -> TRole:
        return self.store.create_model_from_dict(**schema.model_dump())
    ...
    async def _validate_role(self, role: TRole) -> IdentityResult:
        ...
        if self.role_validator:
            result = await self.role_validator.validate(self, role)
