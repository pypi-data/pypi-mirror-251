...
'''
    ...
    def CALLONMODIFY(self, varname:str=''):
        if varname=='':
            for varname in self.__vars.keys():
                self.CALLONMODIFY(varname)
            return
        
        value = self.__vars[varname]
        callbacks = self.__listeners.get(varname, None)
        if callbacks is not None:
            for callback in callbacks:
                callback(value, value)        
        for callback in self.__anylisteners:
            callback(varname, value, value)
        pass
