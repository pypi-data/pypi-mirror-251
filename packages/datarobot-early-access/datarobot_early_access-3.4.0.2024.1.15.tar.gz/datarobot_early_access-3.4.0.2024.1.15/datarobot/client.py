...
def _create_client(
    *args, **kwargs
    ...
    trace_context : str, optional
        ...
        a request.
    """
    drconfig = _create_drconfig(*args, **kwargs)
...
def _get_first_non_none_value(*args) -> Any | None:
    return next((arg for arg in args if arg is not None), None)


# Custom function to get the first non-none value and its source
def _get_value_and_source(*values) -> tuple[Any, int] | tuple[None, None]:
    return next(
...
def _create_drconfig(
    ...
    ssl_verify = _get_first_non_none_value(
        ssl_verify, config.get("ssl_verify"), default_config.get("ssl_verify", True)
    )
...
def _config_from_file(config_path: str) -> Dict[str, Any]:
    """
    ...
    return _config_dict_from_data(data)
