...
class _FairMLBase(PALBase):
    ...
    def _fit(self,
        ...
        if self.submodel_ is not None:
            ...
            dummy_df._columns = ['ID', 'X', 'Y']#pylint:disable=protected-access
            dummy_df._dtypes = [('ID', 'INTEGER', 10),#pylint:disable=protected-access
                                ('X', 'DOUBLE', 15),
                                ('Y', 'INTEGER', 15)]
            self.submodel_.fit(data=dummy_df, key='ID', features=['X'], label='Y')
