...
def subplot_image_batch(x, title=[''], suptitle='', colorbar=[False], vmin=None, vmax=None, cmap='gray', axes=False, returns=False):
    ...
    for i in range(num_img):
        ...
        fig = plt.figure(figsize=(4 * len(title), 4))
        ...
        if suptitle is not None:
            if title[0] != '':
                fig.tight_layout(rect=[0, 0.05, 0, 1.2])
                # fig.tight_layout()
            else:
                fig.tight_layout(rect=[0, 0.05, 0, 1.2])
                fig.tight_layout()
            ...
            # fig.tight_layout(rect=[0, 0.05, 0, 0.9])
            fig.tight_layout()
        if returns:
            fig_list.append(fig)
        plt.show()
...
if __name__ == "__main__":
    ...
    imgs = [data.camera()[...,None], data.camera()[...,None], data.camera()[...,None]]
    ...
    subplot_image_batch(imgs, suptitle="subplot_image_batch", title=["Img 1", "Img 2", "Img 3"],
                        colorbar=True, cmap="gray")
    subplot_image_batch(imgs, suptitle="subplot_image_batch", title=[""],
                        colorbar=True, cmap="gray")
    
    subplot_image_batch(imgs[:2], suptitle="subplot_image_batch", title=["Img 1", "Img 2"],
                        colorbar=True, cmap="gray")
    subplot_image_batch(imgs[:2], suptitle="subplot_image_batch", title=[""],
                        colorbar=True, cmap="gray")
