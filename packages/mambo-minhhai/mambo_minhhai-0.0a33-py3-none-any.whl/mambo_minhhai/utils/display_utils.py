...
def plot_image(x, title='', colorbar=False, show=True, vmin=None, vmax=None, cmap='gray', axes=False, ax=None):
    """
    ...
    if colorbar:
        if ax == None:
            ...
            plt.colorbar(im, ax=ax, fraction=0.047 * (H / W))
...
def subplot_image_batch(x, title=[''], suptitle='', colorbar=[False], vmin=None, vmax=None, cmap='gray', axes=False):
    """
    ...
    for i in range(num_img):
        ...
        if suptitle is not None:
            fig.tight_layout(rect=[0, 0.03, 1, 1.67])
