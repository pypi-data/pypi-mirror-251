...
def payloadJobAll(reportTags, oldmilliSecs, current_time_millis, jobName, jobNumber, page, boolean):
    ...
    if reportTags != "":
        for i, reportTaging in enumerate(reportTags.split(";")):
            # payload.add("tags[" + str(i) + "]", reportTaging) TODO: changes for new api
            payload.add("tags", reportTaging)
    if current_time_millis != 0:
        payload.add("endExecutionTime[0]", current_time_millis)
    # payload.add("_page", page) TODO: changes for new api
    if jobName != "":
        if jobName != "All Jobs":
    ...
    if jobNumber != "" and boolean:
        for i, jobNumber in enumerate(jobNumber.split(";")):
            # payload.add("jobNumber[" + str(i) + "]", jobNumber) TODO: changes for new api
            payload.add("jobNumber", int(jobNumber))
    print(str(payload))
    return payload


def payloadJobAll_old(reportTags, oldmilliSecs, current_time_millis, jobName, jobNumber, page, boolean):
    payload = my_dictionary()
    if oldmilliSecs != 0:
        payload.add("startExecutionTime[0]", oldmilliSecs)
    if reportTags != "":
...
def retrieve_tests_executions(daysOlder, jobName, jobNumber, page, startDate, endDate, reportTag, skip, pageSize):
...
def retrieve_tests_executions_old(daysOlder, jobName, jobNumber, page, startDate, endDate, reportTag):
    current_time_millis = 0
    oldmilliSecs = 0
    if endDate != "":
        if "-" not in endDate:
...
def get_resources(truncated, reportTag, page, daysOlder, resources, jobName, jobNumber, startDate, endDate):
    executionList = {'executionList': '1'}
    skip, pageSize = 0, 200
    while len(executionList) > 0:
        executions = retrieve_tests_executions(
            daysOlder, jobName, jobNumber, page, startDate, endDate, reportTag, skip, pageSize)
        ...
        if len(executionList) == 0:
            ...
            # print(len(executionList))
            resources.extend(executionList)
            skip += pageSize
    return resources


def get_resources_old(truncated, reportTag, page, daysOlder, resources, jobName, jobNumber, startDate, endDate):
...
async def prepare_failed_table(failed_df):
    ...
    # TODO: hyperlinking video link to Failed Steps
...
def format_df(df, jobName, jobNumber, automation_owners):
    ...
    df = df.rename(
        columns={
            ...
            # #TODO: added the below two linesfor new internal api
            "failureReason/name": "Custom Failure Reasons",
