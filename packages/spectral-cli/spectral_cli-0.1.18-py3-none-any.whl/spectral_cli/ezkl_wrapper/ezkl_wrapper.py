...
def generate_proof(data, model_path, prover_key_path, settings_path, srs_path, cwd, mock_first=True, prefix=""):
    ...
    _generate_witness(input_path, model_path, witness_path)
    zk_output = _calculate_zk_output(witness_path, settings_path)
    if mock_first: _generate_mock(witness_path, model_path)
    return (zk_output, _generate_proof(witness_path, model_path, prover_key_path, srs_path, proof_path))

def _generate_witness(input_path, model_path, witness_path):
    ezkl.gen_witness(data=input_path, model=model_path, output=witness_path)
...
def _generate_mock(witness_path, model_path):
    ezkl.mock(witness=witness_path, model=model_path)

def _generate_proof(witness_path, model_path, prover_key_path, srs_path, proof_path):
