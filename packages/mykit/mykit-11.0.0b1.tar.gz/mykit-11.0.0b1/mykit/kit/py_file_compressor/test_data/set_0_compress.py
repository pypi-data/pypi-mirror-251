...
>>>     print('foo\n\nbar')
>>>     print('baz')
>>> print(repr(captured.value))  
  """
  self.fd = fd
  if fd in[0,2]:raise NotImplementedError
  if fd not in[0,1,2]:raise ValueError(f'Invalid `fd` value: {repr(fd)}.')
  self.holder=_io.StringIO()
  self.old=None
 def __enter__(self):
  if self.fd==0:self.old=_sys.stdin
  if self.fd==1:self.old=_sys.stdout
  if self.fd==2:self.old=_sys.stderr
  self.holder=self.holder
  if self.fd==0:_sys.stdin=self.holder
  if self.fd==1:_sys.stdout=self.holder
  if self.fd==2:_sys.stderr=self.holder
  return self
 def __exit__(self,exc_type,exc_value,traceback):
  if self.fd==0:_sys.stdin=self.old
  if self.fd==1:_sys.stdout=self.old
  if self.fd==2:_sys.stderr=self.old
 @property
 def value(self):
