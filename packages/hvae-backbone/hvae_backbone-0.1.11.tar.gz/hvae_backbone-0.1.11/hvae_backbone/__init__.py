...
def training(config):
    ...
    wandb = utils.wandb_init(name=p.log_params.name, config=p.to_json())
...
def analysis(config):
    ...
    checkpoint, save_path = utils.load_experiment_for('test', log_params=p.log_params)
    ...
    for operation in p.analysis_params.ops:
        if operation == 'generation':
            analysis.generation(model, wandb, logger)
        elif operation == 'mei':
            analysis.mei(model, wandb, logger)
        elif operation == 'white_noise_analysis':
            analysis.white_noise_analysis(model, wandb, logger)
        elif operation == 'decodability':
            analysis.decodability(model, dataset, wandb, logger)
        elif operation == 'latent_step':
            dataloader = dataset.get_val_loader(p.eval_params.batch_size)
            analysis.latent_step_analysis(model, dataloader, wandb, logger)
        else:
