...
def save_column_order(
    ...
    columns_without_order = [
        col.order for col in current_node_revision.columns if not col.order
    ]
    if columns_without_order:
        _logger.info(
            "Found columns without order for %s node `%s`",
            node.type,
            node.name,
        )
        if node.type == NodeType.SOURCE:
            ...
            for col in current_node_revision.columns:
                ...
                session.add(col)
            _logger.info(
                "Assigning new column order to %s node `%s`",
                node.type,
                node.name,
            )
            session.commit()
        ...
        return JSONResponse(
            ...
            content={
                ...
                "columns": [
                    ColumnOutput.from_orm(col) for col in current_node_revision.columns
                ],
            },
        )
    return JSONResponse(
