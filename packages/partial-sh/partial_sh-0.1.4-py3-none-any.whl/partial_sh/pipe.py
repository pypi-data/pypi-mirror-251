...
def detect_mode_chain(state: State) -> ChatPromptTemplate:
...
def gen_code_chain(state: State) -> ChatPromptTemplate:
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
...
def execute_step(
    state: State, instruction: str, data: str, header: str | None, prev: str | None
) -> str:
...
def detect_instruction_mode(state: State, instruction, data):
...
def execute_code(state, pipeline, line, instruction):
...
def process_line(state, line, pipeline):
    ...
    for instruciton in pipeline.instructions:
        ...
        if mode == Modes.LLM:
            ...
            output = execute_code(state, pipeline, line, instruciton.instruction)
