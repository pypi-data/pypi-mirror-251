...
def setup_s21(keys,trusted,filename):
    s21 = Srv21()
    s21.auth = get_auth()
    s21.results = {}
    s21.threads = []
...
class MyServer():
  def __init__(self,keys,trusted,filename='s21_b64.txt'):
    self.keys = keys
  ...
  def setup_my_s21(self):
    return setup_s21(self.keys,self.trusted,self.filename)

  def get_my_srv(self):
    try:
      mod_time = self.get_file_mod_time(self.filename)
    except:
      srv = self.setup_my_s21(self.keys,self.trusted,self.filename)
      #mod_time is now
