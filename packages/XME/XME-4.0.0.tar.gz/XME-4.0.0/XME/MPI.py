import XME,pickle,time,numpy as np,traceback
...
class MPI:
	...
	def block_acquire(self, status, connets, funid, arg=(), kwargs={}, to=ANY_PROCESS):
		if to==ANY_PROCESS:
			closed=[]
			while True:
				for tpid,sta in status.items():
					if sta==STATUS_IDLE:
						connets[tpid].send(self.__encode__((funid,arg,kwargs)))
						return self.__decode__(connets[tpid].recv())
			...
			while True:
				if status[to]==STATUS_IDLE:
					...
					return self.__decode__(connets[to].recv())
				elif status[to]==STATUS_END:return

	def acquire(self, status, connets, funid, arg=(), kwargs={}, to=ANY_PROCESS):
		...
		if to==ANY_PROCESS:
			...
			while True:
				for tpid,sta in status.items():
					if sta==STATUS_IDLE:
						pos=self.bufferpos
						Thread(target=send_and_recv,args=(tpid,pos)).start()
						self.bufferpos+=1
			...
			while True:
				if status[to]==STATUS_IDLE:
					pos=self.bufferpos
					Thread(target=send_and_recv,args=(to,pos)).start()
					self.bufferpos+=1
					if self.bufferpos==self.buffersize: self.bufferpos=0
					return pos
				elif status[to]==STATUS_END:return
