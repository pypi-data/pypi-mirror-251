...
class Monitor:
	...
	def marshal(self):
		...
		"""
		'''
		if self.__onlock__("Reader"):result=self.__compact_data__(pickle.dumps(self.__Message__).encode("utf-8"))
		self.__unlock__("Reader")
		return result
		'''
		return self.__compact_data__(pickle.dumps(self.__Message__).encode("utf-8"))
	def unmarshal(self,m_message):
		...
		self.update(pickle.loads(m_message))
