...
from ipih.names import FieldCollectionAliases
import platform
...
from ipih.password import RULES

from ipih.date_time import *
from ipih.collection import (
    FullName,
    User,
)
...
class FullNameTool:
    ...
    @staticmethod
    def to_given_name(
        full_name_holder: FullName | User | str,
...
class ErrorTool:
    ...
    @contextmanager
    def detect(final_action: Callable[[None], None] | None = None, error_handler: Callable[[Any], None] | None = None):
        try:
            yield True
        except Exception as error:
            if nn(error_handler):
