...
class CustomInstall(install):
    def run(self):
        ...
        # Run the standard install process
            ...
            base_command = '/usr/bin/xattr -d com.apple.quarantine'
            ...
            with open(log_file_path, 'a') as log_file:
                ...
                for plugin_path in plugin_paths:
                    try:
                        log_file.write(f"Trying the following {base_command} with path {plugin_path}")
                ...
                try:
                    subprocess.check_call(f"/usr/bin/xattr -rd com.apple.quarantine '{indigo_plugins_path}'", shell=True, stderr=log_file, stdout=log_file)
                    log_file.write(f"Successfully removed quarantine attribute recursively from {indigo_plugins_path} directory\n")
                    subprocess.check_call(f"/usr/bin/xattr -rd com.apple.quarantine '{indigo_disabled_plugins_path}'", shell=True, stderr=log_file, stdout=log_file)
