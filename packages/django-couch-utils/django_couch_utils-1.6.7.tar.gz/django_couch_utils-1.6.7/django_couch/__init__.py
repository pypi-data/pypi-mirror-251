...
try:
    from couchdbrq import Server, Document, ResourceConflict, ResourceNotFound
except ImportError:
...
def db(name=None, partitioning_data=None):
    """name is a key in settings.COUCHDB. If None, returns default database.
    Examples::
      ...
      settings.COUCHDB = {
        ...
        'partitioned_db': {
          'server': 'http://node1:5984',
          'database': 'database_%s',
          'partitioning_function': lambda dbname, params: dbname % params.strftime('%Y'),
        },
      }
    ...
    Next, you may access django databases as::
      ...
      django_couch.db('partitioned_db')() # Database3 (key - partitioned_db) @ http://node1:5984/partitioned_db_2012
      django_couch.db('partitioned_db')(date(2013, 1, 1)) # Database3 (key - partitioned_db) @ http://node1:5984/partitioned_db_2013
    ...
    row = settings.COUCHDB[name]
    dbname = settings.COUCHDB[name]['database']

    if 'partitioning_function' in row:
        dbname = row['partitioning_function'](dbname, partitioning_data)
...
class Updater(object):
    ...
    def push(self):
        ...
        if self.documents:
            res = self.db.update(self.documents)
            self.documents = []
            return res
...
def view_iterator(db, view, **kwargs):
    """Iterator helper. You may call this when you need to iterate over a very large number of rows. This function will automatically split requests in 300 result rows, accurate handling ``start_docid``.
    ...
    block_size = kwargs.get('block_size', 300)
...
class CouchMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        for dbname in settings.COUCHDB:
            ...
            if 'partitioning_function' in settings.COUCHDB[dbname]:
                setattr(request, dbname, partial(db, dbname))
            else:
