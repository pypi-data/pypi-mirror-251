...
<% } %>`),
    ...
    async _closeReviewRequests(closeType) {
      try {
        ...
        alert(interpolate(gettext("An error occurred when attempting to close review requests: %(err)s"), {
          "err": err
        }, true));
      }
    },
    ...
    _showCloseResults(successes, failures) {
      const $dlg = $('<div>').append($('<p>').text(interpolate(ngettext("%s review request has been closed.", "%s review requests have been closed.", successes), [successes])));
      if (failures > 0) {
        $dlg.append($('<p>').text(interpolate(ngettext("%s review request could not be closed.", "%s review requests could not be closed.", failures), [failures]))).append($('<p>').text(gettext("You may not have permission to close them.")));
      }
      $dlg.modalBox({
    ...
    _confirmClose: function () {
      return new Promise((resolve, reject) => {
        const $dialog = $('<div>').append($('<p>').text(gettext("If these review requests have unpublished drafts, they will be discarded."))).append($('<p>').text(gettext("Are you sure you want to close these review requests?"))).modalBox({
          title: gettext("Close review requests"),
          buttons: [$('<input type="button">').val(gettext("Cancel")).click(() => resolve(false)), $('<input type="button">').val(gettext("Close Review Requests")).click(() => resolve(true))]
        }).on('close', () => {
    ...
    _onMuteClicked(ev) {
      ev.stopPropagation();
      ev.preventDefault();
      const $dialog = $('<div>').append($('<p>').text(gettext("Are you sure you want to mute these review requests?"))).modalBox({
        title: gettext("Mute review requests"),
        buttons: [$('<input type="button">').val(gettext("Cancel")), $('<input type="button">').val(gettext("Mute Review Requests")).click(() => this.model.updateVisibility('mute'))]
