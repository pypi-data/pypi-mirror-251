 ...
 <div id="diffs"></div>
  ...
  describe('Diff view buttons', function () {
    describe('Initial state', function () {
      it('Defaults', function () {
        ...
        expect($button.attr('id')).toBe('action-diff-toggle-collapse-changes');
        expect($button.text().trim()).toBe('Collapse changes');
        expect($button.attr('title')).toBe('All lines of the files are being shown. Toggle to ' + 'collapse down to only modified sections instead.');
        $button = $buttons.eq(1);
        expect($button.attr('id')).toBe('action-diff-toggle-whitespace-only');
        expect($button.text().trim()).toBe('Hide whitespace-only changes');
        expect($button.attr('title')).toBe('Sections of the diff containing only whitespace ' + 'changes are being shown. Toggle to hide those instead.');
      });
      it('With allChunksCollapsed=true', function () {
        ...
        expect($button.attr('title')).toBe('Only modified sections of the files are being shown. ' + 'Toggle to show all lines instead.');
        $button = $buttons.eq(1);
        expect($button.attr('id')).toBe('action-diff-toggle-whitespace-only');
        expect($button.text().trim()).toBe('Hide whitespace-only changes');
        expect($button.attr('title')).toBe('Sections of the diff containing only whitespace ' + 'changes are being shown. Toggle to hide those instead.');
      ...
      it('With canToggleExtraWhitespace=true', function () {
        ...
        expect($button.attr('title')).toBe('All lines of the files are being shown. Toggle to ' + 'collapse down to only modified sections instead.');
        $button = $buttons.eq(1);
        expect($button.attr('id')).toBe('action-diff-toggle-extra-whitespace');
        expect($button.text().trim()).toBe('Hide extra whitespace');
        expect($button.attr('title')).toBe('Mismatched indentation and trailing whitespace are ' + 'being shown. Toggle to hide instead.');
        $button = $buttons.eq(2);
        expect($button.attr('id')).toBe('action-diff-toggle-whitespace-only');
        expect($button.text().trim()).toBe('Hide whitespace-only changes');
        expect($button.attr('title')).toBe('Sections of the diff containing only whitespace ' + 'changes are being shown. Toggle to hide those instead.');
      });
      it('With canToggleExtraWhitespace=true and ' + 'diffsShowExtraWhitespace=true', function () {
        RB.UserSession.instance.set('diffsShowExtraWhitespace', true);
        setupPageView({
          canToggleExtraWhitespace: true
        });
    ...
    describe('Actions', function () {
      ...
      it('Show extra whitespace', function () {
        ...
        $button.click();
        expect(RB.UserSession.instance.get('diffsShowExtraWhitespace')).toBeTrue();
        expect($button.text().trim()).toBe('Hide extra whitespace');
      });
      it('Hide extra whitespace', function () {
        ...
        expect($button.text().trim()).toBe('Hide extra whitespace');
        $button.click();
        expect(RB.UserSession.instance.get('diffsShowExtraWhitespace')).toBeFalse();
        expect($button.text().trim()).toBe('Show extra whitespace');
      });
      it('Show whitespace-only changes', function () {
         ...
         */
        $button.click();
        expect(diffReviewableView.toggleWhitespaceOnlyChunks).toHaveBeenCalled();
        expect($button.text().trim()).toBe('Show whitespace-only changes');
  ...
  describe('Without commits', function () {
    beforeEach(function () {
      ...
      spyOn(page.get('reviewRequest'), 'ready').and.callFake((options, context) => options.ready.call(context));
    });
