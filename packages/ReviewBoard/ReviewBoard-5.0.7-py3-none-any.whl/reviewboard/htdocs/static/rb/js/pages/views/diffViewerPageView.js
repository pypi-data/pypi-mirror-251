...
RB.DiffViewerPageView = RB.ReviewablePageView.extend({
 ...
 <div class="diff-box">
  ...
  _viewToggleButtonContentTemplate: _.template(`<span class="fa <%- iconClass %>"></span> <%- text %>`),
  ...
  render() {
    ...
    if (model.diffReviewables.length > 0) {
      model.diffReviewables.each(diffReviewable => this._onDiffReviewableAdded(diffReviewable));
  ...
  addViewToggleButton(options) {
    ...
    const updateButton = isActive => {
      ...
      $button.data('is-active', isActive).attr('title', description).html(this._viewToggleButtonContentTemplate({
        iconClass: icon,
        text: text
      }));
    };
