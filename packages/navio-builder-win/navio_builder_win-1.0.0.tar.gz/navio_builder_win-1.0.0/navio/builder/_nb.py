...
def print_tasks(module, file):
    ...
    for task in sorted(tasks, key=lambda task: task.name):
        ...
        task_list += task_help_format.format(task.name, (f"[{joined_attributes}]") if attributes else "", task.doc)
    print(task_list + "\n\n" + _CREDIT_LINE)


def _get_default_task(module):
    matching_tasks = [task for name, task in inspect.getmembers(module, Task.is_task) if name == "__DEFAULT__"]
    if matching_tasks:
...
def _get_task(module, name, tasks):
    ...
    if not matching_tasks:
        raise Exception(
            "Invalid task '{}'. Task should be one of {}".format(name, ", ".join([task.name for task in tasks]))
        )
    if len(matching_tasks) == 1:
        return matching_tasks[0], args, kwargs
    raise Exception(
        "Conflicting matches {} for task {}".format(", ".join([task.name for task in matching_tasks]), task_name)
    )
...
def _parse_args(args_str):
    ...
    for i, part in enumerate(arg_parts):
        if "=" in part:
            ...
            if len(kwargs) > 0:
                raise Exception("Non keyword arg %s " "cannot follows a keyword arg %s" % (part, arg_parts[i - 1]))
    ...
    if from_command_line or task not in completed_tasks:
        if task.ignored:
            ...
            try:
                ...
                stopTime = int(round(time.time() * 1000))
                logger.critical(f'Error in task "{task.name}". Time: {(float(stopTime) - startTime) / 1000} sec')
                logger.critical("Aborting build for %s" % os.path.abspath(module.__file__))
                raise

            logger.info(f'Completed task "{task.name}". Time: {(float(stopTime) - startTime) / 1000} sec')
...
def _create_parser():
    ...
    parser.add_argument("tasks", help="perform specified task and all its dependencies", metavar="task", nargs="*")

    parser.add_argument("-l", "--list-tasks", help="List the tasks", action="store_true")

    parser.add_argument("-v", "--version", help="Display the version information", action="store_true")
