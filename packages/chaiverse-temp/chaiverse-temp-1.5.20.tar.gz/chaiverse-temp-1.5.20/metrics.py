...
def display_competition_leaderboard(
    ...
    submission_date_range = (
        competition.get('submission_start_date') or '2024-01-01',
        competition.get('submission_end_date') or '2024-01-13'
    )
    feedback_date_range = (
    ...
    if len(df) > 0:
        ...
        title = f'{competition_id} Leaderboard'
        _pprint_leaderboard(display_df, title)
    else:
...
def get_display_leaderboard(df, detailed, competition_type):
    ...
    sort_column = competition_configuration['sort_column']
    output_columns = competition_configuration['output_columns']

    df = df if detailed else _get_ranked_leaderboard(df, sort_column)
...
def get_leaderboard_row(submission_item, developer_key=None, feedback_time_range=None):
    ...
    metrics = get_submission_metrics(submission_id, developer_key, reload=is_updated, feedback_time_range=feedback_time_range)
