"""
    Launcher script to launch a Prod5bMCPipeAlphaSSTsFullMoonNSBJob
...
from CTADIRAC.Interfaces.API.Prod5bMCPipeAlphaSSTsFullMoonNSBJob import (
    Prod5bMCPipeAlphaSSTsFullMoonNSBJob,
)
...
def submit_transformation(job, trans_name):
    ...
    trans.setDescription("Prod5 Alpha SST-only full moon NSB")
    trans.setLongDescription(
        "Prod5 simulation pipeline Alpha SST-only full moon NSB"
    )  # mandatory
...
def submit_wms(job):
    ...
    """
    dirac = Dirac()
    job.setJobGroup("Prod5bAlphaSSTsFullMoonNSBJob")
...
def run_simulation(args):
    ...
    # job setup
    job = Prod5bMCPipeAlphaSSTsFullMoonNSBJob()  # to be adjusted!!
    job.version = "2020-06-29b"
    job.compiler = "gcc83_matchcpu"
    # override for testing
    job.setName("Prod5b_SST_Full_Moon_NSB")
    ...
    if mode == "WMS":
        ...
        trans_name = "Prod5b_SSTOnly_Alpha_Full_Moon_{}_{}_{}_{}{}".format(
            job.cta_site,
            job.particle,
            job.pointing_dir,
            job.zenith_angle,
