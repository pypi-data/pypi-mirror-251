        ...
        try:
            if src.client_data['operacao'].lower().__contains__('okvendas'):
                ...
                clients = [Cliente(
                    ...
                    cpf=c.cpf or "",
                    cnpj=c.cnpj,
                    endereco=Endereco(True, c.address_type, c.address, c.zipcode, c.number, c.complement, c.neighbourhood,
...
def insert_update_semaphore_client_jobs(job_config_dict: dict, db_config: DatabaseConfig, lists: List[Client]) -> bool:
    ...
    """
    try:
        params = [(li.cnpj if li.cpf is None else li.cpf, ' ', IntegrationType.CLIENTE.value, 'SUCESSO') for li in lists]
