...
class Patient():
	...
	def get_indices_in_structure(self, structure):
		...
		if is_tool("plastimatch"):# and mask_path == "":
			...
			for ROIcontourSequence in self.rtstruct.ROIContourSequence:
			...
			for i in range(max_number_of_contours_in_one_slice):
				temporary_rtstructs.append(copy.deepcopy(self.rtstruct))
			...
			with tempfile.TemporaryDirectory() as temp_dir:
				...
				for i,ds in enumerate(temporary_rtstructs):
					...
					command += "--output-prefix {} --prefix-format nrrd --output-ss-list {}/image.txt ".format(temp_dir+"/"+str(i), temp_dir+"/"+str(i))
					command += "--input-dose-img {}".format(self.path_to_rtdose_dose) + " > /dev/null 2> /dev/null"
					
					subprocess.run(command, shell=True)
				
				mask_paths = [temp_dir+"/"+str(i)+"/"+structure_name+".nrrd" for i in range(len(temporary_rtstructs))]

				mask_images = [sitk.ReadImage(mask_path) for mask_path in mask_paths]
		...
		return find_struct_indices_in_rtdose(self.rtstruct, self.rtdose_dose, structure_name)
