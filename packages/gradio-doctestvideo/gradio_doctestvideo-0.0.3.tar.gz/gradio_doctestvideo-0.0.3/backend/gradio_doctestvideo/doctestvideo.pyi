...
class DocTestVideo(Component):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A path or URL for the default value that DocTestVideo component is going to take. Can also be a tuple consisting of (video filepath, subtitle filepath). If a subtitle file is provided, it should be of type .srt or .vtt. Or can be callable, in which case the function will be called whenever the app loads to set the initial value of the component.
    ...
    def preprocess(self, payload: VideoData | None) -> str | None:
        ...
        if self.min_length is not None or self.max_length is not None:
            ...
            if self.min_length is not None and duration < self.min_length:
                raise gr.Error(
                    f"DocTestVideo is too short, and must be at least {self.min_length} seconds"
                )
            if self.max_length is not None and duration > self.max_length:
                raise gr.Error(
                    f"DocTestVideo is too long, and must be at most {self.max_length} seconds"
                )
        ...
        if needs_formatting or flip:
            ...
            if wasm_utils.IS_WASM:
                raise wasm_utils.WasmUnsupportedError(
                    "DocTestVideo formatting is not supported in the Wasm mode."
                )
            ff = FFmpeg(
    ...
    def _format_video(self, video: str | Path | None) -> FileData | None:
        ...
        if (
            ...
            warnings.warn(
                "DocTestVideo does not have browser-compatible container or codec. Converting to mp4"
            )
