...
class JarvisReduceDocumentsChain(BaseCombineDocumentsChain):
        ...
        **kwargs: Any,
        ...
        while num_tokens is not None and num_tokens > _token_max:
            ...
            for docs in new_result_doc_list:
                ...
                # result_docs.append(new_doc)
                task = asyncio.create_task(acollapse_docs(docs, _collapse_docs_func, **kwargs))
                task_list.append(task)
            print(len(task_list))
            a = await asyncio.gather(*task_list)
            print(len(a))
            result_docs.extend(a)
            num_tokens = length_func(result_docs, **kwargs)
        return result_docs, {}
