...
class JarvisVectorStoreRetrieverMemory(BaseMemory):
    ...
    cache: Cache = LRUCache(maxsize=256)
    @property
    ...
    def _get_docs(self, query: str) -> List[Document]:
        cached_docs = self.cache.get(f"{self.memory_id}:{query}", None)
        if cached_docs is not None:
            docs = cached_docs
            print("------------------------------")
        else:
            docs = self.retriever.get_relevant_documents(query)
            self.cache[f"{self.memory_id}:{query}"] = docs
        return docs
