...
from .handlers.environments.handler import EnvironmentsHandler
...
class JupyterEnvironmentsExtensionApp(ExtensionAppJinjaMixin, ExtensionApp):
    ...
    def initialize_handlers(self):
        self.log.debug("Jupyter Environments Config {}".format(self.settings['jupyter_environments_jinja2_env']))
        handlers = [
            ("jupyter_environments", IndexHandler),
            (url_path_join("jupyter_environments", "config"), ConfigHandler),
            (url_path_join("jupyter_environments", "environments"), EnvironmentsHandler),
