...
class OTPTokenViewSet(GenericViewSet):
    ...
    def login(self, request: Request, *args, **kwargs) -> Response:
        ...
        if otp_settings.ENABLE_OTP_AUTHENTICATION:
            ...
            verification_token = self.service.generate_and_notify_token(user=serializer.validated_data["user"], extra_context=extra_context)

        data = self.get_success_response_data(verification_token=verification_token, user=serializer.validated_data["user"])
    ...
    def resend_token(self, request: Request, *args, **kwargs) -> Response:
        ...
        extra_context = self.get_extra_context_data(request=request, validated_data=serializer.validated_data)
        verification_token = self.service.resend_token(user=request.user, extra_context=extra_context)
    ...
    def get_success_response_data(self, **kwargs) -> Dict:
        if self.action in [self.LOGIN, self.RESEND_TOKEN] and otp_settings.ENABLE_OTP_AUTHENTICATION:
