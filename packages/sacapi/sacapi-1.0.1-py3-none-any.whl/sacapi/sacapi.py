...
class SACConnection(object):
    ...
    def addStringFilter(self, providerID, columnName, filterValue, operator):
        ...
        if (operator != self.filterStringOperations.CONTAINS) and (operator != self.filterStringOperations.ENDS_WITH) and (operator != self.filterStringOperations.STARTS_WITH):
            ...
            errMessage = "Invalid value '%s' passed as dimension, measure or account column selection.  Valid values for this model are %s" % (columnName, validCols)
            raise RESTParamsError(errMessage)
        else:
    ...
    def getModelMetadata(self, providerID):
        try:
            ...
            errorMsg = "Error during model metadata acquisition."
            if response.status_code:
                if response.status_code == 404:
                    if providerID not in self.providers.keys():
                        errorMsg = "%s  Status code %s from server.  providerID (model ID) %s is not in the tenant model catalog." % (errorMsg, response.status_code, providerID)
                    else:
                        errorMsg = "Unknown %s  Status code %s from server.  providerID (model ID) %s is in the tenant model catalog, but response status_code was still 404." % (errorMsg, response.status_code, providerID)
                else:
                    errorMsg = "%s  Status code %s from server.  %s" % (errorMsg, response.status_code, e.error)
                raise RESTError(errorMsg)
            else:
                errorMsg = "%s  %s" % (errorMsg, e.error)
                raise Exception(errorMsg)
