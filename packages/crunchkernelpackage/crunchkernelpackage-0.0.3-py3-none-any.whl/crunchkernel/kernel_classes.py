...
class BarChart(AbstractChart):
    def create_chart(self):
        encoded_chart = self._get_encoded_chart(alt.Chart(self.data).mark_bar());
        return encoded_chart

class LineChart(AbstractChart):
    def create_chart(self):
        encoded_chart = self._get_encoded_chart(alt.Chart(self.data).mark_line());
        return encoded_chart

class ScatterChart(AbstractChart):
    def create_chart(self):
        encoded_chart = self._get_encoded_chart(alt.Chart(self.data).mark_circle());
        return encoded_chart
...
def create_dynamic_chart(chart_type_str, data, x_column, y_column, x_aggregate=None, y_aggregate=None,x_type=None, y_type=None,width=720,height=350):
    ...
    if chart_type == ChartType.BAR:
        chart =  BarChart(data, x_column, y_column, x_aggregate, y_aggregate,x_type, y_type, width,height).create_chart()
    elif chart_type == ChartType.LINE:
        chart =  LineChart(data, x_column, y_column, x_aggregate, y_aggregate,x_type, y_type,width,height).create_chart()
    elif chart_type == ChartType.SCATTER:
        chart =  ScatterChart(data, x_column, y_column, x_aggregate, y_aggregate,x_type, y_type,width,height).create_chart()
    else:
        raise ValueError("Invalid chart type. Supported types: 'bar', 'line', 'scatter'")
...
class KernelUtils(AbstractKernelUtils):
    ...
    def list_packages():
        ...
        res = list(map(lambda x: { "name": x.key, "version": x.version }, installed_packages))
...
class ConnectionManager:
    ...
    def getKernelId(self):
        kernel = get_ipython().kernel
        cf=kernel.config['IPKernelApp']['connection_file']
        start = cf.index("kernel")
        kernel_id: str = cf[start:-5]
        return kernel_id

    def getConnectionParams(self, kernel_id, integration_name):
        event_analytics_ur = os.getenv('EVENT_ANALYTICS_URL')
        url = f"{event_analytics_ur}/data/source/recconect?kernelId={kernel_id}&integrationName={integration_name}"

        payload = {}
        headers = { 'workspace': '100' }
        response = requests.request("GET", url, headers=headers, data=payload)
        return response.text

    def executeQueryOnSource(self, query: str, id, depth = 0):
        try:
            if self.connections.get("id"):
                ...
                kernel_id = self.getKernelId()
                params = self.getConnectionParams(kernel_id=kernel_id, integration_name=id)
                self.connectSource(*params)
                return self.executeQueryOnSource(query=query, id=id)
        except Exception as e:
            print(e)
            return False
