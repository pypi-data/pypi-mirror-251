...
class ForeachRule(object, metaclass=Singleton):
    ...
    def rule_task(self,
        ...
        start_datetime_str = start_datetime.strftime(RuleParser.datetime_format())
        end_datetime_str = end_datetime.strftime(RuleParser.datetime_format())
        logging.info("start_time: %s, end_time: %s" % (start_datetime_str, end_datetime_str))
        ...
        for rule in rules:
            ...
            rule_data = self.transform_to_rule_data_pd(result_set, rule, start_datetime_str)
    ...
    @staticmethod
    def transform_to_rule_data_pd(process_result_set,  # type: ResultSet
                                  rule,  # type: dict
                                  start_time,  # type: str
                                  ):
        # type: (...) -> dict
        ...
        for r in process_result_set:
            ...
            if rule_data is None:
                rule_data = RuleData(device_id, rule_ids, datasource_times=start_time)
