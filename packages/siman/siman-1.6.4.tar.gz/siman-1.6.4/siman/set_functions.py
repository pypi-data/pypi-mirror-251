...
def init_default_sets(init = 0):
    ...
    if init or setname not in varset: #init only once
        ...
        s.update()
        header.varset[setname] = copy.deepcopy(s)
        header.varset = read_vasp_sets([('opt' ,'static', {'IBRION'    : 1, 'ISIF'      : 2, 'NSW':20, 'EDIFFG':-0.05, },)] )
    ...
    header.varset = read_vasp_sets([('opt_low' ,'static_low', {'IBRION'    : 1, 'ISIF'      : 2, 'NSW':20, 'EDIFFG':-0.1},)] )
