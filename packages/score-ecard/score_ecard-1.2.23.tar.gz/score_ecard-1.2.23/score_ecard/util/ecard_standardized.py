...
class ECardStandardized(object):
    ...
    def load(self, model_name:str):
        ...
        for i, row in self.score_card.iterrows():
            ...
            bins_ = row['bins_']
            score_ = row['score_']
            if field_ not in self.score_card_processed:
                self.score_card_processed[field_] = []
            self.score_card_processed[field_].append((bins_, score_))
    ...
    def get_single_score(self, data=None):
        ...
        score = 0
        score_detail = {}
        for field_, values in self.score_card_processed.items():
            ...
            try:
                ...
                if not v > -np.inf:
                    v = 0
                for bins_, score_ in values:
                    if v in bins_:
                        score += score_
                        score_detail[field_] = score_
                        break  # 如果找到匹配的bins_, 停止搜索该field_的其他记录
            except:
                score = 0
