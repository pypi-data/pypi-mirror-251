...
class XGBoostWoe:
    ...
    def fit(self,df_X,df_Y,xgb_model=None,sample_weight=None,call_back=call_back_default):
        '''
        ...
        clf_xgb.fit(df_X, df_Y, sample_weight=sample_weight,xgb_model=xgb_model)
        self.xgb_ = clf_xgb
        ...
        if len(clf_xgb.classes_)>2:
            classes_list = clf_xgb.classes_.tolist() * clf_xgb.best_ntree_limit
        else:
            classes_list = clf_xgb.classes_.tolist()[:1] * clf_xgb.best_ntree_limit
        assert len(classes_list)==len(xgb_boundaries),print(len(classes_list),len(xgb_boundaries))
