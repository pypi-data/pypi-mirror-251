...
class posix搜索接口类(搜索接口基类):
    """
    这是一个基于 ssh 管道,通过 paramiko.SSHClient() 执行 locate 命令,并分析返回结果来进行的搜索
    """
    ...
    def 搜索(self,
        ...
        locate搜索字串: str = 搜索关键字
        if not locate搜索字串.startswith('^'):
            locate搜索字串 = f'^{locate搜索字串}'
        if not locate搜索字串.endswith('$'):
            locate搜索字串 = f'{locate搜索字串}$'

        if 参考路径存在:
            locate搜索字串 = locate搜索字串.lstrip('^')
            locate搜索字串 = f'{参考路径.rstrip(路径分隔符)}{路径分隔符}{locate搜索字串}'
        ...
        # 如果条件满足,则执行搜索过程
        if self.可用 and locate搜索字串:
            locate指令: str = ''
            locate参数字: str = f'-er{"" if 大小写敏感 else "i"}{"w" if 路径分隔符 in locate搜索字串 else "b"}'
            if 指定find参数:
                locate指令 = f'locate {指定find参数.lstrip("locate ")}'
            elif 搜文档:
                locate指令 = f'''locate {locate参数字} "{locate搜索字串}" | xargs -I {{}} sh -c 'if [ -f "{{}}" ]; then echo {{}}; fi' '''
            elif 搜路径:
                locate指令 = f'''locate {locate参数字} "{locate搜索字串}" | xargs -I {{}} sh -c 'if [ -d "{{}}" ]; then echo {{}}; fi' '''
