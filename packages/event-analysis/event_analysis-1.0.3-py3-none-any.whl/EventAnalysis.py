...
class EventAnalysis:
    def __init__(self, event_df_:pd.DataFrame, device_Id:None|int = None):
        ...
        self.T = ( ending_date - starting_date ).total_seconds() // utils.number_of_seconds_per_hour
        ...
        self.grid_to_event_map, \
            ...
            utils.create_grid_metadata_arrays(date_index, rain_event_matrix)
    ...
    def initialiseCuda(self):
        ...
        cuda.init()
        self.ES_Source = ESKernel.ES_CODE
        self.ECA_Source = ECAKernel.ECA_CODE
        self.ECA_Pval_Source = ECAKernel.PVALUE_CODE
        ...
        self.ECA_GPU = utils.createGPUFunction(self.ECA_Source, ECAKernel.ECA_kernel_name , self.ECA_block_cnfg)
        self.ECA_Pval_GPU = utils.createGPUFunction(self.ECA_Pval_Source, ECAKernel.P_value_kernel_name , self.ECA_block_cnfg)
        self.ES_GPU = utils.createGPUFunction(self.ES_Source, ESKernel.ES_kernel_name , self.ES_block_cnfg)
    ...
    def ES(self, tauMax:float = np.Inf):
        ...
        ESLoop.ES_Loop(self.number_of_grid_points, self.grdPnt_numOfEvent, Tau_hlpr, tauMax, self.grid_to_event_map, c_i_j)
        end_time = time.time()
        time_spent = end_time - start_time
        utils.printTime(time_spent)
        
        Q = np.add(c_i_j, c_i_j.T).astype('float32')
        ESLoop.normaliseQMatrix(Q, self.grdPnt_numOfEvent)
    ...
    def ECA(self, Delta_T_obj:timedelta, tau:int = 0, return_p_values = False, pValFillNA = True):
        ...
        Delta_T = Delta_T_obj.total_seconds() // utils.number_of_seconds_per_hour
        
        time_spent = 0
        start_time = time.time()
        
        ECALoop.ECA_Loop(self.number_of_grid_points, self.grdPnt_numOfEvent, self.grid_to_event_map, Delta_T, tau, r_precursor_i_j, r_trigger_i_j)
        end_time = time.time()
        time_spent += end_time - start_time
        ...
        if return_p_values:
            ...
            ECALoop.find_p_value_new(self.number_of_grid_points, self.grdPnt_numOfEvent, Delta_T, tau, r_precursor_i_j, r_trigger_i_j, pval_precursor, pval_trigger, self.T, self.logedFactorial)
    ...
    def ECA_vec(self, Delta_T_objs:list[timedelta], taus:int|np.ndarray|list[int] = 0, return_p_values = False, pValFillNA = True):
        ...
        for res in np.arange(numr_results):
            Delta_T = int(Delta_T_objs[res].total_seconds() // utils.number_of_seconds_per_hour)
    ...
    def ECA_Cuda(self, Delta_T_obj:timedelta, tau:int = 0, return_p_values = False, block = None, pValFillNA = True):
        ...
        Delta_T_GPU = np.array(Delta_T_obj.total_seconds() // utils.number_of_seconds_per_hour, dtype=np.int32)
    ...
    def ECA_vec_Cuda(self, Delta_T_objs:list[timedelta], taus:int|np.ndarray|list[int] = 0, return_p_values = False, block = None, pValFillNA = True):
        ...
        Delta_T_GPU = np.array([Delta_T_obj.total_seconds() // utils.number_of_seconds_per_hour for Delta_T_obj in Delta_T_objs], dtype=np.int32)
        for res in np.arange(numr_results):
            # print(res)
