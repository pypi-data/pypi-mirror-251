...
AZ_CONNECTION_STRING = env["AZ_CONNECTION_STRING"]
blob_name = 'test_data_ege.csv'

# Download the test data set from Azure Blob store
CONTAINER_NAME = 'test-data'
blob_name = 'test_data_ege.csv'
blob_service = BlobServiceClient.from_connection_string(AZ_CONNECTION_STRING)
...
def test_coordinate_transform(payload, expected_coords):
    ...
    ct_geog = CoordTrans(crs_from=crs_to, crs_to=crs_from, trans_from=ct_authcode, points=lst_of_point)
    ...
    ct = CoordTrans(crs_from=crs_from, crs_to=crs_to, trans_to=ct_authcode, points=transformed_coords_geog)
...
def test_engineering_transform(payload_engineering, expected_coords):
    ...
    ct_eng2glob = CoordTrans(crs_from=crs_from, crs_to=crs_to, trans_from=ct_authcode, points=points_to_transform)
    ...
    ct_glob2eng = CoordTrans(crs_from=crs_to, crs_to=crs_from, trans_to=ct_authcode, points=transformed_coords_glob)
