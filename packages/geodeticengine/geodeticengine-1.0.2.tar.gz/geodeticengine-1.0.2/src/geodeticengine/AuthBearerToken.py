...
class AuthBearerToken:
    ...
    def get_environment(self, uri) -> str:
        if ".app." in uri or "https://api.equinor.com" in uri:
            return "prod"
        else:
            match = re.search(r"(dev|test|prod)", uri)
            return match.group(1) if match else None

    def get_authorization(self, uri) -> str:
        cred = self._credentials.get(uri)
        if not cred:
            environment = self.get_environment(uri)
            cred = self._credentials.get(environment)
            self._credentials[uri] = cred
