...
try:
    # subprocess.check_call(['pyproj', 'sync', "--all" ,f"--target-directory {PYPROJ_DATADIR}", "-v"])
    # subprocess.check_call(['pyproj', 'sync', "--all", "--include-already-downloaded", "-v"])
    subprocess.check_call(['pyproj', 'sync', "--all", "-v"])
except ConnectionError:
    print("Check your VPN connection or connect to a Equinor network")
