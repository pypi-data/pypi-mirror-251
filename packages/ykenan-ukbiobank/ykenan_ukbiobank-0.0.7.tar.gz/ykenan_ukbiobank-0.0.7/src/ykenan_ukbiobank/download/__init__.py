...
class Download:
    ...
    def _exec_(self, param: Tuple):
        ...
        # 移动文件
        lsi_log_file_dict = self.file.entry_contents_dict(self.log_path, 1, ".lsi")
        lsi_log_file_list = lsi_log_file_dict["name"]

        for lsi_log_file in lsi_log_file_list:
            lsi_log_file: str
            # 读取文件
            filenames = self.file.read_file_line(lsi_log_file_dict[lsi_log_file])
            if len(filenames) > 0:
                # 文件移动
                for filename in filenames:
                    self.file.move_file(filename, os.path.join(self.output_path, filename))
