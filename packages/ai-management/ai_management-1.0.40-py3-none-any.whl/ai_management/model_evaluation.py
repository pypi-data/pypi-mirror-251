...
class ModelEvaluation:
    ...
    def convert_to_arr_if_lst(self, lst):
        """
        Convert a list to a numpy array if the input type is list.

        Parameters:
        - lst: list or another type of array

        Returns:
        - Numpy array if the input type is list, otherwise returns the array itself.
        """
        if type(lst) == list:
            return np.array(lst)
        else:
    ...
    def convert_lsts_to_arrs(self, y_test, y_pred):
        """
        Convert lists to numpy arrays if their types are lists.

        Parameters:
        - y_test: list or another type of array
        - y_pred: list or another type of array

        Returns:
        - Tuple containing y_test and y_pred converted to arrays if necessary.
        """
        y_test = self.convert_to_arr_if_lst(y_test)
        y_pred = self.convert_to_arr_if_lst(y_pred)
        return y_test, y_pred
    ...
    # def define_final_kpi_arr(self, lst, lst_kpis):
        # """
    ...
    def define_final_kpi_arr(self, arr, arr_kpis):
        """
        Defines the final array of KPIs.

        Args:
            arr : numpy.ndarray 
                Array of KPIs to be added.
            arr_kpis : numpy.ndarray
                Array of existing KPIs.

        Returns:
            arr_kpis : numpy.ndarray
                Array of existing KPIs (updated).
        """
        if arr_kpis.size > 0:
            arr_kpis = np.concatenate((arr_kpis, arr))
        else:
            arr_kpis = arr.copy()
        return arr_kpis
    ...
    def binary_classification_evaluation(self, lst_kpis, y_test, y_pred):
        ...
        lst_kpis = self.define_final_kpi_arr(lst, lst_kpis)
        return lst_kpis
...
#             lst.append([f'roc_auc - {multi_class}', roc_auc_score(y_test, y_pred, multi_class=multi_class)])
    ...
    def historize_custom_metric(self, soltn_nm, lst_mdls):
        ...
        lst_cols = []
        for ele in dict_config['schemaExport']:
          lst_cols.append(ele["name"])
        ...
        for model in lst_mdls:
            ...
            for kpi_data in model['data']:
                kpi_nm, kpi_qty, kpi_arg = kpi_data
                kpi_arg_dict = json.dumps(kpi_arg) if kpi_arg else None
                ...
                row_data = {
                    ...
                    'KPI_NM': kpi_nm,
                    'KPI_QTY': kpi_qty,
                    'KPI_ARG': kpi_arg_dict,
                    'TRNNG_TS': datetime.now(),
                    'TEC_CRE_DT': datetime.now(),
                    'TEC_UPT_DT': datetime.now(),
                    'TEC_USER_UPT': soltn_nm
                }

                df_new = pd.DataFrame(columns=lst_cols, data=row_data, index=[0])
