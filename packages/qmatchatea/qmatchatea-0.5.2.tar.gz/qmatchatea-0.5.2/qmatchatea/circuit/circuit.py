...
class Qcircuit:
    ...
    def add_site(self, new_register, position, reference_register="default"):
        ...
        # relative position
        for reg, idxs in self._qregisters.items():
            mask = idxs >= true_pos
            self._qregisters[reg][mask] += 1
        ...
        for layer in self._data:
            for instruction in layer:
                ...
                while search:
                    ...
                    if name in operators.ops:
                        ii = 0
                        if np.allclose(matrix, operators.ops[name]):
                            search = False
                        else:
                        ...
                        search = False
                        operators.ops[name] = matrix.reshape(
                            [self.local_dim] * len(sites)
                        )
                        self._namelist.append(name)

    def write(self, dest, id_mapping):
        """
        Write on file for the fortran simulator, with the correct formatting
