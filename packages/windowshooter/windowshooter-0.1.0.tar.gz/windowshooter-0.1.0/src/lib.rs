...
#[pyclass]
struct PyWindow(screenshot::Window);

#[pyclass]
struct PyRecorder(screenshot::Recorder);

#[pymethods]
impl PyWindow {
    #[new]
    fn new(window_name: String) -> Self {
        PyWindow(screenshot::Window::new_from_name(window_name).unwrap())
    }
}
#[pymethods]
impl PyRecorder {
    #[new]
    fn new(window: &PyWindow) -> Self {
        PyRecorder(window.0.create_record().unwrap())
    }
...
#[pymodule]
fn screenshooter(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
