...
def test_data_model_with_default_values():
    ...
    assert settings.access_token == None
    assert settings.cuda_visible_devices == None
    assert settings.disable_continuous_generation == False
    ...
    assert settings.disable_kernel_injection == 0
    assert settings.disable_static == 0
    assert settings.export_traces == False
