...
class ValidatorLogger:
    ...
    Args:
            config (:obj:`nimble.Config`, `optional`):
                nimble.server.config()
    """
    ...
    def print_weights_table(
        ...
        for uid, weight in zip(sample_uids.tolist(), sample_weights.tolist()):
            if uid in neuron_stats:
                _neuron_stats[uid] = {
                    k: v for k, v in neuron_stats[uid].items()
                }
    ...
    def print_console_validator_identifier(
        self, uid: int, wallet: "nimble.wallet", external_ip: str
    ):
...
class ValidatorPrometheus:
    ...
    def __init__(self, config):
        self.config = config
        self.info = Info(
            "neuron_info", "Info sumamries for the running server-miner."
        )
        self.gauges = Gauge(
            "validator_gauges",
        ...
        self.step_time = Histogram(
            ...
            buckets=list(range(0, 2 * nimble.__blocktime__, 1)),
        )
    ...
    def log_epoch_end(
        self, uid: int, metagraph: "nimble.Metagraph", current_block: int
    ):
