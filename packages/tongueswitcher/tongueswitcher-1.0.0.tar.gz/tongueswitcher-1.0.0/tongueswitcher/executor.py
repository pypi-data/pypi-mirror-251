...
class Executor(TweetProcessor):
    def __init__(self):
        ...
        self.data = pickle.load(open("data/dictionaries.pkl", "rb"))
        self.bigrams = self.open_json("data/combined_bigrams.json")
        self.clh_pos_tags = self.open_json("data/clh_pos_editted.json")

        self.mixed_tagger = ht.HanoverTagger('data/morphmodel_geren.pgz')
        self.en_tagger = ht.HanoverTagger('data/morphmodel_en.pgz')
        self.de_tagger = ht.HanoverTagger('data/morphmodel_ger.pgz')
