...
class WaveformOptions:
    """
    A dataclass for specifying options for the waveform display in the DocTestAudio component. An instance of this class can be passed into the `waveform_options` parameter of `gr.Audio`.
    Parameters:
...
class DocTestAudio(
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A path, URL, or [sample_rate, numpy array] tuple (sample rate in Hz, audio data as a float or int numpy array) for the default value that DocTestAudio component is going to take. If callable, the function will be called whenever the app loads to set the initial value of the component.
        ...
        if self.streaming and "microphone" not in self.sources:
            raise ValueError(
                "DocTestAudio streaming only available if sources includes 'microphone'."
            )
        self.format = format
    ...
    def preprocess(
        ...
        if self.min_length is not None and duration < self.min_length:
            raise Error(
                f"DocTestAudio is too short, and must be at least {self.min_length} seconds"
            )
        if self.max_length is not None and duration > self.max_length:
            raise Error(
                f"DocTestAudio is too long, and must be at most {self.max_length} seconds"
            )
    ...
    def postprocess(
        ...
        if isinstance(value, bytes):
            ...
            if not isinstance(value, (str, Path)):
                raise ValueError(f"Cannot process {value} as DocTestAudio")
            file_path = str(value)
    ...
    def check_streamable(self):
        if (
            ...
            raise ValueError(
                "DocTestAudio streaming only available if source includes 'microphone'."
            )
