    ...
    try:
        if len(sys.argv) > 1:
            if sys.argv[1] == 'resource':
                ...
                try:
                    if sys.argv[2] in WORKSPACE_LIST or sys.argv[2] in ['-h', '--help']:
                        # WORKSPACE
                        parser, subparser = main_parser(sys.argv[1])
                        ServiceParser.create_service_parser(
                            sys.argv[1], subparser)
                        ...
                        logger.error(
                            f"Please provide \"yeedu workspace [workspace-service]\"\n")
                        logger.error(get_similar_subcommand(
                            sys.argv[2], WORKSPACE_LIST))
                        sys.exit(-1)
                except IndexError as e:
                    logger.error(
                        f"Please provide \"yeedu workspace [-h] [workspace-services]\"\n")
                    sys.exit(-1)
                ...
                try:
                    if sys.argv[2] in NOTEBOOK_LIST or sys.argv[2] in ['-h', '--help']:
                        # NOTEBOOK
                        parser, subparser = main_parser(sys.argv[1])
                        ServiceParser.create_service_parser(
                            sys.argv[1], subparser)
                        ...
                        logger.error(
                            f"Please provide \"yeedu notebook [notebook-service]\"\n")
                        logger.error(get_similar_subcommand(
                            sys.argv[2], JOB_LIST))
                        sys.exit(-1)
                except IndexError as e:
                    logger.error(
                        f"Please provide \"yeedu notebook [-h] [notebook-services]\"\n")
                    sys.exit(-1)
...
def call_service_subcommand(parser, command=None):
    try:
        ...
        elif args.subcommand in WORKSPACE_LIST and command == 'workspace':
        ...
        elif args.subcommand in NOTEBOOK_LIST and command == 'notebook':
