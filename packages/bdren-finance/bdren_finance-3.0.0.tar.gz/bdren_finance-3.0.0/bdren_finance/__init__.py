...
def create_entry(payload: dict | str) -> dict:
    """
...
def update_entry(trans_id: int | str, payload: dict | str) -> dict:
    """
    ...
    try:
        ...
        response = session.put(BASE_URL + 'entry/edit/' + str(trans_id) + '/', data=json_payload, headers=headers)
        session.close()
        return response.json()
