        ...
        for ThisSliceNumber in range(NumSlices):
            ...
            for x in range(XTileCounter):
                for y in range(YTileCounter):
                    ...
                    HasFoundImage = False
                    for XRoundingError in range(5):
                        for YRoundingError in range(5):
                            ...
                            try:
                                ...
                                InvertedYIndex = (YTileCounter - 1) - y
                                position = [x * ReferenceImageSize[0],  InvertedYIndex * ReferenceImageSize[1]]  # Corrected positions

                                if (self.AddBorders):
                                            position[0] += ((x + 1) * self.BorderSize_px)
                                            position[1] += ((InvertedYIndex + 1) * self.BorderSize_px)
