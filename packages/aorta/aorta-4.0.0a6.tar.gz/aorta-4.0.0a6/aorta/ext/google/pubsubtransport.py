...
class PubsubTransport:
    ...
    async def send(
        ...
        for envelope in messages:
            for topic in self.get_topics(envelope, is_retry=is_retry):
                futures.append(
                    asyncio.ensure_future(self._send(topic, bytes(envelope)))
                )
    ...
    @retry.Retry(lambda e: True) # type: ignore
    async def _send(self, topic: str, message: bytes):
        future: asyncio.Future[Any] = asyncio.wrap_future(
            future=self.client.publish(topic, message) # type: ignore
        )
        await future