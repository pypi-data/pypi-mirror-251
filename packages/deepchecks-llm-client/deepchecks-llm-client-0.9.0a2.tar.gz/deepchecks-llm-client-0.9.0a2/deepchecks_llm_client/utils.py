...
class Result:
    ...
    return_value: t.Union[str, uuid.UUID, t.List[t.Dict], t.Dict, pd.DataFrame, None]
    ...
    def wraps(self, original_func: t.Callable[..., t.Any]):
        ...
        def wrapper(dc_client_instance, *args, **kwargs):
            try:
                ...
                return Result(status=ResultStatusEnum.ERROR, return_value=None)
            if self.return_self:
                return dc_client_instance
            return Result(status=ResultStatusEnum.SUCCESS, return_value=result)
