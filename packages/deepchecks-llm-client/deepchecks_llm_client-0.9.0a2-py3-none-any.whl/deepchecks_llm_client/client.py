...
from deepchecks_llm_client.utils import HandleExceptions, HandleGeneratorExceptions, Result, get_timestamp
...
class DeepchecksLLMClient:
    ...
    def init(
        ...
        silent_mode: bool = os.environ.get("DEEPCHECKS_SDK_CLIENT_SILENT_MODE", True)
    ) -> Result:
        """
        ...
        if auto_collect:
            self.instrumentor = OpenAIInstrumentor(self.api, log_level, auto_collect)
    ...
    def auto_collect(self, enabled: bool):
        if not self.instrumentor:
            self.instrumentor = OpenAIInstrumentor(
                api=self.api, log_level=self._log_level, auto_collect=enabled,
            )
            self.instrumentor.perform_patch()
        else:
    ...
    def annotate(self, user_interaction_id: str, version_name: str,
                 annotation: AnnotationType = None, reason: t.Optional[str] = None) -> Result:
        """
    ...
    def log_interaction(self,
                        ...
                        custom_props: t.Dict[str, t.Any] = None) -> Result:
        # pylint: disable=redefined-builtin
    ...
    @HandleExceptions(logger)
    def log_batch_interactions(self, interactions: t.List[LogInteractionType]) -> Result:
