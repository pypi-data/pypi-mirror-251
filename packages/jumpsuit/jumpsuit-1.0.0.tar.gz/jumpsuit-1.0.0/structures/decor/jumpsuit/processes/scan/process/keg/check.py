...
try:
	______jumpsuit ["checks"] = checks;	
	______jumpsuit ["checks FOUND"] = True;
except Exception as E:
	print (E)
	______jumpsuit ["checks FOUND"] = False;
		'''

	return contents


import jumpsuit.functions.exceptions as bs_exceptions
...
def start (FIND):
	...
	try:
		...
		exec (
			CONTENTS, 
			{ 
				'______jumpsuit': ______jumpsuit,
				'__file__': FIND
			}
		)
		

		if (______jumpsuit ["checks FOUND"] == False):
			return {
				"empty": True,
				"parsed": True
			}
