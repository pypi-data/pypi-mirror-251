...
m2 = 2
n = len(data[0])
ro, s,x,y, mux, muy,e = symbols('rho sigma x y mu mu exp')
f0 = 1/(2*pi*s*(1-ro**2)**0.5) * exp(-((x-mux)**2-2*ro*(x-mux)*(y-muy)+(y-muy)**2)/(2*s**2*(1-ro)**2))
...
def moneta():
    s = '''
k=11 # количество монет
n = 257 # количество комбинаций
...
def tabl_chast_Ey_stdX():
    s = '''
xk = np.array([100, 300])
xp = np.array([21+17+12, 10+27+13])/100
X = rv_discrete(name='custm', values=(xk, xp))
yk = np.array([1, 2, 4])
yp = np.array([21+10, 17+27, 12+13])/100
Y = rv_discrete(name='custm', values=(yk, yp))
xyk = np.array([100, 200, 400, 300, 600, 1200])
xyp = np.array([21, 17, 12, 10, 27, 13])/100
XY = rv_discrete(name='custm', values=(xyk, xyp))
N = 100
n = 6
print(Y.mean())
