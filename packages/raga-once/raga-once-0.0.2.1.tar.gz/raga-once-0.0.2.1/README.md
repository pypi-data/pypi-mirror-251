...
```bash
python3 -m ragaonce --api_key "YOUR_OPENAI_API_KEY"
```
...
```bash
python3 -m ragaonce --model "gpt-4" --api_key "YOUR_OPENAI_API_KEY" --embeddings "BAAI/bge-small-en" --dataset "path/to/dataset.csv"
```
