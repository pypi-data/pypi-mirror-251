...
class MultiProcessingIterator:
    ...
    def start_processes(self):
        ...
        self.logger.info(f"Multiprocess dataoader starting processes {self.pid}\n")
    ...
    def get_data(self):
        ...
        while True:
            ...
            if isinstance(out, StopIteration):
                self.logger.info(f"MPIterator StopIteration received {self.pid}\n")
                ...
                self.logger.info(f"MPIterator StopChildProcess received {self.pid}\n")
                self.fetch_next_job()
                self.iter_pos += 1
                self.logger.info(f"MPIterator StopChildProcess finished {self.pid}\n")
                raise StopIteration
