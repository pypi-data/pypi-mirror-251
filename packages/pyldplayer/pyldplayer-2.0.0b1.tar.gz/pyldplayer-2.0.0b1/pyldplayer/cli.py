...
from pyldplayer._internal.utils import find_ldconsole
import json
...
@click.option("--search-new", "-s", is_flag=True, help="search for new ldconsole.exe")
@click.pass_context
def cli(ctx: click.Context, path, search_new):
    ctx.ensure_object(dict)
    if path is not None:
        ctx.obj["path"] = path
        dump(path)
    elif os.path.exists(cfg_path) and not search_new:
...
@click.pass_context
def query(ctx, command):
    res = ctx.obj["console"]._LDConsole__console.query(*command.split())
    for r in res:
        click.echo(r)
...
def exec(ctx, command):
    ctx.obj["console"]._LDConsole__console.exec(*command.split())
