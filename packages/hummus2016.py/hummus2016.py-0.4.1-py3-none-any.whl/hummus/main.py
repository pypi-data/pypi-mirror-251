...
"""
  ...
  for arg in args:
    ...
    #print(f"arg {param_name} with annotations {param.annotation.__name__} and kwargs {param.default if param.default is not inspect.Parameter.empty else '_empty'}")
   ...
   try:
    for arg_name, arg in func_args.items():
      if arg.annotation == str:
        custom_kwargs[arg_name] = str(refined[idx])
      if arg.annotation == int:
        custom_kwargs[arg_name] = int(refined[idx])
      if arg.annotation == Author:
        try:
          custom_kwargs[arg_name] = mentions[mention_idx]
        except Exception:
          print(f"Ignoring exception: Provided argument '{refined[idx]}' is not a mention")
        mention_idx += 1
      if arg.annotation.__name__ == "_empty":
        custom_kwargs[arg_name] = refined[idx]
      idx += 1
   except Exception as e:
     print(f"yeah ok uh {e}")
  else:
    for arg in args:
    ...
    async def RUN(self):
      ...
      while True:
        ...
        #finally:
          #print("closing")
          #websocket.close()
          #time.sleep(5)#this is really fucky, you should likely remove this