import numpy as np
from starknet_skproof.float_num.FloatNum import FloatNum
import math
import os
...
class MLPClassifierProver:
    """
    Class for generating cairo prover for pre-trained MLPClassifier circuit.
    ...
    generate_statements(expressions, inputs, outputs)
        Generates and outputs Cairo language addition, multiplication and relu function call statements
    ...
    generate_ar_statement(arg_1, op, arg_2) -> string
        Generates FloatNum addition and multiplication statements using Cairo language syntax, based on the given argumens and operation
    generate_ac_statement(value, act_func) -> string
        Generates activation function call using Cairo language syntax, based on the given argument
    ...
    import_lib()
        Outputs Cairo language struct Float and methods for computations using Float numbers 
    generate_circuit(X)
        Outputs Cairo language circuit for the given input data
    prove(X)
        Generates Cairo language circuit and generates proof for the given input data
    """
    ...
    def generate_statements(self, expressions, inputs, outputs, outputs_string):
        statements = []
        ...
        for e in expressions:
            ...
            if isinstance(arg_1, FloatNum):
                arg_1 = arg_1.get_cairo_input()

            if isinstance(arg_2, FloatNum):
                arg_2 = arg_2.get_cairo_input()
        ...
        declarations += f'\tlet X_0_0 = Float' + '{ ' + f'sign: 0, mantissa: 5100000, exponent: 94' + ' };\n'
        declarations += f'\tlet X_0_1 = Float' + '{ ' + f'sign: 0, mantissa: 3500000, exponent: 94' + ' };\n'
        declarations += f'\tlet X_0_2 = Float' + '{ ' + f'sign: 0, mantissa: 1400000, exponent: 94' + ' };\n'
        declarations += f'\tlet X_0_3 = Float' + '{ ' + f'sign: 0, mantissa: 2000000, exponent: 93' + ' };\n'

        for o in range(len(outputs)):
            f = outputs[o]
            declarations += f'\tlet y_{o+1} = Float' + '{ ' + f'sign: {0 if f.mantissa >= 0 else 1}, mantissa: {f.mantissa}, exponent: {f.exponent}' + ' };\n'
        declarations += '\n'
        ...
        i = 1
        for out in outputs_string:
            constrains += f'\tassert( {out}.sign == y_{y_num}.sign, \'assert {i}\');\n'
            constrains += f'\tassert( {out}.mantissa == y_{y_num}.mantissa, \'assert {i+1}\');\n'
            constrains += f'\tassert( {out}.exponent == y_{y_num}.exponent, \'assert {i+2}\');\n'
            i+=3
            y_num += 1
    ...
    def import_lib(self):
        ...
        lib_data = lib_data.replace('const precision : u256 = 5;', f'const precision : u256 = {self.precision};')
    ...
    def generate_circuit(self, X):
        ...
        # Generate Cairo language statements
        self.generate_statements(expressions, inputs, output_values, outputs)
