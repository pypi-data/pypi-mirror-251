...
from ..tools import expandpath, convert_container, fileiter, soft, sitems
from ..tools import file_lookup
from .containers import merge, Dom, register_container, rebuild, walk
...
def save_config(config, srcfile, condense=True):
    ...
    # save the file
    dumper(config, open(srcfile, "w"), **kwargs)
    return config  # return the (condensed) configuration


def process_config(path, mode="add", **ctx):
    """interprete some special fields to enhance the current config:
...
def build_config(root, pattern, base={}):
    base = Config(base)
