...
class OntoEnv:
    ...
    def import_graph(self, dest: rdflib.Graph, src: FileOrGraph) -> None:
        ...
        if isinstance(src, rdflib.Graph):
            dest += src
        else:
            src = Path(src)
            if not os.path.exists(src):
                raise Exception(f"Could not find {src}")
            self._cache_file(src)
            dest.parse(src, format=rdflib.util.guess_format(str(src)))
        ...
        dest.remove((None, rdflib.OWL.imports, None))
    ...
    def _get_ontology_definition(self, graph: Optional[rdflib.Graph] = None, filename: Optional[OntologyLocation] = None) -> None:
        ...
        if (
            ...
            return
        if graph is None:
            # Check if the file was modified before the state.json file
            state_file_mod_time = os.path.getmtime(self.oedir / "state.json")
            file_mod_time = os.path.getmtime(filename)
            if file_mod_time < state_file_mod_time:
                return
            graph = rdflib.Graph()
