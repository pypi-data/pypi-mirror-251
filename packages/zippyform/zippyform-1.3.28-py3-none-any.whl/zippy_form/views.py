...
from zippy_form.utils import FORM_STATUS, FORM_FIELD_STATUS, FIELD_TYPES, FORM_SUBMISSION_STATUS, \
    ...
    DEFAULT_STRIPE_APPLICATION_FEE_AMOUNT, convert_utc_to_timezone,get_stripe_secret_key,get_stripe_public_key,get_stripe_connect_url, \
...
def form_list(req):
    ...
    if exclude_categories:
        forms = forms.filter(category__in=exclude_categories)
...
def form_submission_details(req,form_id, form_submission_id):
    ...
    if form_type == FORM_TYPE[1][0]:
        # Add Column Headers related to Payment, if the form is Payment Form
        column_headers.append({"field_type": '_price', "label": 'Payment Type'})
