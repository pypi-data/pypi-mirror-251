...
class PersonsLocal(GenericCRUD):
    ...
    def insert_if_not_exists(self, person: Person,
        ...
        lang_code = lang_code or logger.user_context.get_effective_profile_preferred_lang_code()
        ...
        person_id = super().insert(data_json=data_json, ignore_duplicate=True)
        ...
        person_ml_id = super().insert(table_name="person_ml_table", data_json=data_json_ml, ignore_duplicate=True)
    ...
    def update_person_ml_first_name_by_person_id(person_id: int,
        ...
        data = (first_name, lang_code, person_id)
        connection = Connector.connect('person')
        cursor = connection.cursor(dictionary=True, buffered=True)
        cursor.execute(query, data)
        connection.commit()
    ...
    def update_person_ml_last_name_by_person_id(self, person_id: int,
        ...
        data = (last_name, lang_code, person_id)
        connection = Connector.connect('person')
        cursor = connection.cursor(dictionary=True, buffered=True)
        cursor.execute(query, data)
        connection.commit()
    ...
    def delete_person_by_id(self, person_id: int) -> None:
        "delete person"
        logger.start(f"Delete person by person id",
                     object={"person_id": person_id})
        self.delete_by_id(id_column_value=person_id)
        logger.end()
