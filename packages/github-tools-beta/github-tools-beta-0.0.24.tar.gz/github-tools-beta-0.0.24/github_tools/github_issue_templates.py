...
def main():
    ...
    parser.add_argument("-w", "--write_templates", action='store_true',
    ...
    write_teamplates = args.write_templates
    ...
    if write_teamplates:
        ...
        issue_template.handle_templates(write_templates=write_teamplates)
        if title:
    ...
    template = issue_template.get_template(title, update_templates=write_teamplates)
    if template:
        logging.info(f"OK. Outputting template for '{title}' type issue..")
        return template
