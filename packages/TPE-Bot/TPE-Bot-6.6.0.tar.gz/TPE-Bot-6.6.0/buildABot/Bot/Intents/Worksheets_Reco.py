...
class Worksheets_Reco():
  ...
  def createWsIntent(dfWS):
    ...
    for i in range(len(worksheets)):
      button = Worksheets_Reco.getButtonPayload()
      button["text"] = worksheets[i]
      button["link"] = links[i]
      buttons.append(button)

      card = Worksheets_Reco.getTeleInline()
      card["text"] = worksheets[i]
      card["postback"] = links[i]
      cards.append(card)

    intent_payload = Worksheets_Reco.getIntentTemplate()
  ...
  def createHotTopics(df, intent_payload):
    hotTopicsIntents = []
    hotTopicsTele = []
    ...
    for x, rows in df.iterrows():
      ...
      hotTopicTextPayload = Worksheets_Reco.getTextPayload()
      ...
      inline = Worksheets_Reco.getTeleKeyboard(rows["FAQ"])
      # inline["text"] = rows["FAQ"]
      # inline["callback_data"] = rows["FAQ"]
      hotTopicsTele.append(inline)

    # hotTopicsTele.append(Worksheets_Reco.getTeleKeyboard('Learn & Explore'))
    # hotTopicsTele.append(Worksheets_Reco.getTeleKeyboard('Results & Recommendations'))
  ...
  def createSentimentIntents(self, intent_payload):
    ...
    trainingIntent = Worksheets_Reco.getTrainingIntent()
    ...
    for i in range(4):
        ...
        Worksheets_Reco.write_file_json(os.getenv('intentsLogin').format(sentiments[i]), intent_payload)
