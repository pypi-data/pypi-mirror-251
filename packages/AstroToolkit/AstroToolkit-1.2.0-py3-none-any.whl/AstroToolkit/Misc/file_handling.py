...
def create_file(data_copy):
    ...
    # if data type is a dictionary (e.g. data, phot, sed, spectra, etc.)
    if isinstance(data,dict):
        
        # handles image files
        if data['type']=='image':
            # gets file name prefix
            if data['metadata']['source']!=None:
                prefix=data['metadata']['source']
            elif data['metadata']['pos']!=None:
            ...
            # create fits file
            hdu.writeto(f'{prefix}_ATKimage.fits',overwrite=True)
        
        # handles data or phot files
        if data['type']=='data' or data['type']=='phot':
            source=data['source'] 
            ...
            # formats file name
            if data['type']=='data':
                file_name=f'{prefix}_{survey}_ATKdata.csv'
            else:
                file_name=f'{prefix}_{survey}_ATKphot.csv'
        ...
        if data['type']=='bulkphot':
            ...
            file_name=f'{prefix}_ATKbulkphot.csv'
        ...
        if data['type']=='sed':
            ...
            # gets file name
            file_name=f'{prefix}_ATKsed.csv'
        ...
        if data['type']=='spectra':
            ...
            file_name=f'{prefix}_ATKspectrum.csv'
            data_type=data['type']
    ...
    # if data type is a list (e.g. lightcurves)
        ...
        if data[0]['type']=='lightcurve':
            if data[0]['source']!=None:
                prefix=data[0]['source']
                ...
                ra,dec=data[0]['pos'][0],data[0]['pos'][1]
                prefix=f'{ra}_{dec}'
                for i in range(0,len(data)):
                    if data[i]!=None:
                        del data[i]['pos']
            ...
            file_name=f'{prefix}_ATKlightcurve.csv'
...
def read_file(file_name):
    ...
    if 'ATKimage' in file_name:
        image=fits.open(f'{file_name}.fits')[0]
    ...
    if 'ATKlightcurve' in file_name:
        ...
        for element in data_arr:
            ...
            hjd=element['hjd'].tolist()
            hjd_ori=element['hjd_ori'].tolist()
            ...
            element_dict={'type':file_type,'source':source,'pos':pos,'survey':survey,'band':band,'data':{'ra':ra,'dec':dec,'hjd':hjd,'hjd_ori':hjd_ori,'mag':mag,'mag_err':mag_err}}
