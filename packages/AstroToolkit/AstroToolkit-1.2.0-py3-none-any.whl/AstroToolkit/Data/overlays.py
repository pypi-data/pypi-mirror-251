...
def get_overlay(image_dict,survey,ra_name,dec_name,mag_name,overlay_type='detection',marker_type='circle'):
	...
	# 'tracer' here refers to timeseries surveys (e.g. ztf), which can 'trace' an object through time
		...
		if survey=='ztf':
			# set ztf radius to a circle with radius = half diagonal of square image
			radius=math.sqrt(2*(size/2)**2)			

			# get lightcurve data in format [g,r,i]
			if source==None:
				data_arr=lightcurvequery(survey='ztf',pos=location,radius=radius)
			else:
				data_arr=lightcurvequery(survey='ztf',source=source,radius=radius)
