	...
	if 'survey' in keys:
		...
		if tool=='imagequery':
			if survey not in ['panstarrs','skymapper','dss','any']:
				raise Exception(f'Unsupported imagequery survey, supported imagequery surveys are [panstarrs,skymapper,dss].')
		elif tool=='dataquery':
			if survey not in ['gaia','galex','rosat','panstarrs','skymapper','sdss','twomass','wise']:
				raise Exception(f'Unsupported dataquery survey. Supported dataquery surveys are [gaia,galex,rosat,panstarrs,skymapper,sdss,twomass,wise].')
		elif tool=='photquery':
			if survey not in ['gaia','galex','rosat','panstarrs','skymapper','sdss','twomass','wise']:
			...
			if survey not in ['ztf']:
				raise Exception(f'Unsupported lightcurvequery survey. Supported lightcurvequery surveys are [ztf].')
