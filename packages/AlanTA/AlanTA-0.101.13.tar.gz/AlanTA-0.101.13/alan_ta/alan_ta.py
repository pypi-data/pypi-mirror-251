...
def pandas_decorator(func):
    def wrapper(df_ohlc: pd.DataFrame, *args, **kwargs):
        print(f"wrapper kwargs: {kwargs}")
        high, low, close =df_ohlc['high'].values, df_ohlc['low'].values, df_ohlc['close'].values
        kwargs["high"] = high
        kwargs["low"] = low
        kwargs["close"] = close
        func_result = func(*args, **kwargs)
        if isinstance(func_result, tuple):
            result_kwargs = pd.DataFrame(func_result).transpose()
        else:
            result_kwargs = pd.DataFrame(func_result)
        # concat_start - time.time()
        # for i in range(len(func_result))
        # print(f"result decorator: \n{result_kwargs}")
        return result_kwargs
    return wrapper


@pandas_decorator
def atr_pd(high, low, close, length):
    return atr(high, low, close, length)


@pandas_decorator
def dmi_pd(high, low, close, di_length, adx_length):
    return dmi(high, low, close, di_length, adx_length)


@pandas_decorator
def bb_pd(source, length, mult, **kwargs):
    nb_source = source.values
    return bb_pd(nb_source, length, mult)


@pandas_decorator
def ds_pd(high, low, close, factor, atr_length):
    return ds(high, low, close, factor, atr_length)
