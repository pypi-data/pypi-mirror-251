...
class Noop(Token):
    FILLERWORDS = set(
        ...
        during out up around except outside upon as over via past with at before
...
class TimeKeyword(TimeToken):
    name: str
    KEYWORDS: ClassVar = set("ago for in after".split())
...
class Property(Token):
    KEYWORDS: ClassVar = set("open openstart openend closed timepoint empty".split())
...
class FieldModifier(Token):
    ...
    def from_str(
        ...
        if m := re.search(
            ...
            match operator:
                case "+":
                    return AddToField(field=field, values=values, string=string)
                case "-":
                    return RemoveFromField(field=field, values=values, string=string)
                case _ if values:
                    return SetField(field=field, values=values, string=string)
                case _:
