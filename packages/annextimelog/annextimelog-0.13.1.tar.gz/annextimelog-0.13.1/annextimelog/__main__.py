...
def continue_cmd_handler(
    ...
    try:
        ...
        logger.critical(
            f"While interpreting {Token.join(timetokens)!r} as starting time for continuing events matching {Token.join(other)!r}: {e}"
        )
        sys.exit(1)
    actions_: List[Token] = []
    now = dt.now()
    match actions:
        case []:
            actions_.append(TimeFrame(start=now))
        case [Time(time=t) as tt]:
            actions_.append(TimeFrame(start=t))
        case [TimeFrame(start=start, end=end) as tt] if not start:
            actions_.append(tt.evolve(start=now))
        case [TimeFrame(start=start, end=end) as tt]:
            actions_.append(tt)
        case [Duration(duration as d)]:
            actions_.append(TimeFrame(start=now, end=now + d))
        case _:
            logger.critical(
                f"Don't know how to interpret {Token.joinpretty(actions)} as start for continuing events matching {Token.join(other)!r}"
            )
            sys.exit(1)
    actions = [t.evolve(is_condition=False) for t in actions_]
    conditions = [t.evolve(is_condition=True) for t in other]
