...
class AnnextimelogRepo:
    ...
    def store(self, event: Event) -> bool:
        ...
        if (is_new := self.dirty) and event.config.get(
            "annextimelog.commit", "true"
        ) == "true":
            with stderr.status(
                status := f"Committing addition of event {event.id!r}..."
            ):
                result = self.run_git(
                    subprocess.run,
                    [
                ...
                if not result.returncode:
                    logger.info(f"✅ Committed addition of event {event.id!r}")
        if not is_new:
            # reset metadata
...
class Event:
        ...
        for token in tokens:
            match token:
                ...
                case TimeEnd(time=t):
                    self.end = t
                case TimeFrame(start=start, end=end):
                    # setting to 'until now' keeps the old start. OK?
        ...
        if isinstance(tokens, Token):  # only one token given
            ...
            match token:
                ...
                case Property(name="closed"):
                    return bool(self.start and self.end) and not (
                        self.start == self.end
                    )
    ...
    def to_rich(self, long=None) -> RenderableType:
        ...
        if not any(self.to_dict().values()):
            ...
            return table
        table.add_column("", justify="left", width=None if emojis else 0)
        ...
        if self.id:
            table.add_row("💳" if emojis else "·", "id", f"[b]{self.id}[/b]")
        ...
        if self.paths and longlist:
            table.add_row("🔑" if emojis else "·", "key", self.key)
        timehighlighter = ISO8601Highlighter()
        start, end = self.start, self.end
        if start and end and start == end:
            table.add_row(
                "🕰️" if emojis else "·", "time", start.astimezone().strftime("%c%Z")
            )
        elif start or end:
            if start:
                table.add_row(
                    "🚀" if emojis else "·", "start", start.astimezone().strftime("%c%Z")
                )
            if end:
                table.add_row(
                    "⏱️" if emojis else "·", "end", end.astimezone().strftime("%c%Z")
                )
            if start and end:
                table.add_row(
                    "⌛" if emojis else "·",
                    "duration",
                    td(seconds=(end - start).total_seconds()).pretty_duration,
                )
        if location := self.fields.get("location", set()):
            table.add_row(
                "📍" if emojis else "·",
                "location",
                " ".join([f"{'📍 ' if emojis else '·'}{t}" for t in sorted(location)]),
            )
        if self.tags:
            table.add_row(
                "🏷️" if emojis else "·",
        ...
        if self.note:
            table.add_row("📝" if emojis else "·", "note", self.note)
        return table
