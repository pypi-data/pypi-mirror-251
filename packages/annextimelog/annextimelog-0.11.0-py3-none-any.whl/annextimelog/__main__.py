...
def summary_cmd_handler(args: argparse.Namespace, other_args: List[str]):
    ...
    events = sorted(
        args.repo.find_events(tokens), key=lambda e: e.start or e.end or dt.now()
    )
...
def edit_cmd_handler(args: argparse.Namespace, other_args: List[str]):
    if other_args:
        logger.warning(f"🙈 Ignoring other arguments {other_args}")
    try:
    ...
    for event in events:
        ...
        if (
            ...
            table.add_column(width=3, ratio=0.1)
