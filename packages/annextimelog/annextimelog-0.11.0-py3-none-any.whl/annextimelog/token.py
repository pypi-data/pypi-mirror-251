        ...
        for subcls in cls.__subclasses__():
            if from_str := getattr(subcls, "from_str", None):
                try:
                    ...
                    logger.error(
                        f"Calling {from_str}({string!r}) didn't work: {e.__class__.__name__} {e!r}"
                    )
        ...
        *triggers: type[Token],
        ...
        for token in tokens:
            if any(isinstance(token, t) for t in prohibited):
                raise ValueError(
                    "in this context (splitting by {'/'.join(t.__name__ for t in triggers)}), "
        ...
        if wanted:
            ...
            if len(result) == 1 and any(isinstance(result[0], c) for c in wanted):
                ...
                raise ValueError(
                    ...
                    f"(instead it reduces to {len(result)} tokens {Token.join(result)!r} ({'·'.join(t.__class__.__name__ for t in result)})"
                )
            return

        def reduce_fail_msg(problemtokens):
            return (
                f"Don't know how to interpret or reduce time-related tokens {Token.join(problemtokens)!r} ({'·'.join(t.__class__.__name__ for t in problemtokens)}) (from {Token.join(tokens)!r}). "
        ...
        if None in (conds := set(t.is_condition for t in tokens)) or len(conds) > 1:
            ...
            if unknown:
                match bool(actions), bool(conditions):
                    ...
                    case _:
                        raise ValueError(
                            f"Don't know whether tokens {Token.join(unknown)!r} are actions or conditions in this context (no 'set', 'if', etc. and {is_condition=})."
                        )
            ...
            if sincetokens or untiltokens:
                match bool(sincetokens), bool(untiltokens):
                    case True, False if unknowntokens:
                        logger.debug(
                            f"It's *since* {Token.join(sincetokens)!r}, so *untiltokens* {Token.join(unknowntokens)!r}"
                        )
                        untiltokens.extend(unknowntokens)
                    case False, True if unknowntokens:
                        logger.debug(
                            f"It's *untiltokens* {Token.join(untiltokens)!r}, so *since* {Token.join(unknowntokens)!r}"
                        )
                ...
                case [  # month, week, hour, etc...
                    ...
                    match tokens:
                        ...
                        case [TimeKeywordIter() as w, TimeKeywordUnit() as u]:
                            unit, n = u, w.n
                    yield TimeFrame(
                        start=now.this(unit.name, n, **wss),
                        end=now.this(unit.name, n + 1, **wss),
                        **ss,
                    )
                case [  # this Monday
    ...
    def from_strings(
        ...
        **kwargs,
        ...
        if reduce:
            ...
            warnings.warn("from_strings(): unused {kwargs = } with {reduce = }")
        return tokens
...
class TimeKeyword(TimeToken):
    ...
    def from_str(cls, string: str, **kwargs) -> Union[TimeKeyword, None]:
        for subcls in cls.__subclasses__():
            if token := subcls.from_str(string, **kwargs):
                return token
        if string.lower() in cls.KEYWORDS:
            return cls(string=string, name=string, **kwargs)
        return None
...
class FieldModifier(Token):
    ...
    def from_str(
        ...
        if m := re.search(
            ...
            sep = sep or ","
            cast(List[str], values)  # make mypy happy
            values = list(filter(bool, re.split(rf"(?:{re.escape(sep)})+", values)))
            match field.lower():
                case "start" | "end" as field_:
                    ...
                    match field_:
                        case "start":
                            return TimeStart(string=string, time=time)
                        case "end":
                            return TimeEnd(string=string, time=time)
                ...
                case str() as f if "location".startswith(
                    ...
                    field = "location"
            values = cast(FrozenSet[str], frozenset(values))
            match operator:
                case "+":
