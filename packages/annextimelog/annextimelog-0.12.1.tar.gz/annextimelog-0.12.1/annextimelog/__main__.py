...
def summary_cmd_handler(
    ...
    events = sorted(
        ...
        key=lambda e: e.start or e.end or dt.now().astimezone(),
    )
    ...
    for event in events:
        if getattr(args, "id_only", None):
            ...
            continue
        match event:
            case Event(start=start, end=end) if start and end:
    ...
    logger.info(
        ...
        f"with a closed total of {td(seconds=total_seconds).pretty_duration} tracked."
    )


def edit_cmd_handler(args: argparse.Namespace, other_args: Optional[List[str]] = None):
    if other_args:
        logger.warning(f"🙈 Ignoring other arguments {other_args}")
    try:
        logger.debug(f"Interpreting query {shlex.join(args.query)!r}")
        tokens = Token.from_strings(args.query, config=args.repo.config)
    except ValueError as e:
        if logger.getEffectiveLevel() <= logging.DEBUG:
...
def stop_cmd_handler(args: argparse.Namespace, other_args: Optional[List[str]] = None):
    if other_args:
        logger.warning(f"🙈 Ignoring other arguments {other_args}")
    args.query = shlex.split("set end=now if .openend") + args.query
    logger.info(f"Transformed query into {shlex.join(args.query)!r}")
    return edit_cmd_handler(args)
...
> atl mod project=secret if id=O3YzvZ4m  # add custom tag to specific event
> atl mod id=LXKGrBGA update end=now     # change end of specific event
