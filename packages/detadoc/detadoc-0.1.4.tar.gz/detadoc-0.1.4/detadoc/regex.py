...
class Transaction(AbstractRegex):
    GROUP_PATTERN = re.compile(r'^(?P<account>[A-Z]{3})\s(?P<amount>\d+[.,]\d+|\d+)\s(?P<type>[CD])\s(?P<accounting_date>\d{4}-\d{2}-\d{2})\s(?P<invoice_key>\w+)\s(?P<description>[\w\s$/]+)')
    SEPARATOR = ' '
    ...
    @property
    def display(self):
        return f'{self.parsed_value} R$ {self.accounting_date} {self.account.title} {self.description}'
...
if __name__ == '__main__':
    x = Transaction('CAT 2000 D 2024-01-01 ddddd daniel victor R$ teste')
    print(x)
    print(x.invoice_key)
    print(x.description)
