...
@pytest.mark.parametrize("HeaderName, ParamName1, ParamName2, ParamName3, ParamName4", excelbdd.get_example_list(excelBDDFile,"Sheet1","Scenario"))
...
def test_loaddata_from_Sheet2():
    testcaseSetList = excelbdd.get_example_list(excelBDDFile,"Sheet2","Scenario")
...
@pytest.mark.parametrize("HeaderName, ParamName1, ParamName2, ParamName3, ParamName4", excelbdd.get_example_list(excelBDDFile,"Sheet3"))
...
@pytest.mark.parametrize("HeaderName,ExcelFileName, SheetName, HeaderMatcher, HeaderUnmatcher, Header1Name, FirstGridValue, LastGridValue, \
                         ...
                         excelbdd.get_example_list(excelBDDFile,"SmartBDD"))
def test_example_list(HeaderName,ExcelFileName, SheetName, HeaderMatcher, HeaderUnmatcher, Header1Name, FirstGridValue, LastGridValue, 
                      ParamName1InSet2Value, ParamName2InSet2Value, ParamName3Value, MaxBlankThreshold, ParameterCount, TestDataSetCount):
    testcaseSetList = excelbdd.get_example_list("../BDDExcel/" + ExcelFileName, SheetName, HeaderMatcher,HeaderUnmatcher)
...
@pytest.mark.parametrize("HeaderName, ExcelFileName, SheetName, Message", 
                         excelbdd.get_example_list(excelBDDFile,"Exception"))
def test_excelbdd_exception(HeaderName, ExcelFileName, SheetName, Message):
    ...
    with pytest.raises(Exception) as ex:
        testcaseSetList = excelbdd.get_example_list("../BDDExcel/" + ExcelFileName, SheetName)

    assert Message in str(ex.value)   

def test_excelbdd_no_sheetName():
    testcaseSetList = excelbdd.get_example_list("../BDDExcel/ExcelBDDSampleA.xlsx")
    assert len(testcaseSetList) == 5

def test_excelbdd_one_set():
    testcaseSetList = excelbdd.get_example_list("../BDDExcel/Sample.xlsx", 'Gain')
    print(testcaseSetList)
    assert len(testcaseSetList) == 1 

def test_excelbdd_stop_when_blank_header():
    testcaseSetList = excelbdd.get_example_list("../BDDExcel/Sample.xlsx", 'Gain2')
    print(testcaseSetList)
