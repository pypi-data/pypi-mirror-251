...
class IosDriver(object):
    ...
    def __init__(self, device_id: str = None, pkg_name: str = None):
        ...
        if not self.pkg_name.startswith('com.'):
            self.pkg_name = TideviceUtil.get_bundleid_by_name(pkg_name)
        else:
    ...
    def start_bundle(self, pkg_name=None, stop=True):
        """启动应用
        @param pkg_name:
        @param stop: 是否先停止应用
        """
...
if __name__ == '__main__':
    TideviceUtil.get_bundleid_by_name('企知道')
