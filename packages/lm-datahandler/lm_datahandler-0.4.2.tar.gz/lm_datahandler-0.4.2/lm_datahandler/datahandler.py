...
class DataHandler(object):
    ...
    def load_data_file(self, data_name, eeg_path, acc_path, ble_path, sti_path, person_info=None, do_padding=True):
        """
        ...
        self.raw_eeg, self.eeg, self.raw_acc, self.acc, self.raw_sti, self.disconnections, self.total_time, self.start_time, self.end_time, self.package_loss_rate, self.disconnect_rate = data_loader.load_data(
            eeg_path, acc_path, sti_path, ble_path, data_type=self.device_type, logger=self.logger, do_padding=do_padding)
    ...
    def concat_data_file(self, eeg_path, acc_path, ble_path, sti_path, do_padding):
        """
        拼接数据文件：
        中间的时间间隔会进行数据补0
        ...
        raw_eeg, eeg, raw_acc, acc, raw_sti, disconnections, total_time, start_time, end_time, package_loss_rate, disconnect_rate = data_loader.load_data(
            eeg_path, acc_path, sti_path, ble_path, data_type=self.data_type, logger=self.logger, do_padding=do_padding)
    ...
    def concat_data(self, data_path, do_padding):
        """
        ...
        self.concat_data_file(eeg_path, acc_path, ble_path, sti_path, do_padding)

    def load_data(self, device_type, data_name, data_path, patient_info=None, do_padding=True):
        """
        ...
        self.load_data_file(data_name, eeg_path, acc_path, ble_path, sti_path, None, do_padding)
        return self
