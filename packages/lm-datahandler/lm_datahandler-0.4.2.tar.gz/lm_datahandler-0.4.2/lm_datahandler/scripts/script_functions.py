...
def local_datas_full_analyse(data_path, data_names, analysis_save_path=None, analyse_param=None):
    ...
    if analyse_param is None:
        ...
        do_padding = analyse_param["do_padding"]
    ...
    for i, data_name in enumerate(data_names):
        ...
        try:
            ...
            data_handler.load_data(device_type=device_type, data_name=data_name, data_path=temp_data_path,
                                   patient_info=patient_info, do_padding=do_padding)
            if parse_to_mat:
...
def local_data_concat_and_analyse(data_path, data_names, analysis_save_path=None, analyse_param=None):
    ...
    for i in range(len(data_names)):
        ...
        if i == 0:
            ...
            data_handler.load_data(device_type=device_type, data_name=data_names[0],
                                   data_path=os.path.join(data_path, data_names[i]),
                                   patient_info=patient_info, do_padding=do_padding)
        else:
            data_handler.concat_data(data_path=os.path.join(data_path, data_names[i]), do_padding=do_padding)
    ...
    if data_type == "sleep":
        ...
        # 进行睡眠分期，计算睡眠指标，绘制睡眠综合情况图，并保存
        data_handler.preprocess(filter_param=pre_process_param).sleep_staging_with_internal_model(
            sleep_staging_param['use_acc'],
            sleep_staging_param['use_time'],
            sleep_staging_param['staging_mode'])
        ...
        if save_features:
            data_handler.save_features_to_parquet(feature_path=features_save_path)
