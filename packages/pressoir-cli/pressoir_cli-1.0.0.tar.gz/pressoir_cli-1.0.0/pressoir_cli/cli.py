...
def init(repository_path: Path, collection: str = ""):
    """Initialize a new book to `repository_path`.
...
def build(
    ...
    """Build a book from `repository_path`.
...
@cli
def serve(repository_path: Path, port: int = 8000):
    """Serve an HTML book from `repository_path`/public.
