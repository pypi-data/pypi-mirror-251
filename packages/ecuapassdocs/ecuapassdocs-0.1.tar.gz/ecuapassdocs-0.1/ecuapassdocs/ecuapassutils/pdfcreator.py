...
#----------------------------------------------------------------
class CreadorPDF:
#	cartaportePdfPath    = join (settings.STATIC_ROOT, "creador", 
#	                       'images', 'image-cartaporte-vacia-SILOG.pdf')
#	cartaporteImgPath    = join (settings.STATIC_ROOT, 'creador', 
#	                       'images', 'image-cartaporte-vacia-SILOG.png')
#	cartaporteInputsPath = join (settings.STATIC_ROOT, "creador", 
#	                       "scripts", "cartaporte_inputs.json")

	def __init__ (self, docType):
		if docType == "cartaporte":
			self.backgroundPdf = ResourceLoader.loadPdf ("form-cartaporte", 'image-cartaporte-vacia-SILOG.pdf')
			self.backgroundImg = ResourceLoader.loadImage ("form-cartaporte", 'image-cartaporte-vacia-SILOG.png')
			self.inputBounds   = ResourceLoader.loadJson ("form-cartaporte", 'cartaporte_parameters_for_inputs.json')
		else:
	...
	#----------------------------------------------------------------
	def crearCartaportePDF (self, inputValues, pdfType):
		...
		print (">>> crearCartaportePDF: txt00:", inputValues ["txt00"])

		numberCPI   = inputValues ['txt00']
		inputValues ["txt24"] = copyString;
		outPdfPath  = join (tmpPath, f"CPI-{numberCPI}.pdf") 
		outJsonPath = join (tmpPath, f"CPI-{numberCPI}.json") 
...
if __name__ == "__main__":
	...
	#docSize = getPdfSize(backgroundPdfPath)
	#print(f"Doc size: {docSize[0]} x {docSize[1]} pixels")
