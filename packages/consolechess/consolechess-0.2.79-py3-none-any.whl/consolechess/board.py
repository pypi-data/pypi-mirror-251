...
FEN_REPRESENTATIONS: Final[dict[str, tuple[PieceType, Color]]] = {
    value: key for key, value in PLAINTEXT_ABBRS_BY_TYPE_AND_COLOR.items()
}
...
PGN_RESULT_BY_WINNER: Final[dict[Color | None, str]] = {
    value: key for key, value in WINNER_BY_PGN_RESULT.items()
}
...
_FISCHER_RANKS_AND_COLORS: Final[tuple[tuple[int, int, Color], ...]] = (
    (2, 1, "white"),
    (7, 8, "black"),
)
_FISCHER_MAJOR_PIECES: Final = (
    "knight",
    "knight",
    "bishop_1",
    "bishop_2",
...
class ChessBoard:
    ...
    def set_random(self) -> None:
        ...
        for rank, _, color in _FISCHER_RANKS_AND_COLORS:  # Populate pawns.
            for sq in squares_in_rank(rank):
                self[sq] = Piece("pawn", color)
        major_pieces = [*_FISCHER_MAJOR_PIECES]
        while True:
            ...
            a_side_rook, h_side_rook = (
                (rook_1_rank, rook_2_rank)
                if rook_1_rank < rook_2_rank
                else (rook_2_rank, rook_1_rank)
            )
            king_rank = major_pieces.index("king")
            if king_rank > h_side_rook or king_rank < a_side_rook:
                continue
            break
        for _, rank, color in _FISCHER_RANKS_AND_COLORS:  # Populate major pieces.
    ...
    def export_pgn(
        ...
        *,
        ...
        if "Result" in fields_ and not self._status.game_over:
            match fields_["Result"]:
                case "1/2-1/2":
                    self.draw()
                case "1-0" | "0-1" as res:
                    self._status = GameStatus(
    ...
    def can_block_or_capture_check(
        ...
        for check in checks:
            ...
            possible_squares = [
                check,
                *DIRECTION_GENERATORS[rank_direction, file_direction](check),
            ]
