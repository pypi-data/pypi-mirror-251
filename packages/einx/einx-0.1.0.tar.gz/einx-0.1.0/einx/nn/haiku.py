...
class Linear(hk.Module):
    ...
    def __init__(self, expr, bias=True, init_weight=None, init_bias=None, dtype="float32", name=None, **kwargs):
        super().__init__(name=name)
        self.expr = expr
        self.bias = bias
        self.dtype = dtype
    ...
    def __call__(self, x):
        return einx.nn.linear(
            x,
            self.expr,
            bias=param(hk.get_parameter, name="bias", init=self.init_bias) if self.bias else None,
            weight=param(hk.get_parameter, name="weight", init=self.init_weight),
            **self.kwargs,
        )
