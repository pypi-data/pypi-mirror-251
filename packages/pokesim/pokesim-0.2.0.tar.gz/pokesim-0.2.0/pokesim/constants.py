...
class Status(enum.IntEnum):
	...
	@staticmethod
	def colors() -> dict["Status", color.Color]:
		"""
		Returns a dictionary for getting the background color associated with a type
		"""
		return {
			Status.Paralyzed: color.Color(248, 208, 48),
			Status.Poisoned: color.Color(160, 64, 160),
			Status.Burned: color.Color(240, 128, 48),
			Status.Asleep: color.Color(140, 136, 140),
			Status.Frozen: color.Color(152, 216, 216)
		}

	@staticmethod
	def abbrev() -> dict['Status', str]:
		"""
	...
	def __str__(self) -> str:
		...
		if stdout.isatty():
			return color.colorSprintfBckgrnd(Status.abbrev()[self], Status.colors()[self])
