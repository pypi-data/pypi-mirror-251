...
import sys
from typing import (
    Any, Generic, Literal, MutableSequence, Optional, Type, TypeVar, Union, final, overload
)
...
class DescriptorBase(Generic[T]):
    ...
    __slots__ = ("__name", "__attr_name", "__given_attr_name", "__owner", "__type")
    ...
    def __set_name__(self, owner: Type[Any], name: str) -> None:
        ...
        if given_attr_name.startswith("__") and not given_attr_name.endswith("__"):
        ...
        if hasattr(owner, "__slots__") and "__dict__" not in owner.__slots__:
            if given_attr_name not in owner.__slots__:
                raise AttributeError(
                    ('Private' if given_attr_name.startswith('__') else 'Protected')
                    +f" attribute {given_attr_name!r} must be defined in __slots__. "
                    +f"Found {owner.__slots__ = }"
                )
