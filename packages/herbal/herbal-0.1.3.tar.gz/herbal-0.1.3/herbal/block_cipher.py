...
def to_blocks(data: bytes, add_padding: bool = False) -> Iterable[Block64]:
    ...
    if add_padding:
        ...
        raise PaddingError(f"Data length is not a multiple of {size}.")
    uint32_array = (
...
def from_blocks(
    ...
    if remove_padding:
        ...
        padding = data[-data[-1] :]
        if len(set(padding)) != 1 or len(padding) != padding[0]:
            raise PaddingError("Invalid block padding.")
            ...
        return data.removesuffix(padding)
    return data
