...
from resemble.controller.settings import USER_CONTAINER_PORT
...
from respect.logging import ERROR, INFO, get_logger, set_log_level
...
class Application:
    ...
    def __init__(
        ...
        *,
        ...
        if run_environment == RunEnvironment.RSM_DEV:
            # Only when running as part of `rsm dev` do we need to bring up a
            # Resemble cluster instance in-memory.
            #
    ...
    async def _run_rsm_dev(self) -> int:
        ...
        local_envoy_port: int = int(
            os.environ.get(ENVVAR_RSM_DEV_LOCAL_ENVOY_PORT, '9991')
        )
    ...
    async def _run_kubernetes(self) -> int:
        ...
        try:
            ...
            server = Server.create(
                servicers=self._servicers,
                listen_address=f'0.0.0.0:{USER_CONTAINER_PORT}',
                initialize_callback=self._initialize,
            )
    ...
    async def run(self) -> int:
        ...
        try:
            ...
            # Give a helpful error message pointing them to `rsm dev`.
            #
