...
class KubernetesConsensusManager(ConsensusManager):
    ...
    async def _set_consensus(
        ...
        # original ServerDeployment object associated with this consensus. (The
        # owner object must be in the same namespace as the objects being
        ...
        await self._k8s_client.persistent_volume_claims.create_or_update(
            ...
            # TODO(rjh): make this configurable via the ServerDeployment.
            storage_request="1Gi",
        ...
        await self._k8s_client.deployments.create_or_update(
            ...
            service_account_name=APPLICATION_SERVICE_ACCOUNT,
    ...
    async def _delete_consensus(
        ...
        await self._k8s_client.deployments.delete(
            namespace=consensus.k8s_namespace, name=deployment_name
        )
...
class LocalConsensusManager(ConsensusManager):
    ...
    def __del__(self):
        ...
        try:
            ...
            current_event_loop.create_task(shutdown_and_wait())
        except RuntimeError:
    ...
    async def _launch_subprocess_consensus(
        ...
        async def get_port() -> int:
            ...
            if isinstance(port_or_stringified_error, str):
                ...
                raise InputError(
                    reason=
                    f"Consensus '{consensus.consensus_name}' failed to start:"
                    "\n"
                    "\n"
                    f"{stringified_error}",
                )
