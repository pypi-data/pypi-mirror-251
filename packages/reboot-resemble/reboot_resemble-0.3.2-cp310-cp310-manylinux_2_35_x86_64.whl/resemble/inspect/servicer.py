...
from resemble.v1alpha1.inspect.inspect_rsm import (
    GetAllActorsRequest,
...
class InspectServicer(Inspect.Interface):

    # Singleton actor ID.
    #
    # TODO(benh): remove this once we have first-class singleton support.
    SINGLETON_ACTOR_ID: ActorId = '(singleton)'
                ...
                async for actors in self._state_manager.actors():
                    ...
                    for service in watch_actor_tasks:
                        for actor_id in watch_actor_tasks[service]:
                            if not service in actors or actor_id not in actors[
                                service]:
