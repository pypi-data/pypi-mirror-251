...
class ResourceRepository(BaseDatastoreStorage, IResourceRepository[IDatastoreTransaction]):
    ...
    async def exists(self, key: ITyped[ResourceType] | ObjectReference[Any]) -> bool:
        if not isinstance(key, (ObjectReference, PrimaryKey)):
        ...
        if isinstance(key, PrimaryKey):
            ...
            if not key.is_local():
                raise TypeError("Can not lookup non-local keys.")
            q = self.query(
                model=pydantic.BaseModel,
                kind=f'{key.api_group}/{key.kind}',
                filters=filters,
    ...
    async def persist_status(
        self,
        object: StatefulResource[Any, ResourceStatus[Any]],
        key: IDatastoreKey,
        ...
        for condition in object.status.conditions:
            ...
            dao = object.status.storage_model({
                **condition.model_dump(),
                ...
                'uid': await self.allocate_identifier(object.status.ConditionStorageModel)
            })
            ...
            entity = self.entity_factory(
                key=self.entity_key(
                    ...
                    identifier=dao.root.uid, # type: ignore
