...
class ResourceRouter(fastapi.APIRouter, Generic[R]):
    ...
    @classmethod
    def with_model(cls, app: ResourceApplication, model: type[R], **kwargs: Any) -> 'ResourceRouter[R]':
        ...
        for m in models: # type: ignore
            impl = type(f'{m.__name__}Router', (cls,), {
                'resource_model': m
            })
            child = impl()
            child.add_to_application(app, **kwargs)
            router.include_router(router=child)
        return cast(ResourceRouter[model], router)

    def __init__(self, enabled_verbs: set[str] | None = None, **kwargs: Any):
        ...
        self.verbs: list[verbs.Verb[Any, Any]] = []
        ...
        self.add_resource_route(
            ...
            summary=f'Get the permissions granted on {model.__name__}',
            description=(
        ...
        self.add_api_route(
            ...
            summary=f'Replace an existing {model.__name__}',
    ...
    def add_to_application(self, app: ResourceApplication):
        if not app.openapi_tags:
            app.openapi_tags = []
        for verb in self.verbs:
            verb.add_to_router(self, app.register_resource)
    ...
    def verb(
        ...
        def f(func: Callable[..., Any]) -> Callable[..., Any]:
            ...
            if any([v.verb == impl.verb for v in self.verbs]):
                raise ValueError(f"Verb '{impl.verb}' is already declared.")
            self.verbs.append(impl(self.resource_model, permissions=permissions)) # type: ignore
            self.verbs_enabled.add(impl.verb)
            return func
        return f
