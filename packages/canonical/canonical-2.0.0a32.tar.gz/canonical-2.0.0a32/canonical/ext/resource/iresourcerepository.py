...
class IResourceRepository(IStorage[ResourceType], Protocol):
    ...
    async def import_documents(
        ...
        async with self.transaction() as tx:
            for doc in documents:
                resource = adapter.validate_python(doc)
                await self.persist(cast(Resource[Any], resource), transaction=tx)
                yield resource
...
class TypedResourceRepository(Generic[R]):
    ...
    def __init__(self, repo: IResourceRepository, model: type[R]):
        self.model = model
        self.repo = repo
