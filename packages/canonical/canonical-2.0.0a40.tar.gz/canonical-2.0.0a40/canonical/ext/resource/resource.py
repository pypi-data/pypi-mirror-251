...
class Resource(pydantic.BaseModel, Generic[M]):
    _storage: IStorage[Self, Any] | None = pydantic.PrivateAttr(default=None)
    ...
    def get_references(self) -> list[ObjectReferenceModel[Any]]:
        """Return the references this instance holds to other resources."""
        refs: list[ObjectReferenceModel[Any]] = []
        for name in self.model_fields:
            refs.extend(self._get_references(getattr(self, name)))
        return refs
    ...
    async def persist_key(
        self,
        key: Any,
        storage: IStorage[Self, Any] | None = None,
        ...
        try:
            old = cast(Self, await storage.get(key))
    ...
    def _get_references(self, data: Mapping[Any, Any] | Collection[Any]) -> list[ObjectReferenceModel[Any]]:
        refs: list[ObjectReferenceModel[Any]] = []
        if not isinstance(data, (Mapping, Collection)):
            return []
        if isinstance(data, Mapping):
            items = cast(list[Any], data.values())
        else:
            items = list(data)
        for v in items:
            if v is None:
            ...
            if isinstance(v, (bytes, str)):
                continue
            if isinstance(v, ObjectReferenceModel):
                refs.append(cast(ObjectReferenceModel[Any], v))
                continue
            if isinstance(v, pydantic.BaseModel):
                for name in v.model_fields:
                    refs.extend(self._get_references(getattr(v, name)))
                continue
            if isinstance(v, (Collection, Mapping)):
                refs.extend(self._get_references(cast(Any, v)))
                continue
        return refs