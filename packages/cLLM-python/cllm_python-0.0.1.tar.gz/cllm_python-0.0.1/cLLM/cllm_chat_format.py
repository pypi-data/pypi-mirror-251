...
import cLLM.cllm as cllm
import cLLM.cllm_types as cllm_types
import cLLM.cllm_grammar as cllm_grammar
...
class LlamaChatCompletionHandler(Protocol):
    def __call__(
            ...
            *,
            cllm: cllm.cLLM,
            messages: List[cllm_types.ChatCompletionRequestMessage],
            functions: Optional[List[cllm_types.ChatCompletionFunction]] = None,
            function_call: Optional[cllm_types.ChatCompletionRequestFunctionCall] = None,
            tools: Optional[List[cllm_types.ChatCompletionTool]] = None,
            tool_choice: Optional[cllm_types.ChatCompletionToolChoiceOption] = None,
            ...
            response_format: Optional[
                cllm_types.ChatCompletionRequestResponseFormat
            ] = None,
            ...
            logits_processor: Optional[cllm.LogitsProcessorList] = None,
            grammar: Optional[cllm.CLLMGrammar] = None,
            logit_bias: Optional[Dict[str, float]] = None,
            **kwargs,  # type: ignore
    ) -> Union[
        cllm_types.CreateChatCompletionResponse,
        Iterator[cllm_types.CreateChatCompletionStreamResponse],
    ]:
        ...
...
def _map_roles(
        messages: List[cllm_types.ChatCompletionRequestMessage], role_map: Dict[str, str]
...
def _convert_text_completion_to_chat(
        completion: cllm_types.Completion,
) -> cllm_types.ChatCompletion:
    return {
...
def _convert_text_completion_chunks_to_chat(
        chunks: Iterator[cllm_types.CreateCompletionStreamResponse],
) -> Iterator[cllm_types.ChatCompletionChunk]:
...
def _convert_completion_to_chat(
        completion_or_chunks: Union[
            cllm_types.CreateCompletionResponse,
            Iterator[cllm_types.CreateCompletionStreamResponse],
        ],
        stream: bool = False,
) -> Union[
    cllm_types.CreateChatCompletionResponse, Iterator[cllm_types.ChatCompletionChunk]
]:
    if stream:
        chunks: Iterator[cllm_types.CreateCompletionStreamResponse] = completion_or_chunks  # type: ignore
        return _convert_text_completion_chunks_to_chat(chunks)
    else:
        completion: cllm_types.Completion = completion_or_chunks  # type: ignore
...
def register_chat_format(name: str):
    def decorator(f: ChatFormatter):
        def basic_create_chat_completion(
                *,
                cllm_model: cllm.cLLM,
                messages: List[cllm_types.ChatCompletionRequestMessage],
                functions: Optional[List[cllm_types.ChatCompletionFunction]] = None,
                function_call: Optional[
                    cllm_types.ChatCompletionRequestFunctionCall
                ] = None,
                tools: Optional[List[cllm_types.ChatCompletionTool]] = None,
                tool_choice: Optional[cllm_types.ChatCompletionToolChoiceOption] = None,
                temperature: float = 0.2,
            ...
            if response_format is not None and response_format["type"] == "json_object":
                grammar = cllm_grammar.CLLMGrammar.from_string(
                    cllm_grammar.JSON_GBNF
                )

            completion_or_chunks = cllm_model.create_completion(
                prompt=prompt,
...
def functionary_chat_handler(
        ...
        stream: bool = False,
        stop: Optional[Union[str, List[str]]] = [],
        response_format: Optional[cllm_types.ChatCompletionRequestResponseFormat] = None,
        max_tokens: Optional[int] = None,
        presence_penalty: float = 0.0,
        frequency_penalty: float = 0.0,
        repeat_penalty: float = 1.1,
        ...
        grammar: Optional[cllm.CLLMGrammar] = None,
        **kwargs,
) -> Union[cllm_types.ChatCompletion, Iterator[cllm_types.ChatCompletionChunk]]:
    default_system_message = (
        "A chat between a curious user and an artificial intelligence assistant."
        " The assistant gives helpful, detailed, and polite answers to the user's"
        " questions. The assistant calls functions with appropriate input when necessary"
    )

    def generate_type_definition(
            param: Dict[str, cllm_types.JsonType], indent_level: int, shared_defs
    ) -> str:
        indent = "  " * indent_level
        if "$ref" in param:
    ...
    def prepare_messages_for_inference(
            messages: List[cllm_types.ChatCompletionRequestMessage],
            functions: Optional[List[cllm_types.ChatCompletionFunctions]] = None,
            tools: Optional[List[cllm_types.ChatCompletionTool]] = None,
    ):
        all_messages: List[cllm_types.ChatCompletionRequestMessage] = []
        if functions is not None:
            all_messages.append(
                cllm_types.ChatCompletionRequestSystemMessage(
        ...
        all_messages.append(
            cllm_types.ChatCompletionRequestAssistantMessage(
                role="assistant", content=None
            )
        )

        def message_to_str(msg: cllm_types.ChatCompletionRequestMessage):
            if msg["role"] == "system":
                return f"system:\n{msg['content']}\n"
    ...
    if function_call is None and (functions is None or len(functions) == 0):
        completion_or_completion_chunks = cllm_model.create_completion(
            prompt=prompt + ":\n",
            temperature=temperature,
            top_p=top_p,
            top_k=top_k,
    ...
    if function_call is None or (
        ...
        stop = "\n"
        completion: cllm_types.Completion = cllm_model.create_completion(
    ...
    if function_body is not None:
        try:
            with suppress_stdout_stderr(disable=cllm_model.verbose):
                grammar_text = cllm_grammar.json_schema_to_gbnf(
                    json.dumps(function_body)
                )
                grammar = cllm_grammar.CLLMGrammar.from_string(
                    cllm_grammar.json_schema_to_gbnf(json.dumps(function_body))
                )
                print(grammar_text)
        except Exception as e:
            if cllm_model.verbose:
                print(
        ...
        with suppress_stdout_stderr(disable=cllm_model.verbose):
            grammar = cllm_grammar.CLLMGrammar.from_string(cllm_grammar.JSON_GBNF)
    ...
    return cllm_types.CreateChatCompletionResponse(
        id="chat" + completion["id"],
        object="chat.completion",
        created=completion["created"],
        model=completion["model"],
...
class Llava15ChatHandler:
    ...
    def __init__(self, clip_checkpoint_path: str, verbose: bool = False):
        import cLLM.llava_cpp as llava_cpp
        ...
        with suppress_stdout_stderr(disable=self.verbose):
            self.clip_ctx = self._llava_cpp.clip_model_load(
                self.clip_checkpoint_path.encode(), 0
            )
    ...
    def __call__(
        ...
        assert (
                cllm_model.context_params.logits_all is True
        ...
        assistant_role = "\nASSISTANT:"
        cllm_model.reset()
        cllm_model.eval(cllm_model.tokenize(system_prompt.encode("utf8"), add_bos=True))
        for message in messages:
            if message["role"] == "user" and message["content"] is not None:
                if isinstance(message["content"], str):
                    ...
                    cllm_model.eval(
                        cllm_model.tokenize(f"{user_role} ".encode("utf8"), add_bos=False)
                    )
                            ...
                            with suppress_stdout_stderr(disable=self.verbose):
                                embed = (
                                    self._llava_cpp.llava_image_embed_make_with_bytes(
                                        ...
                                        n_threads=cllm_model.context_params.n_threads,
                                        image_bytes=c_ubyte_ptr,
                                        image_bytes_length=len(image_bytes),
                                    )
                                )
                            try:
                                n_past = ctypes.c_int(cllm_model.n_tokens)
                                n_past_p = ctypes.pointer(n_past)
                                with suppress_stdout_stderr(disable=self.verbose):
                                    self._llava_cpp.llava_eval_image_embed(
                                        ctx_cllm=cllm_model.ctx,
                                        embed=embed,
                                        n_batch=cllm_model.n_batch,
                                        n_past=n_past_p,

                                    )
                                assert cllm_model.n_ctx() >= n_past.value
                                cllm_model.n_tokens = n_past.value
                            finally:
                                with suppress_stdout_stderr(disable=self.verbose):
            ...
            if message["role"] == "assistant" and message["content"] is not None:
                ...
                assert cllm_model.n_ctx() >= cllm_model.n_tokens
        cllm_model.eval(cllm_model.tokenize(f"{assistant_role}".encode("utf8"), add_bos=False))
        assert cllm_model.n_ctx() >= cllm_model.n_tokens

        prompt = cllm_model.input_ids[: cllm_model.n_tokens].tolist()
