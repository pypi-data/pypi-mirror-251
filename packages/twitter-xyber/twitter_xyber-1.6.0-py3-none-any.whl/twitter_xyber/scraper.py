    ...
    def timeline_tweet(self, username: str, tweet_count: int = 80) -> dict:
        ...
        with sync_playwright() as pw:
            ...
            while _scroll_count < _max_scrolls:
                ...
                for tr in tweet_result:
                    try:
                        if any("tweet" in s for s in tr['entryId'].split("-")):
                            ...
                            tweet_time = datetime.strptime(legacy['created_at'], "%a %b %d %H:%M:%S %z %Y")
                            day = tweet_time.strftime("%A")
                            hour = tweet_time.strftime("%H")

                            heatmap_data.setdefault(day, {}).setdefault(hour, 0)
                ...
                heatmap_result = [{"day": day, "hour": hour, "count": count} for day, hours in heatmap_data.items() for hour, count in hours.items()]

            context.close()
            browser.close()
            # return timeline
            return {"timelines": timeline, "hashtags": hashtags_result, "mentions": mentions_result, "source":source_result, "heatmap":heatmap_result}
