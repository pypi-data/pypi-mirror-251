...
class HeadlessPreviewMixin:
    ...
    def get_preview_url(self, request, token):
        _, _, relative_page_url = self.get_url_parts(request)
        querystring = urlencode({"content_type": self.get_content_type_str(), "token": token})
        return (
            f"{self.get_client_root_url()}{relative_page_url}?{querystring}"
        )
