...
from .types import (
    NO_FEEDBACK, BASE_FILTER_OUT_GLOBALS, Feedback, NoFeedback, FeedbackType,
    FeedbackPiece, FeedbackMessage
)
from .config import (
    Config, PrimedFeedbackMakerF, ExistFeedbackMakerInput,
...
class FeedbackAssertionError(SystemExit):  # pylint: disable=missing-class-docstring
    def __init__(self, feedback: Feedback) -> None:
        ...
        if isinstance(feedback, NoFeedback):
            super().__init__("No feedback provided")
        else:
...
class Asserter:  # pylint: disable=missing-class-docstring
    def __init__(self, config: Config = Config()) -> None:
        ...
        * Define custom feedback makers in the
          :class:`cg_feedback_helpers.config.Config` when instantiating
          the :class:`Asserter`.

        :param config: The configuration overrides. Check
            :class:`cg_feedback_helpers.config.Config`.
        """
        self._config = config
    ...
    def _open_output(self) -> t.Tuple[subprocess.Popen | None, t.IO[bytes]]:
        if self._config.is_atv2 or 'CG_FEEDBACK_HELPERS_TRUNCATE' in os.environ:
    ...
    def _fail(
        ...
        self._maybe_write_feedback(feedback, FeedbackType.NEGATIVE)
        raise FeedbackAssertionError(feedback)
    ...
    def emit_success(self, *, feedback: t.Optional[Feedback] = None) -> None:
        ...
        """
        self._maybe_write_feedback(
            feedback if feedback is not None else self._config.success_message,
    ...
    def variable_exists(
        ...
        *,
        ...
        :param negative_feedback: Overrides the feedback the user receives if
            the assertion fails.
        """
        inp = ExistFeedbackMakerInput("variable", repr(varname))
        self._assert(
    ...
    def is_set(
        ...
        *,
        ...
        :param negative_feedback: Overrides the feedback the user receives if
            the assertion fails.
        """
        inp = ExistFeedbackMakerInput("key", repr(key))
        self._assert(
            key in dictionary,
            self._config.exist_feedback_maker.get_feedback_maker(inp),
            positive_feedback,
    ...
    def file_exists(
        ...
        *,
        ...
        :param negative_feedback: Overrides the feedback the user receives if
            the assertion fails.
        """
        inp = ExistFeedbackMakerInput("file", file_path)
        self._assert(
