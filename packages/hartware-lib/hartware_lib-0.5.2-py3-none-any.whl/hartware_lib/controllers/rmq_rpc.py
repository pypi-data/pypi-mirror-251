...
class RpcProbe:
    ...
    async def handle_message(
        ...
        if not func and not property:
            # if V: print("should have func or property specified")
        ...
        try:
            if func:
                ...
                if asyncio.iscoroutinefunction(func):
                    ...
                    # print(f"Async Func {func}({args}, {kwargs})")
                else:
                ...
                if "property_set" in message:
                    ...
                    # print(f"Property {property} set to: {property_set}")
                else:
                    result = getattr(self.subject, property)

                    # print(f"Property {property} : {result}")
        except Exception as exc:
            await self.rpc_out.publish({"error": str(exc)})
        ...
        # if V: print("====", self.__class__.__name__, result)
...
class RpcCaller:
    ...
    async def _store_result(
        ...
        # if V: print("_store_result", result)
        self._result = result
    ...
    async def _process(self, request: Dict[str, Any]) -> Any:
        ...
        async with self.command_lock:
            # if V: print("=====", self.__class__.__name__, request)
