...
export const menu: JupyterFrontEndPlugin<void> = {
  ...
  activate: async (app: JupyterFrontEnd, mainMenu: IMainMenu, notebookTracker: INotebookTracker, docManager: IDocumentManager) => {
    ...
    // add try catch for when creating menu
    let menu: Menu;
    try {
      menu = new Menu({ commands });
      console.log('Created menu in skillsnetwork-authoring-extension menu plugin!');
    } catch (e) {
      console.log(e);
      return;
    }
    ...
    // Add command to menu
    try {
      menu.addItem({ command: editLabFromToken, args: {} });
      console.log('Added command to menu in skillsnetwork-authoring-extension menu plugin!');
    }
    catch (e) {
      console.log("error happened in extensions", e);
      return;
    }
