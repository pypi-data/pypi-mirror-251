...
class Itsperfect:
    ...
    def __add_item_id(self, base_url: str, item_id: str)-> str:
        """Add item_id to the url if it is not empty and numeric
        ...
        """
        if item_id != "" and not item_id.isnumeric():
            logging.error("Item ID must be numeric.")
            raise ItsperfectException()

        return f"{base_url}/{item_id}" if item_id != "" and item_id.isnumeric() else base_url

    def __add_path_extension(self, base_url: str, endpoint: dict)-> str:
