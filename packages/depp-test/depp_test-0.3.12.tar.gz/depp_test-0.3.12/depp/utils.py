...
def distance_between(node1, node2):
    d1 = {node1: 0}
    current_node = node1.parent
    pre_node = node1
    while current_node is not None:
        d1[current_node] = d1[pre_node] + pre_node.edge_length
    ...
    d2 = {node2: 0}
    while current_node is not None:
        d2[current_node] = d2[pre_node] + pre_node.edge_length
        if current_node in d1:
            return d1[current_node] + d2[current_node]
        pre_node = current_node
        current_node = current_node.parent
...
def combine_identical_seq_intree(tree, seq, thr=None, outdir=None, cluster=None, replicate=False):
    ...
    for node in best_node_group:
        ...
        for lca in lca_nodes:
            edge_length = np.mean([distance_between(name_to_node[i], node) for i in best_node_group[node] if LCA_node_name[i] == lca])
            new_node = ts.Node(edge_length=edge_length, label=lca)
