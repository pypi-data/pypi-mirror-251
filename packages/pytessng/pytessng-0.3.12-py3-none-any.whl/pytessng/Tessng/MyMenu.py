...
class Tools():
    ...
    def network_import(widget, params):
        ...
        try:
            message = other2tess(netiface, params, widget.mode)
            # 为 osm 可能没数据特地加了一个 message 参数
            if message:
                info, mode = message, "info"
            else:
                info, mode = "导入成功", "info"
        except:
...
class MyMenu(QMenu):
    def __init__(self, *args, extension=False):
        super().__init__(*args)
        # 文件路径
        self.instruction_path = os.path.abspath(Path(__file__).resolve().parent / ".." / "Files" / "Doc" / "pytessng说明书.pdf")
    ...
    def init(self):
        ...
        # 4.轨迹导出
        self.action_trajectory_export = self.addAction('轨迹数据导出')
...
class NetworkImportOpenstreetmap(QWidget):
    ...
    def create_network_online(lon_1, lat_1, lon_2, lat_2, parseLevel):
        ...
        # 构建空类
        class Widget: mode, close = "osm", lambda self: None
        # 执行创建
