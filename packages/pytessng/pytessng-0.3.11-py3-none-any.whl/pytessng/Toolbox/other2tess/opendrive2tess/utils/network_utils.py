...
class Network:
    ...
    def create_network(self, tess_lane_types, netiface=None):
        ...
        # move
        if is_center:
            x_move = round(self.xy_move[0], 3)
            y_move = round(self.xy_move[1], 3)
        else:
            x_move, y_move = 0, 0
        other_info = {"move_distance": {"x_move": float(x_move), "y_move": float(y_move)}}
        # 记录属性，如：路网来源
        netiface.setNetAttrs("OpenDrive Network", otherAttrsJson=other_info)
        print("Network attrs: ", netiface.netAttrs().otherAttrs())
        # 设置场景显示区域
