...
from feed2exec.model import (
    Feed,
    FeedConfStorage,
    FeedContentCacheStorage,
    FeedItemCacheStorage,
)
...
class FeedManager(object):
    ...
    on initialization, a new :class:`requests.Session` object is
    ...
    def sessionConfig(self, cache=True):
        """our custom session configuration

        we change the user agent and set the file:// handler. extra
        ...
        if self.db_path is None or cachecontrol is None:
            return
        if not cache:
            # reset the session adapters to *not* use an eventual cache controller
            # from requests.Session.__init__()
