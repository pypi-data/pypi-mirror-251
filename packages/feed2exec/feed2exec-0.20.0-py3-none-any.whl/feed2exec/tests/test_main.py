...
from feed2exec.tests.test_feeds import test_nasa, test_sample
...
def test_no_maildir(tmpdir, monkeypatch):
    ...
    1. ~/Maildir (possibly, if not overridden) gets created by mistake

    2. the `mailbox` setting gets created even if overridden by an
