...
class LLM(torch.nn.Module):
    def __init__(self,
                 ...
                 system_message: Union[str, TextTensor] = "You are an expert, who answers only with the requested texts. Keep it short.",
    ...
    def forward(self, input: TextTensor):
        ...
        for i, m in enumerate(input):
            ...
            if np.all([key in ["user", "assistant"] for key in m.keys()]):
                input[i] = m.items()
            elif "assistant" not in m.keys():
                # NOT STRICT
                input[i] = [("user", str(m))]
                logging.debug(
                    f"A text without 'assistant' keys was passed to OpenAI Chat. Assuming the whole Text is a user message: = '{str(m)[:25]}'... ")
            else:
                raise ValueError(
        ...
        result = chat(input, system_messages, model=self.model, cache=self.cache, as_str=True, **self.kwargs)
