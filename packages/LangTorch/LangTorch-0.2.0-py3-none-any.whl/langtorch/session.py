...
class Session(metaclass=SingletonMeta):
    ...
    def request(self, provider, type, request_strings, cache=True):
        ...
        if cache:
            ids, request_strings =[id for id in ids if not id in self._responses], [request for id, request in zip(ids, request_strings) if not id in self._responses]
    ...
    def _append_chat_to_cache(self, key, texts):
        with h5py.File(self._cache_file, 'a') as f:
            if key in f:
                del f[key]  # Delete the existing dataset
    ...
    def get_responses(self, provider, type, job_id=-1):
        self.reload()
        return self.get_job(job_id, type, provider)
    ...
    def completions(self, type, provider, request_strings):
        ...
        if type == "embeddings":
            ...
            return TextTensor(responses).set_key_("assistant")
