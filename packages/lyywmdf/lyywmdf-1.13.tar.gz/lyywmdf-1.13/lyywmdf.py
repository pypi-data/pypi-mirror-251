...
def calc_lastdate_kline_number(code, last_date_dict, debug=False):
    ...
    if code in last_date_dict.keys():
        ...
        if debug: print(f"code={code} not in last_date_dict.keys()")
...
def wmdf(api, stk_code_num, to_down_kline, server_ip=None, debug=False) -> pd.DataFrame:
    ...
    if debug:
        t0 = datetime.now()
    try:
        df = 通达信下载原始分钟K线(api, stk_code_num, to_down_kline, server_ip=server_ip, debug=debug)
        if df.empty:
            raise Exception("通达信下载原始分钟K线 error: DataFrame must not be empty")
    except Exception as e:
        print("Fuction: wmdf, try to run 通达信下载原始分钟线 error。stk_code_num:", stk_code_num, "to_down_kline:", to_down_kline, "api:", api, e)
...
def 通达信下载原始分钟K线(api, 股票数字代码, 要下载的K线数量, server_ip="120.79.210.76", debug=False) -> pd.DataFrame:
    """
    通达信下载原始分钟K线(tdxserverip:str, 股票数字代码:str, 开始日期:str, 结束日期, debug=False)
    """
    ...
    if debug: print("市场代码=", 市场代码, "，股票数字代码=", 股票数字代码, ",", k_type['15min'], ",要下载的K线数量=", 要下载的K线数量)
    # api = TdxHq_API()
    # tdxserverip = "120.79.210.76"
    # api.connect(tdxserverip, 7709)
    # 股票数字代码="873833"

    df_tdx = api.to_df(api.get_security_bars(k_type['15min'], 市场代码, 股票数字代码, 0, 要下载的K线数量))
