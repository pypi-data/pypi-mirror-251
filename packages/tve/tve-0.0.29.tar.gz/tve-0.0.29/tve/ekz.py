...
def q3(num = 0):
    try:
        if num == 0:
                image_path_5 = os.path.join(main_path, 'all_5.png')
                img5 = Image.open(image_path_5)
                display(img5)
        if num != 0:
                # Определяем путь к изображению
                image_path_1 = os.path.join(main_path, f"usl_list_{num}.png")
    ...
    n1, n2
    ____
    nui = np.array(list(len(pi[pi == i]) for i in range(10)))
    nui
    ____
    mui = np.array(list(len(e[e == i]) for i in range(10)))
    mui
    ____             
    chi2 = n1*n2*sum((nui/n1 - mui/n2)**2 / (nui + mui))
    K = sts.chi2(l-1).isf(alpha)
    chi2, K
    ''')
    ...
    plt.grid(color='DarkTurquoise', alpha=0.75, linestyle=':', linewidth=2)
    ########################################
    plt.xlim(0,110)
    #plt.ylim(P-0.001,P+0.001)
    #########################################
    x=np.linspace(0,110,10000)
    y = np.vectorize(Fecdf0, otypes=[float])#возвращает ссылку на "векторизированную" функцию. 
    y1= X.cdf(x)
    plt.plot(x, y(x),linewidth = 5,color='r')
    plt.plot(x, y1,linewidth = 3,color='g')
    plt.show()
                
    def f(x):
        return -abs(y(x)-X.cdf(x))
                
    res=minimize_scalar(f,bounds=(58, 60), method='bounded')
    res
    ...
    y0=np.array([0]*len(x1))
    #########################################
    x=np.linspace(0,110,10000)
    y = np.vectorize(Fecdf0, otypes=[float])#возвращает ссылку на "векторизированную" функцию. 
    y1 = np.vectorize(X.cdf, otypes=[float])
    #y1= X.cdf(x)
    plt.plot(x, y(x),linewidth = 5,color='r')
    plt.plot(x, y1(x),linewidth = 5,color='g')
    plt.plot(x,abs(y1(x)-y(x)))
    plt.plot(xmax,ymax, marker='o', mfc='red', mec='black', ms=20, mew=2)
    plt.plot(xmax,X.cdf(xmax), marker='o', mfc='red', mec='black', ms=20, mew=2)
    plt.plot(xmax,Fecdf0(xmax), marker='o', mfc='red', mec='black', ms=20, mew=2)
    plt.plot(x,y0,color='black')
    plt.text(xmax+1,ymax+0.01, r'$0,\!\!041$',fontsize=50,color='r')
    plt.text(xmax,X.cdf(xmax)-0.1, r'$0,\!\!599$',fontsize=50,color='r')
    plt.text(xmax-1,Fecdf0(xmax)+0.05, r'$0,\!\!64$',fontsize=50,color='r')
    plt.show()
    fig.savefig("Sol_33_Exam.pdf", bbox_inches='tight')
    ...
    # Cпособ 1
    F1 = np.append([0], sts.ecdf(pi).cdf.probabilities)
    F2 = sts.ecdf(e).cdf.probabilities
    D = max(abs(F1 - F2)) * np.sqrt((n1*n2)/(n1 + n2))
    ...
    ''')
        ...
        if num == 35:
            print('''
    obs = np.array([[60, 54, 46, 41],
                [40, 44, 53, 57]])
    chi2_obs = sts.chi2_contingency(obs).statistic
    pv = sts.chi2_contingency(obs).pvalue
    k = 2
    m = 4
    alpha = 0.05
    alpha2 = 0.025
    chi2_cr = sts.chi2((k-1)*(m-1)).isf(alpha)
    chi2_cr2 = sts.chi2((k-1)*(m-1)).isf(alpha2)
    chi2_obs, chi2_cr, chi2_cr2, pv
