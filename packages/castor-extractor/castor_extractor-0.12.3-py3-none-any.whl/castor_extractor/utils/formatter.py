...
def _remove_unsupported_byte(row: List[ScalarValue]) -> List[ScalarValue]:
    return [
...
def to_csv(buffer: IO[str], data: Iterable[dict]) -> bool:
    ...
    for row in data:
        ...
        try:
            ...
            writer.writerow(_remove_unsupported_byte(converted))
            logger.info("Removed unsupported byte to write to csv")
    return True
