...
class lmc_linked_model():
    ...
    def log_state(self):
        """
        这个函数是`lmc_linked_model`类的一个方法，用于记录当前`llm`对象的状态信息。
        
        函数首先输出一条`Summary:`的日志，然后遍历`llm`对象的状态`state()`，并将每个状态信息以`name: value`的形式记录到日志中。
    ...
    def __call__(self, prompt=None, **kwargs):
        ...
        for time_idx in range(self.invoke_times):
            ...
            for retry_id in range(self.retry_count):
                log(f"invoke time:{time_idx}, retry:{retry_id}")
                ...
                if len(self.exception_func_list) == 0:
                    ...
                    log(f"invoke time:{time_idx}, retry:{retry_id} failed.")
                    continue

                log(f"invoke time:{time_idx}, retry:{retry_id} failed, run fix...")
