...
class Subscription(RegistrableVoucher, PartnerRelated, SalesPrintable,
    ...
    def compute_summary_values(self):
        ...
        # get the last invoiced subscription period
        qs = SubscriptionPeriod.objects.filter(master=self, must_invoice=True)
        last_isp = qs.order_by('end_date').last()
        if last_isp is not None:
            # When there has been at least one invoice, Lino opens a new
            # period if needed and otherwise does nothing.
            start_date = last_isp.end_date + ONE_DAY
        ...
        while start_date - renew_before < dd.today():
            ...
            if not SubscriptionPeriod.objects.filter(**kw).exists():
...
class SubscriptionPeriod(DateRange, InvoiceGenerator):
    ...
    def get_invoice_items(self, info, invoice, ar):
        # dd.logger.info("20181116a %s", self)
        # print("20230630 get_invoice_items()", self.master.items.all())
