...
class MsgProgressPublisher(MsgPublisher):
    ...
    async def update(
        ...
        *,
        ...
        refresh: bool = True,
        when: Optional[datetime|float] = None
    ) -> None:
        ...
        Args:
            ...
            when (datetime|float, optional): Time when the task must reach the desired `completed` value.
                If it is a float, it represents the number of seconds from now.
                If it is a datetime, should be utc.
                On the client side, task will automatically increment the progress towards the `completed` value.

        Raises:
            ValueError: If the `when` parameter is in the past or not accompanied by the `completed` parameter`
        """
        ...
        if when is not None:
            if completed is None:
                raise ValueError("`when` have meaning only when `completed` is also specified")
            if isinstance(when, float):
                task.when = datetime.now(timezone.utc) + timedelta(seconds=when)
            else:
                task.when = when
            if datetime.now(timezone.utc) > task.when:
        ...
        if (
            ...
            if task.when is not None: # setting future finishing itme as we already know it
                task.finished_time = task.when
            else:
