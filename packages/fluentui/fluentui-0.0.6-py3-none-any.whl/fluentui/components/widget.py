...
class GridPos:
    __slots__ = ['row', 'column', 'row_span', 'column_span', 'align']

    def __init__(self, row=-1, column=0, row_span=-1, column_span=-1, align=Qt.AlignmentFlag(0)):
        self.row = row  # 在 grid 中的行
        self.column = column  # 在 grid 中的列
        self.row_span = row_span  # 在 grid 中的行跨度
        self.column_span = column_span  # 在 grid 中的列跨度
        self.align = align  # 在布局中的对齐方式
...
class WidgetMix:
    ...
    def __init__(self: QWidget | Self, *args,
        ...
        for x in slices.concat(attributes, [
