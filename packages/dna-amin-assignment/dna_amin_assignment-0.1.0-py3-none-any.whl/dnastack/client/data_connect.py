...
class InvalidDataLoadingError(RuntimeError):
    ...
    def load(self) -> List[Dict[str, Any]]:
        ...
        with self._http_session as session:
            try:
                ...
                reason = f'HTTP {status_code}: {feedback or ""} ({len(feedback)} B)'
                if status_code == 400:
                    if not self._current_url:
                        if self.__query:
                            raise InvalidQueryError(f'{self.__query}\n\nas the server responded because {reason}') from e
                        else:
                            raise InvalidDataLoadingError(f'{self._initial_url} (init) because {reason}') from e
                    else:
                        raise InvalidDataLoadingError(f'{self._current_url} (follow-up) because {reason}') from e
                else:
                    raise UnexpectedRequestError(reason) from e
            except HttpError as e:
                status_code = e.response.status_code
