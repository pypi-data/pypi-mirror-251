...
""",
    ...
    File(
        description="Aria network manager connection",
        platforms=["Linux"],
        name="aria",
...
def remediate(message: str, action: str, remediation: Callable):
    ...
    if ans == "" or ans == "y":
        try:
            ...
            failure(
                f"Failed to {action}. Please check https://facebookresearch.github.io/projectaria_tools/docs/ARK/sdk/troubleshoot"
            )
...
def check_python_version():
    ...
    if ver.minor == 8 and ver.micro < 10:
        failure(
            "Python versions below 3.8.10 are not supported. Please update your python version."
        )
...
def check_macos_network():
    ...
    if ARIA_INTERFACE not in interfaces:
        warning(
    ...
    if interfaces[-1] != ARIA_INTERFACE:
        remediate(
