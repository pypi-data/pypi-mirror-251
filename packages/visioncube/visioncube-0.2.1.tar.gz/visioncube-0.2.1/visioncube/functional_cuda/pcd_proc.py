...
def correct_depth_by_xy_plane_cuda(
    ...
    y = torch.randint(0, height - 1, (num_samples,), dtype=torch.long)
    x = torch.randint(0, width - 1, (num_samples,), dtype=torch.long)
    z = image[y, x]
    yx1 = torch.stack([y, x, torch.ones((num_samples,), dtype=image.dtype)], dim=-1)
    ...
    yx1 = torch.stack([grid_x.permute(1, 0), grid_y.permute(1, 0), torch.ones_like(image)], dim=-1)
    plane = yx1 @ m
