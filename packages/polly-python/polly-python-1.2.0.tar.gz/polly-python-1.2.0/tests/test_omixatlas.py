...
def test_add_dataset_all_edge_cases_of_ext_and_names(
    mocker, list_folders_function_mock_fixture
):
    ...
    res = omix_obj.add_datasets(repo_id, source_folder_path)
...
def test_update_dataset_new_data_file_without_metadata_file():
    ...
    file_mapping = {
        "PRAD_CPCG_2017_Mutation_TCGA-2A-A8VL-01_new.gct": "PRAD_CPCG_2017_Mutation_TCGA-2A-A8VL-01"
    }
    with pytest.warns(Warning) as record:
        omix_obj.update_datasets(repo_id, source_folder_path, file_mapping=file_mapping)
        if not record:
            pytest.fail("Expected a warning!")
