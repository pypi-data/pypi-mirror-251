...
from acomms.messageparams import NavPingDigitalTransponder, NavPingGenericTransponder, NavStatistics, NavTurnTimes
...
try:
    from acomms.REMUSnav import dtcodes
except ImportError:
    pass
...
class Micromodem(object):
    def __init__(self, name='modem', unified_log=None, log_path=None, log_level='INFO'):
        ...
        self.packetrx_listeners = []
        self.fdp_listeners = []
        self.cst_listeners = []
        self.xst_listeners = []
        self.ack_listeners = []
        self.ping_listeners = []
        self.ping_reply_listeners = []
        self.snpdt_listeners = []
        self.snpgt_listeners = []
        self.snnst_listeners = []
        self.sntta_listeners = []
        self.mpr_listeners = [] #TODO: implement listener
        self.camua_listeners = []
        self.causb2_listeners = []
        self.casst_listeners = []
        self.catjn_listeners = []
        self.cajrx_listeners = []
        self.carev_listeners = []

        self.incoming_dataframe_queues = []
        self.packetrx_queues = []
        self.incoming_all_packet_queues = []
        self.incoming_fdp_queues = []
        self.incoming_cst_queues = []
        self.incoming_xst_queues = []
        self.incoming_msg_queues = []
        self.incoming_mpr_queues = [] #TODO: implement queue
        self.incoming_causb2_queues = []
        self.incoming_casst_queues = []
        self.incoming_catjn_queues = []
        self.incoming_cajrx_queues = []
        self.incoming_snnst_queues = []
        self.incoming_snpdt_queues = []
        self.incoming_snpgt_queues = []
        self.incoming_sntta_queues = []
        self.incoming_ping_queues = []
        self.incoming_ping_reply_queues = []
        ...
        self.current_fdpacket = None
        self.current_legacy_packet = None
        self.current_janus_packet: FDPacket = None
        ...
        # Use this to work around 7-bit/8-bit addressing issues
        self._ignore_cacyc_address_mismatch = False
    ...
    def version_a_at_least_b(self, verstr_a, verstr_b):
        verstr_regex = re.compile(
            "v?(?P<verstr>(?P<major>\d+).(?P<minor>\d+).(?P<patch>\d+))(-rc(?P<rc>\d+))?(-(?P<ncommits>\d+)-g(?P<hash>\w*))?")

        a = verstr_regex.search(verstr_a)
        b = verstr_regex.search(verstr_b)

        for idx in [2, 3, 4]:
            a_val = int(a.group(idx))
            b_val = int(b.group(idx))
            if a_val == b_val:
                continue
            else:
                return a_val >= b_val

        a_rc = int((a.group('rc') or 999999))
        b_rc = int((b.group('rc') or 999999))

        if a_rc == b_rc:
            a_ncommits = int((a.group('ncommits') or 0))
            b_ncommits = int((b.group('ncommits') or 0))
            return a_ncommits >= b_ncommits
        else:
            return a_rc >= b_rc

    def fw_at_least(self, reference_ver):
        if not self.fw_version:
            self.write_nmea("$CCCFQ,info")
            sleep(1)
            if not self.fw_version:
                self.fw_version = 'v0.0.0'

        return self.version_a_at_least_b(self.fw_version, reference_ver)
    ...
    def _process_incoming_nmea(self, msg):
        if msg is not None:
            try:
                ...
                # # handle janus
                # if 'TJX' in msg.type:
                #     self.current_janus_packet = (self.parser.parse(msg), msg)
    ...
    def on_rxframe(self, dataframe):
        ...
        for func in self.rxframe_listeners:
            try:
                ...
                self._daemon_log.debug(f"ON_RXFRAME Dataframe: {dataframe}")
                try:
                    self._daemon_log.warn(f"Error in rxframe_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in rxframe_listener [{func}]: {exc}\n{traceback.format_exc()}")
    ...
    def on_packetrx_success(self, current_packet):
        self._daemon_log.info("Packet RX succeeded")

        # Callbacks
        self.current_legacy_packet = FDPacket(current_packet.cycleinfo.src, current_packet.cycleinfo.dest, None, current_packet.cycleinfo.rate_num, current_packet.cycleinfo.ack)
        self.current_legacy_packet.load_legacy_frames(current_packet.frames)

        for func in self.packetrx_listeners:
            try:
                func(current_packet)
            except Exception as exc:
                self._daemon_log.debug(f'ON_PACKETRX Packet: {current_packet}')
                try:
                    self._daemon_log.warn(f"Error in packetrx_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in packetrx_listener [{func}]: {exc}\n{traceback.format_exc()}")

        # Append this message to all listening queues
        for q in self.packetrx_queues:
            try:
                q.put_nowait(current_packet)
            except:
                self._daemon_log.warn("Error appending to incoming dataframe queue")
    ...
    def on_ack(self, ack, msg):
        ...
        for func in self.ack_listeners:
            try:
                ...
                self._daemon_log.debug(f'ON_ACK Ack: {ack} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in ack_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in ack_listener [{func}]: {exc}\n{traceback.format_exc()}")
    ...
    def on_cst(self, cst, msg):
        ...
        for func in self.cst_listeners:
            try:
                ...
                self._daemon_log.debug(f'ON_CST: {cst} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in cst_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in cst_listener [{func}]: {exc}\n{traceback.format_exc()}")
        ...
        # Attach CST to an in-progress JANUS reception
        # Hacked way to complete a JANUS reception with
        # cycle stats included
        if self.current_janus_packet is not None:
            self.current_janus_packet[0].add_cyclestats(cst)

            # Raise JANUS receive event
            self.on_cajrx(self.current_janus_packet[0], self.current_janus_packet[1])

            # Reset JANUS state
            self.current_janus_packet = None

        # Attach CST to an in-progress FDP reception
        # Hacked way to complete an FDP reception with
        # cycle stats included
        if self.current_fdpacket is not None:
            self.current_fdpacket.add_cyclestats(cst)

            # Raise FDP receive event
            self.on_fdp(self.current_fdpacket)

            # Reset FDP state
            self.current_fdpacket = None

        # Attach CST to an in-progress legacy reception
        # Hacked way to complete an legacy reception with
        # cycle stats included
        if self.current_legacy_packet is not None:
            self.current_legacy_packet.add_cyclestats(cst)

            # Raise FDP receive event
            self.on_legacy_rx(self.current_legacy_packet)

            # Reset FDP state
            self.current_legacy_packet = None

        # Handle adding CST to JANUS packets
        if self.current_janus_packet is not None:
            self.current_janus_packet.add_cyclestats(cst)
            self.on_janus_rx(self.current_janus_packet)
            self.current_janus_packet = None

    def on_causb2(self, causb2, msg):
        self._daemon_log.debug("Got CAUSB2 message")

        for func in self.causb2_listeners:
            func(causb2, msg)

        for q in self.incoming_causb2_queues:
            try:
                q.put_nowait(causb2)
            except:
                self._daemon_log.warn("Error appending to incoming CAUSB2 queue")

    def on_catjn(self, catjn, msg):
        self._daemon_log.debug("Got CATJN message")

        for func in self.catjn_listeners:
            func(catjn, msg)

        for q in self.incoming_catjn_queues:
            try:
                q.put_nowait(catjn)
            except:
                self._daemon_log.warn("Error appending to incoming CATJN queue")

    def on_cajrx(self, cajrx, msg):
        self._daemon_log.debug("Got CAJRX message")

        # check to see if the JANUS packet is good.  Only start an active packet if it is good
        if cajrx.header_crc_check == 1 and cajrx.data_crc_check == 1:
            self.current_janus_packet = FDPacket(cajrx.src_id, cajrx.dest_id, None,
                                                  -1, cajrx.ack)
            self.current_janus_packet.load_janus_frames(cajrx.hex_data)

        for func in self.cajrx_listeners:
            func(cajrx, msg)

        for q in self.incoming_cajrx_queues:
            try:
                q.put_nowait(cajrx)
            except:
                self._daemon_log.warn("Error appending to incoming CAJRX queue")

    def on_casst(self, casst, msg):
        self._daemon_log.debug("Got CASST message")

        for func in self.casst_listeners:
            try:
                func(casst, msg)
            except Exception as exc:
                self._daemon_log.debug(f'ON_CASST: {casst} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in casst_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in casst_listener [{func}]: {exc}\n{traceback.format_exc()}")

        for q in self.incoming_casst_queues:
            try:
                q.put_nowait(casst)
            except:
                self._daemon_log.warn("Error appending to incoming CASST queue")

    def on_snnst(self, snnst: NavStatistics, msg: Message) -> None:

        for func in self.snnst_listeners:
            try:
                func(snnst, msg)
            except Exception as exc:
                self._daemon_log.debug(f'ON_SNNST: {snnst} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in snnst_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in snnst_listener [{func}]: {exc}\n{traceback.format_exc()}")

        for q in self.incoming_snnst_queues:
            try:
                q.put_nowait(snnst)
            except:
                self._daemon_log.warn("Error appending to incoming SNNST queue")

    def on_snpdt(self, snpdt: NavPingDigitalTransponder, msg: Message) -> None:

        for func in self.snpdt_listeners:
            try:
                func(snpdt, msg)
            except Exception as exc:
                self._daemon_log.debug(f'ON_SNPDT: {snpdt} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in snpdt_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in snpdt_listener [{func}]: {exc}\n{traceback.format_exc()}")

        for q in self.incoming_snpdt_queues:
            try:
                q.put_nowait(snpdt)
            except:
                self._daemon_log.warn("Error appending to incoming SNPDT queue")

    def on_snpgt(self, snpgt: NavPingGenericTransponder, msg: Message) -> None:

        for func in self.snpgt_listeners:
            try:
                func(snpgt, msg)
            except Exception as exc:
                self._daemon_log.debug(f"ON_SNPGT: {snpgt} Message: {msg}")
                try:
                    self._daemon_log.warn(f"Error in snpgt_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in snpgt_listener [{func}]: {exc}\n{traceback.format_exc()}")

        for q in self.incoming_snpgt_queues:
            try:
                q.put_nowait(snpgt)
            except:
                self._daemon_log.warn("Error appending to incoming SNPGT queue")

    def on_sntta(self, sntta: NavTurnTimes, msg: Message) -> None:

        for func in self.sntta_listeners:
            try:
                func(sntta, msg)
            except Exception as exc:
                self._daemon_log.debug(f'ON_SNTTA: {sntta} Message: {msg}')
                try:
                    self._daemon_log.warn(f"Error in sntta_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in sntta_listener [{func}]: {exc}\n{traceback.format_exc()}")

        for q in self.incoming_sntta_queues:
            try:
                q.put_nowait(sntta)
            except:
                self._daemon_log.warn("Error appending to incoming SNTTA queue")

    def on_carev(self, msg):
        self._daemon_log.debug("Got CAREV message")

        for func in self.carev_listeners:
            try:
                func(msg)
            except Exception as exc:
                self._daemon_log.debug(f'ON_CAREV: {msg}')
                try:
                    self._daemon_log.warn(f"Error in carev_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in carev_listener [{func}]: {exc}\n{traceback.format_exc()}")

    def on_ping(self, ping, msg):
        self._daemon_log.debug("Got PING")
        if ping.is_reply():
            for q in self.incoming_ping_reply_queues:
                try:
                    q.put_nowait(ping)
                except Exception as ex:
                    self._daemon_log.error("Error appending to incoming ping queue: " + str(sys.exc_info()[0]))

            for func in self.ping_reply_listeners:
                func(ping, msg)
        else:
            for q in self.incoming_ping_queues:
                try:
                    q.put_nowait(ping)
                except Exception as ex:
                    self._daemon_log.error("Error appending to incoming ping queue: " + str(sys.exc_info()[0]))

            for func in self.ping_listeners:
                func(ping, msg)

    def on_legacy_rx(self,legacy_packet):
        self._daemon_log.info("Got Legacy packet")

        # Append packet to all listening queues
        for q in self.incoming_all_packet_queues:
            try:
                q.put_nowait(legacy_packet)
            except:
                self._daemon_log.warn("Error appending to incoming legacy queue")

    def on_janus_rx(self, janus_packet: FDPacket):
        self._daemon_log.info("Got JANUS packet")

        # Append packet to all listening queues
        for q in self.incoming_all_packet_queues:
            try:
                q.put_nowait(janus_packet)
            except:
                self._daemon_log.warn("Error appending JANUS packet to incoming packet queue")

    def on_fdp(self, fdpacket):
        self._daemon_log.info("Got FDP packet")

        # Execute Callbacks
        for func in self.fdp_listeners:
            try:
                func(fdpacket)
            except Exception as exc:
                self._daemon_log.debug(f'ON_FDP: {fdpacket}')
                try:
                    self._daemon_log.warn(f"Error in fdp_listener [{func}]: {(exc,)}", exc_info=True)
                except:
                    self._daemon_log.warn(f"Error in fdp_listener [{func}]: {exc}\n{traceback.format_exc()}")

        # Append packet to all listening queues
        for q in self.incoming_fdp_queues:
            try:
                q.put_nowait(fdpacket)
            except:
                self._daemon_log.warn("Error appending to incoming FDP queue")

        for q in self.incoming_all_packet_queues:
            try:
                q.put_nowait(fdpacket)
            except:
                self._daemon_log.warn("Error appending to incoming all packet queue")
    ...
    def on_campr(self, campr):
        self._daemon_log.debug("CAMPR received.")
        for func in self.mpr_listeners:
            func(campr)

    def on_camua(self, camua):
        self._daemon_log.debug("Minipacket received")
        for func in self.camua_listeners:
            func(camua)
    ...
    def send_fdpacket(self, fdpacket, use_tdp=False):
        self.state.send_fdpacket(fdpacket, use_tdp)
    ...
    def send_single_shot_sst(self):
        # Build the single shot CCSST message ($CCSST,1)
        msg = {'type': 'CCSST', 'params': [1]}

        self.write_nmea(msg)

    def send_fdp_ping(self, dest_id, rate=1, cdr = 0, hexdata=bytearray()):
        if self.fw_at_least("v3.0.0"):
            if cdr & 0b11000011:
                self._daemon_log.error("Invalid CDR value: {} for FDP Ping".format(cdr))
                return
            if len(hexdata) > 32:
                self._daemon_log.error("Invalid hexdata too long length: {} for FDP Ping. Max 32".format(len(hexdata)))
                return

            self.send_fdp_ping_v2(dest_id, rate, cdr, hexdata)
        else:
            self.send_fdp_ping_v1(dest_id)

    def send_fdp_ping_v2(self, dest_id, rate, cdr, hexdata=bytearray()):
        msg = {'type': 'CCCMD', 'params': ["PNG", self.id, dest_id, rate, cdr, hexstring_from_data(hexdata)]}

        self.write_nmea(msg)

    def send_fdp_ping_v1(self, dest_id):
        msg = {'type': 'CCCMD', 'params': ["PNG", self.id, dest_id, 1, 0]}

        self.write_nmea(msg)

    def wait_for_ping_reply(self, dest_id, timeout=30, include_cst=False):
        time = None
        cst = None
        ping_reply = self.wait_for_nmea_type('CAMPR', timeout=timeout)
        if include_cst:
            cst = self.wait_for_cst(timeout=1)
    ...
    def attach_incoming_ping_reply_queue(self, queue_to_attach):
        self.incoming_ping_reply_queues.append(queue_to_attach)

    def detach_incoming_ping_reply_queue(self, queue_to_detach):
        self.incoming_ping_reply_queues.remove(queue_to_detach)

    def wait_for_fdp_ping_reply(self, timeout=5, include_cst=True):
        incoming_ping_reply_queue = Queue()
        incoming_cst_queue = Queue()
        cst = None
        self.attach_incoming_ping_reply_queue(incoming_ping_reply_queue)

        try:
            ping = incoming_ping_reply_queue.get(block=True, timeout=timeout)
            if include_cst:
                self.attach_incoming_cst_queue(incoming_cst_queue)

                try:
                    cst = incoming_cst_queue.get(block=True, timeout=1)
                except Empty:
                    cst = None
        ...
        self.detach_incoming_ping_reply_queue(incoming_ping_reply_queue)
    ...
    def send_pgt(self, mode,txsig, rxsig1, rxsig2, rxsig3, rxsig4,bwtx,bwrx,timeout_ms):
        ...
        msg = {'type': 'CCPGT',
               'params': [mode, txc.carrierHz, codelen, txc.hexcodestr, timeout_ms, rx1.carrierHz, codelen,
                          rx1.hexcodestr, rx2.hexcodestr, rx3.hexcodestr, rx4.hexcodestr, bwtx, bwrx, 0]}
        self.write_nmea(msg)

    def send_tjn(self, dest_id, ack, hex_header, hex_data, reserved=0):
        self._daemon_log.info("sending $CCTJN %s"% (str([reserved, dest_id, ack, hex_header, hex_data])))
        msg = {'type': 'CCTJN',
               'params': [reserved, dest_id, ack, hex_header, hex_data]}
        self.write_nmea(msg)

    def send_pnt(self, tx_freq_hz, tx_duration_ms, rx_duration_ms, timeout_ms, rx_freqs_hz, sync_flag=False):
    ...
    def send_acm(self, params):
        msg = {'type': 'CCACM', 'params': params}
        self.write_nmea(msg)
    ...
    def set_time(self, time_to_set=None, mode=None, ignore_response=False, extpps_drive_fxn=None):
        ''' Set the modem clock to the specified time (or the current time of the host system if time_to_set is not
         specified.
         Requires that the modem support the $CCTMS command, which any modern firmware does.
        '''
    ...
    def attach_packetrx_queue(self, queue_to_attach):
        self.packetrx_queues.append(queue_to_attach)

    def detach_packetrx_queue(self, queue_to_detach):
        self.packetrx_queues.remove(queue_to_detach)
    ...
    def attach_incoming_causb2_queue(self, queue_to_attach):
        self.incoming_causb2_queues.append(queue_to_attach)

    def detach_incoming_causb2_queue(self, queue_to_detach):
        self.incoming_causb2_queues.remove(queue_to_detach)

    def attach_incoming_catjn_queue(self, queue_to_attach):
        self.incoming_catjn_queues.append(queue_to_attach)

    def detach_incoming_catjn_queue(self, queue_to_detach):
        self.incoming_catjn_queues.remove(queue_to_detach)

    def attach_incoming_cajrx_queue(self, queue_to_attach):
        self.incoming_cajrx_queues.append(queue_to_attach)

    def detach_incoming_cajrx_queue(self, queue_to_detach):
        self.incoming_cajrx_queues.remove(queue_to_detach)

    def attach_incoming_casst_queue(self, queue_to_attach):
        self.incoming_casst_queues.append(queue_to_attach)

    def detach_incoming_casst_queue(self, queue_to_detach):
        self.incoming_casst_queues.remove(queue_to_detach)

    def attach_incoming_snnst_queue(self, queue_to_attach):
        self.incoming_snnst_queues.append(queue_to_attach)

    def detach_incoming_snnst_queue(self, queue_to_detach):
        self.incoming_snnst_queues.remove(queue_to_detach)

    def attach_incoming_snpdt_queue(self, queue_to_attach):
        self.incoming_snpdt_queues.append(queue_to_attach)

    def attach_incoming_snpgt_queue(self, queue_to_attach):
        self.incoming_snpgt_queues.append(queue_to_attach)

    def detach_incoming_snpdt_queue(self, queue_to_detach):
        self.incoming_snpdt_queues.remove(queue_to_detach)

    def detach_incoming_snpgt_queue(self, queue_to_detach):
        self.incoming_snpgt_queues.remove(queue_to_detach)

    def attach_incoming_sntta_queue(self, queue_to_attach):
        self.incoming_sntta_queues.append(queue_to_attach)

    def detach_incoming_sntta_queue(self, queue_to_detach):
        self.incoming_sntta_queues.remove(queue_to_detach)
...
class Message(dict):
    ...
    def __repr__(self):
        message = ",".join([str(p) for p in [self['type']] + self['params']])
        chk = nmeaChecksum(message)
        message = "$" + message.lstrip('$').rstrip('\r\n*') + "*" + chk + "\r\n"
