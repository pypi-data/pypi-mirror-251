...
class QueryDialog(object):
	def __init__(self, column_headers, db_conn, init_sql=""):
		...
		ttk.Button(chars_frame, text="not", width=3, command=lambda:self.sql_text.insert(tk.END, " not ")).grid(row=0, column=2, sticky=tk.W)
		ttk.Button(chars_frame, text="in", width=2, command=lambda:self.sql_text.insert(tk.END, " in ")).grid(row=0, column=3, sticky=tk.W)
		ttk.Button(chars_frame, text="(", width=1, command=lambda:self.sql_text.insert(tk.END, "(")).grid(row=0, column=4, sticky=tk.W)
		ttk.Button(chars_frame, text=",", width=1, command=lambda:self.sql_text.insert(tk.END, ",")).grid(row=0, column=5, sticky=tk.W)
		ttk.Button(chars_frame, text=")", width=1, command=lambda:self.sql_text.insert(tk.END, ")")).grid(row=0, column=6, sticky=tk.W)
		ttk.Button(chars_frame, text="and", width=3, command=lambda:self.sql_text.insert(tk.END, " and ")).grid(row=0, column=7, sticky=tk.W)
		ttk.Button(chars_frame, text="or", width=2, command=lambda:self.sql_text.insert(tk.END, " or ")).grid(row=0, column=8, sticky=tk.W)
		ttk.Button(chars_frame, text="%", width=2, command=lambda:self.sql_text.insert(tk.END, "%")).grid(row=0, column=9, sticky=tk.W)
		ttk.Button(chars_frame, text="_", width=1, command=lambda:self.sql_text.insert(tk.END, "_")).grid(row=0, column=10, sticky=tk.W)
		ttk.Button(chars_frame, text="Clear", width=5, command=lambda:self.sql_text.delete(1.0, tk.END)).grid(row=0, column=11, sticky=tk.E, padx=(3,24))
		chars_frame.columnconfigure(11, weight=1)
		# SQL text entry
		self.sql = init_sql
		...
		if self.plot_data is not None and len(self.plot_data) > 0 and len(self.plot_data[0]) > 0:
			if plot_type == "Category counts":
				...
				if self.linreg and self.x_var.get() in self.numeric_columns:
					...
					self.plot_axes.axline((xmean, ymean), slope=slope, clip_box=bbox, clip_on=True, label="Linear fit", color="black", linestyle="dotted", alpha=0.65)
					if show_regression_stats:
						...
						statdlg.show()
				if self.groupby_var.get() != '' or ((self.loess or self.linreg) and self.x_var.get() in self.numeric_columns):
...
class UnivarStatsDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.dnames = ["Variable", " N ", "Min.", "Max.", "Mean", "Median", "Mode", \
				"Geo. mean", "Std.Dev.", "5th %ile", "95th %ile", "Anderson-Darling p", \
				"Lillefors p", "Rosner's outliers", "Tukey outliers"]
		self.logdnames = ["Variable", " N ", "Min.", "Max.", "Mean", "Median", "Mode", \
				"Std.Dev.", "5th %ile", "95th %ile", "Anderson-Darling p", "Lillefors p", \
		...
		if self.data_labels is not None:
			for i in range(len(self.data_labels)):
				...
				if len(d) > 0:
					...
					dd = [self.data_labels[i], len(d), min(d), max(d), round_figs(statistics.fmean(d), 4)]
					...
					if len(logd) > 0:
						logdd.extend([round_figs(min(logd),4), round_figs(max(logd),4), round_figs(statistics.fmean(logd), 4)])
					else:
...
class HelpHotkeysDialog(object):
	def __init__(self):
		...
		ttk.Label(keyframe, width=10, text="Alt-T").grid(row=8, column=0, sticky=tk.N+tk.W)
		ttk.Label(keyframe, width=50, justify=tk.LEFT, wraplength=350, text="Add or change the plot title.").grid(row=8, column=1)
		ttk.Label(keyframe, width=10, text="Alt-X").grid(row=9, column=0, sticky=tk.N+tk.W)
		ttk.Label(keyframe, width=50, justify=tk.LEFT, wraplength=350, text="Change the label on the plot's X axis.").grid(row=9, column=1)
		ttk.Label(keyframe, width=10, text="Alt-Y").grid(row=10, column=0, sticky=tk.N+tk.W)
		ttk.Label(keyframe, width=50, justify=tk.LEFT, wraplength=350, text="Change the label on the plot's Y axis.").grid(row=10, column=1)
