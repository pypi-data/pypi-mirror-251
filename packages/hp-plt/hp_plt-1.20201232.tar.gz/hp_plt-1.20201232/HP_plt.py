...
def ax_K(ax, df, t, n=2):
    ...
    candlestick_ohlc(ax, days.values, width=.6,
...
def ax_MACD(ax1, mydf):
    ...
    df = mydf.copy()
    fillcolor = '#00ffe8'
    ax1.plot(df.date.values, df.DIFF.values,
             color=rsiCol, lw=1, label="$DIFF$")
    ax1.plot(df.date.values, df.MACD.values, color=negCol, lw=1, label="$DEA$")
    ax1.fill_between(df.date.values, df.MACD.values, 0,
...
def ax_HPYYX(ax2, mydf):
    ...
    ax2.plot(df.date.values, df.ZL.values,
             color=g.cns['cornflowerblue'], lw=3, label="$ZL$")
    ax2.plot(df.date.values, df.HT.values,
             color=g.cns['plum'], lw=2, label="$HT$", linestyle=':')
...
def draw_UFN(ax1, mydf):
    ax2 = plt.subplot2grid((7, 4), (5, 0), sharex=ax1,
                           rowspan=2, colspan=4, fc=g.ubg)
    CLOSE = mydf['close']
    LOW = mydf['low']
    HIGH = mydf['high']
    OPEN = mydf['open']
...
def ax_HPCPX(ax1, mydf):
    ...
    plt.subplots_adjust(left=.075, bottom=.08, right=.96,
                        top=.96, wspace=.15, hspace=0.1)
    # plt.close() # 关窗口
    return ax1
