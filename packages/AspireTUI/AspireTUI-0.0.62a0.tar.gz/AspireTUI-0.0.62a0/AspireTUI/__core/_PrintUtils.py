...
class _StatusEnumV3(_Enum):
	...
	"""
	class Status:
		def __init__(self, id, uni, tty):
			self.id = id
			self.uni = uni
			self.tty = tty

	Fail = Status(False, f"{_cat.front.red}{_cat.text.bold} X {_cat.reset}", f"{_cat.front.red}{_cat.text.bold}FAIL{_cat.reset}")
	Done = Status(True, f"{_cat.front.green}{_cat.text.bold} √ {_cat.reset}", f"{_cat.front.green}{_cat.text.bold}DONE{_cat.reset}")
	DEBUG = Status(1000 + LOG_LEVEL.DEBUG.value, " 🐞 ", "DBUG")
	INFO = Status(1000 + LOG_LEVEL.INFO.value, "ℹ️ℹ️ℹ️", "INFO")
	Warning = Status(1000 + LOG_LEVEL.WARNING.value, " ⚠️ ", "WARN")
	ERROR = Status(1000 + LOG_LEVEL.ERROR.value, " ❌ ", "EROR")
	CRITICAL = Status(1000 + LOG_LEVEL.CRITICAL.value, " 🔴 ", "CRIT")
	FATAL = Status(1000 + LOG_LEVEL.FATAL.value, " ☠️ ", "FATL")
	Off = Status(int(False) + 10, f"{_cat.front.red}{_cat.text.bold} ○ {_cat.reset}", f"{_cat.front.red}{_cat.text.bold}Off {_cat.reset}")
	On = Status(int(True) + 10, f"{_cat.front.green}{_cat.text.bold} ● {_cat.reset}", f"{_cat.front.green}{_cat.text.bold} On {_cat.reset}")
	Todo = Status(2, f"{_cat.front.cyan}{_cat.text.bold} ≡ {_cat.reset}", f"{_cat.front.cyan}{_cat.text.bold}TODO{_cat.reset}")
	Work = Status(3, f"{_cat.front.yellow}{_cat.text.bold} ∞ {_cat.reset}", f"{_cat.front.yellow}{_cat.text.bold}WORK{_cat.reset}")
	Skip = Status(4, " » ", "Skip")
	Next = Status(5, " > ", "Next")
	Prev = Status(6, " < ", "Prev")
	Info = Status(111, "ℹ️ℹ️ℹ️", "Info")

class _StatusEnum(_Enum):
	Status = _StatusEnumV3
	class StatusNOPE(_Enum):
		Fail = (False, f"{_cat.front.red}{_cat.text.bold} X {_cat.reset}", f"{_cat.front.red}{_cat.text.bold}FAIL{_cat.reset}")
		Done = (True, f"{_cat.front.green}{_cat.text.bold} √ {_cat.reset}", f"{_cat.front.green}{_cat.text.bold}DONE{_cat.reset}")
		DEBUG = (1000 + LOG_LEVEL.DEBUG.value, " 🐞 ", "DBUG")
		INFO = (1000 + LOG_LEVEL.INFO.value, "ℹ️ℹ️ℹ️", "INFO")
		Warning = (1000 + LOG_LEVEL.WARNING.value, " ⚠️ ", "WARN")
		ERROR = (1000 + LOG_LEVEL.ERROR.value, " ❌ ", "EROR")
		CRITICAL = (1000 + LOG_LEVEL.CRITICAL.value, " 🔴 ", "CRIT")
		FATAL = (1000 + LOG_LEVEL.FATAL.value, " ☠️ ", "FATL")
		Off = (int(False) + 10, f"{_cat.front.red}{_cat.text.bold} ○ {_cat.reset}", f"{_cat.front.red}{_cat.text.bold}Off {_cat.reset}")
		On = (int(True) + 10, f"{_cat.front.green}{_cat.text.bold} ● {_cat.reset}", f"{_cat.front.green}{_cat.text.bold} On {_cat.reset}")
		Todo = (2, f"{_cat.front.cyan}{_cat.text.bold} ≡ {_cat.reset}", f"{_cat.front.cyan}{_cat.text.bold}TODO{_cat.reset}")
		Work = (3, f"{_cat.front.yellow}{_cat.text.bold} ∞ {_cat.reset}", f"{_cat.front.yellow}{_cat.text.bold}WORK{_cat.reset}")
		Skip = (4, " » ", "Skip")
		Next = (5, " > ", "Next")
		Prev = (6, " < ", "Prev")
		Info = (111, "ℹ️ℹ️ℹ️", "Info")
...
def status(ID: _Union[int, bool, _Enum], seperators="[]"):
	...
	#		val_member = member
	#		break
	for enum_class in _StatusEnum.__members__.values():
		for member in enum_class:
			if getattr(member, 'id', None) == val_ret:
				val_member = member
				break
		if val_member:
			break
