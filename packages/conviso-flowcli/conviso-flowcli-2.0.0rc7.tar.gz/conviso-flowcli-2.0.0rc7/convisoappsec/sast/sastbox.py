...
class SASTBox(object):
    ...
    def _scan_diff(self, code_dir, current_commit, previous_commit, log, token):
        ...
        command = (
            ''' -c {container_dir} --diff={previous_commit},{current_commit} -a -o {report_dir} -v --dedup'''.format(
                container_dir=self.CONTAINER_CODE_DIR, previous_commit=previous_commit, current_commit=current_commit, report_dir=report_dir)
        )  # noqa: E501
