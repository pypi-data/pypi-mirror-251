...
def handle_mrz_extraction(client, image_data, back_id_text, back_data, back_data_dict):
    ...
    return back_data_new, third_part_text
...
#         issuing_authority = ''
    ...
    if empty_string_keys or len(back_data_dict['nationality'])>3 or len(back_data_dict['id_number'])<12 or not back_data_dict.get('mrz2'):
        print("handling")
        back_res, third_part_text = handle_mrz_extraction(client, image_data, back_id_text, back_data, back_data_dict)
        back_data_dict.update(back_res)

    if not back_data_dict.get('first_name_back'):
        print("handling name")
        back_res = handle_name_extraction(third_part_text)
        back_data_dict.update(back_res)
