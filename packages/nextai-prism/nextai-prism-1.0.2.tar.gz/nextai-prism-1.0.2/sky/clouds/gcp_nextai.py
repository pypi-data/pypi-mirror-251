...
def _run_output(cmd, access_token: Optional[str] = None):
...
def is_api_disabled(endpoint: str, project_id: str, access_token: Optional[str] = None) -> bool:
...
class GCPNextAI(clouds.Cloud):
    """Google Cloud Platform via NextAI"""

    _REPR = 'GCP'

    _REPR_ALIAS = 'GCPNextAI'
    ...
    def is_same_cloud(self, other):
        return isinstance(other, GCPNextAI)
    ...
    def make_deploy_resources_variables(
        ...
        resources_vars['disk_tier'] = GCPNextAI._get_disk_type(r.disk_tier)
    ...
    def _get_feasible_launchable_resources(
        ...
        if resources.accelerators is None:
            ...
            host_vm_type = GCPNextAI.get_default_instance_type(
                cpus=resources.cpus,
    ...
    @classmethod
    def _find_application_key_path(cls, credentials_path: Optional[str] = None) -> str:
        ...
        application_key_path = credentials_path if credentials_path is not None else os.environ.get(_GCP_APPLICATION_CREDENTIAL_ENV, None)
    ...
    @classmethod
    def check_credentials(cls, credentials_path: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        """Checks if the user has access credentials to this cloud."""

        """ Set the credentials path so that everyone can access"""
        ...
        try:
            credentials_path = cls._find_application_key_path(credentials_path=credentials_path)
        except FileNotFoundError as e:
            return False, (
        ...
        try:
            ...
            project_id = cls.get_project_id(credentials_path=credentials_path)

            # Note: Skipping the check of user activation / service account activation we are using access tokens directly rather than using ADC (Application Default Credentails)
        ...
        credentials, project = google.auth.load_credentials_from_file(credentials_path) if credentials_path is not None else google.auth.default()
        ...
        if len(diffs) > 0:
            identity_str = credentials.service_account_email
            return False, (
    ...
    def get_current_user_identity(cls, credentials_path: Optional[str] = None, access_token: Optional[str] = None) -> Optional[List[str]]:
        """Returns the email address + project id of the active user."""
        try:
            account = _run_output('gcloud auth list --filter=status:ACTIVE '
                                  '--format="value(account)"', access_token=access_token)
    ...
    @classmethod
    def get_project_id(cls, dryrun: bool = False, credentials_path: Optional[str] = None): 
        if dryrun:
            return 'dryrun-project-id'
        # pylint: disable=import-outside-toplevel
        from google import auth  # type: ignore

        _, project_id = auth.load_credentials_from_file(filename=credentials_path) if credentials_path is not None else auth.default();

        # Retrieve access token
        cls.get_access_token(_)
    ...
    @classmethod
    def get_access_token(cls, _) -> str:
        # pylint: disable=import-outside-toplevel
        from google.auth import impersonated_credentials
        from google.auth.transport.requests import Request
        access_token = os.environ.get('CLOUDSDK_AUTH_ACCESS_TOKEN', None)
        if access_token is None: 
            # Generate impersonated service account credentials via the above file based credentials
            credentials = impersonated_credentials.Credentials(source_credentials=_, target_principal=_.service_account_email, delegates=[], lifetime=3600, target_scopes=[ "https://www.googleapis.com/auth/cloud-platform" ])
            # Request for an access token via auth library
            auth_request = Request()
            # Obtain the access token return it
            credentials.refresh(auth_request)
            access_token = credentials.token
            os.environ["CLOUDSDK_AUTH_ACCESS_TOKEN"] = access_token;
            if access_token is None:
                raise exceptions.CloudUserIdentityError(
                    'Failed to get access token from given credentials. Please make sure you have correct scopes: \nroles/iam.serviceAccountTokenCreator\nroles/iam.serviceAccountUser')
        return access_token
