...
from qat.internal import debug_operations
...
def get_temp_folder() -> str:
    ...
    if 'TEMP' not in os.environ:
        if is_windows():
            ...
            mktemp = subprocess.check_output(['mktemp', '-u'], text=True, shell=False)
            result = subprocess.check_output(
                ['dirname', mktemp.strip()], text=True, shell=False)
            os.environ['TEMP'] = result.strip()
        else:
...
def connect_to(context: app_ctxt.ApplicationContext):
    ...
    if port < 0:
        if last_error is not None:
            print(f"Error reading pid file {error}")
        context.kill()
    ...
    # Lock GUI in release mode
    lock_ui = Settings.lock_ui.lower()
    if lock_ui == "always" or (lock_ui == "nodebug" and not is_debugging()):
        ...
        if is_linux():
            try:
                with subprocess.Popen(
                    ['ps', '-e'], stdout=subprocess.PIPE, text=True, shell=False) as ps_proc:
                    result = subprocess.check_output(
                        ['grep', '-w', f'{pid}'], stdin=ps_proc.stdout, text=True, shell=False)
            ...
            try:
                result = subprocess.check_output(
                    ['powershell', '-command', command], text=True, shell=False)
            except subprocess.CalledProcessError:
                continue
        result = result.strip()
        if len(result) == 0:
