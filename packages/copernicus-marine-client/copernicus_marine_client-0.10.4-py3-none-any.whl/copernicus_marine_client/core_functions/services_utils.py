...
from copernicus_marine_client.core_functions import sessions
...
def _get_first_available_service_type(
    ...
    if dataset_version_part.name != PART_DEFAULT:
        logger.warning(
            "Dataset part is not default, forcing using FTP service"
        )
...
def _get_retrieval_service_from_dataset(
    ...
    if force_dataset_version_label:
        ...
        logger.info(f'Selected dataset version is "{dataset_version.label}"')
...
def _get_retrieval_service_from_dataset_version(
    ...
    if not force_dataset_part_label:
        logger.info(f'Selected dataset part is "{dataset_part.name}"')
    if force_service_type:
        ...
        service = _select_forced_service(
            dataset_part, force_service_type, command_type
        )
    else:
        service = _select_service_by_priority(
            dataset_part, command_type, dataset_subset
        )
        logger.info(f'Selected service is "{service.service_type}"')
