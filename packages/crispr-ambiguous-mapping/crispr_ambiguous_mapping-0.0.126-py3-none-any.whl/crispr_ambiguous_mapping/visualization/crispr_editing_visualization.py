...
def plot_mutation_count_histogram(mutation_counter):
    ...
    # Create a bar plot
    plt.bar(indexes, counts, color='blue')

    # Add labels and title
    plt.xlabel('Mutations Per Allele')
    plt.ylabel('Read Count')
    plt.title('Bar Plot of Counts Sorted by Index')
...
def plot_trinucleotide_mutational_signature(unlinked_mutations_df: pd.DataFrame, label: str, possible_refs: List[str] = ["A","C","G","T"], possible_alts: List[str] = ["A","C","G","T","X","N","-","X"], empty_context_character: str = "_"):
...
def plot_positional_mutational_signature(unlinked_mutations_df: pd.DataFrame, label: str, possible_refs: List[str] = ["A","C","G","T"], possible_alts: List[str] = ["A","C","G","T","X","N","-","X"], empty_context_character: str = "_", min_position: int = 0, max_position: Optional[int] = None):
    fig, ax = plt.subplots(1, figsize=(15, 5*1))
