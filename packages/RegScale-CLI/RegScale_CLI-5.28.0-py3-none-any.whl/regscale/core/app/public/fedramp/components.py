...
def create_asset(component_data: Dict, user_id: str, spp_id: int) -> Dict:
    """
...
def create_component_based_on_type(component_data, user_id, ssp_id, api_handler):
    """
    ...
    """
    if component_data["type"] == "interconnection":
        response = create_interconnection(component_data, user_id, ssp_id, api_handler)
        return response.get("id", 0), "interconnects"

    elif component_data["type"] in ["subnet", "hardware", "software"]:
        response = create_asset(component_data, user_id, ssp_id)
        return response.get("id", 0), "assets"

    else:
        response = create_component(component_data, ssp_id, user_id, api_handler)
        return response.get("id", 0), "components" if response else ("components", 0)
...
def create_component(
    ...
    resp_json = api_handler.post("/api/components", json=comp.dict())
    if resp_json:
...
def create_interconnection(
    ...
    response = api_handler.post("/api/interconnections", json=intercon.dict())
    return response
