...
def update_regscale_links(app: Application, responses: List[Response]) -> None:
    """
...
def process_threats(
    threats: list[Threat], unique_threats: set[dict], reg_threats: list[dict]
) -> Tuple[list, list]:
    """
    Process threats

    :param list[Threat] threats: List of threats to process
    :param set unique_threats: Set of unique threats
    :param list reg_threats: List of RegScale threats
    ...
    for threat in threats:
        if threat and threat.description in unique_threats:
            old_dict = [
                reg for reg in reg_threats if reg["description"] == threat.description
            ][0]
...
def alerts(year: int) -> None:
    ...
    if len(threats) > 0:
        insert_threats, update_threats = process_threats(
            threats, unique_threats, reg_threats
        )
    ...
    update_regscale_links(app, responses)
...
def parse_html(page_url: str, app: Application) -> list:
    ...
    while control["items"] > 0:
        soup = gen_soup(page_url + f"&page={control['page']}", app)
...
def build_threat(
    app: Application, detailed_link: str, short_description: str, title: str
) -> Threat:
    """
...
def process_element(*args) -> Tuple[dict, str, str]:
    ...
    if (
        last_h3
        and nav_string
        and (dat.text).lower().replace("\n", "") not in div_list
        and last_h3.lower() in div_list
    ):
...
def parse_details(
    link: str, app: Application
) -> Optional[Tuple[str, list, list, list]]:
    """
...
def gen_soup(url: str, app: Application) -> BeautifulSoup:
    """
...
def unique(lst: List[str]) -> List[str]:
    ...
    :param list lst: List to make unique
